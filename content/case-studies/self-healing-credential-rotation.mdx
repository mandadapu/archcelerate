...
## 4. Key Takeaway for AI Architects

AI/ML workflows are particularly sensitive to rotation. If your RAG agents are querying HealthLake and the API key rotates mid-session, the agent needs **"retry with fresh credentials" logic** built in.

Without this, a routine security rotation could cause a **cascade failure** across your AI agents — every agent in the pipeline fails simultaneously because they're all holding a stale credential.

The solution: have your microservices fetch the latest secret from the vault on every `401 Unauthorized` error. This turns a potentially catastrophic rotation event into a single retried request with zero user-visible impact.
...
- **AI agents need retry-on-401 logic** — without it, routine credential rotation becomes a cascade failure across your entire AI pipeline
