...
> **Note**: Updated February 2026 with Claude 4.5 and GPT-5 best practices, including prompt caching, structured outputs, and extended thinking modes.
...
async function intelligentRouting(task: string) {
  ...
  if (complexity === 'simple') {
    ...
    return callModel('claude-3-5-sonnet-20241022')  // $3/$15 per M
  } else {
...
const batch = await anthropic.batches.create({
  requests: [
    { custom_id: 'req1', params: { model: 'claude-3-5-sonnet-20241022', messages: [...] } },
    { custom_id: 'req2', params: { model: 'claude-3-5-sonnet-20241022', messages: [...] } }
  ]
})
