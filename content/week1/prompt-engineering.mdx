---
title: "Prompt Engineering & Self-Healing JSON"
description: "Systematic prompting, retry logic, and structured output extraction"
estimatedMinutes: 45
---
...
**Pattern 4: Hidden Thought Traces with Self-Verification** ⭐
...
`
  ...
  const verificationPassed = verificationCriteria.every(criterion =>
    reasoning.toLowerCase().includes('✓') &&
    ...
    hiddenReasoning: {
      ...
      verification: {
        ...
        checks: verificationCriteria.map(c => ({
          criterion: c,
          result: reasoning.includes(`✓ ${c}`) ? 'pass' : 'fail',
...
function extractEvidence(text: string, criterion: string): string {
  const pattern = new RegExp(`[✓✗]\\s*${criterion}:?\\s*([^\\n]+)`, 'i')
...
if (!response.hiddenReasoning.verification.passed) {
  console.warn('⚠️ Model failed self-verification. Manual review required.')
...
#### Task Complexity → Thinking Budget Matrix
...
```typescript
// ❌ WASTEFUL: Using extended thinking for simple classification
const sentimentResponse = await anthropic.messages.create({
  model: 'claude-opus-4-5-20251101',
  max_tokens: 256,
  thinking: {
    type: 'enabled',
    budget_tokens: 2048  // ⚠️ Unnecessary! Adds 300ms latency + 40% cost
  },
  messages: [{
    role: 'user',
...
// ✅ OPTIMIZED: No thinking for simple tasks
const sentimentResponse = await anthropic.messages.create({
...
// ✅ OPTIMIZED: Thinking budget for complex reasoning
const diagnosisResponse = await anthropic.messages.create({
  model: 'claude-opus-4-5-20251101',
  max_tokens: 2048,
  thinking: {
    type: 'enabled',
    budget_tokens: 4096  // ✅ Justified for medical reasoning
  },
  messages: [{
    role: 'user',
...
## 🛡️ Self-Healing JSON Patterns: Schema Drift & Hallucination Defense
...
async function extractWithSelfHealing(
  ...
  while (attempts < maxRetries) {
    try {
      ...
      const validated = UserSchema.parse(json)  // ← Strict validation

      console.log(`✅ Valid JSON on attempt ${attempts + 1}`)
      return validated
      ...
      console.warn(`⚠️ Attempt ${attempts} failed: ${previousError}`)
...
interface SchemaMonitoring {
  ...
  unexpectedFields: Map<string, number>  // field → occurrence count
  missingFields: Map<string, number>     // field → occurrence count
}
...
function monitorSchemaDrift(
  ...
  if (unexpected.size > 0) {
    console.warn(`⚠️ Schema drift detected: unexpected fields ${[...unexpected.keys()]}`)
  }
  if (missing.size > 0) {
    console.error(`❌ Missing required fields: ${[...missing.keys()]}`)
  }
...
await database.save(data)  // ← SQL injection risk if schema violated

// ✅ SAFE: Always validate
const validated = UserSchema.parse(JSON.parse(response.content[0].text))
await database.save(validated)  // ← Type-safe, validated data
```
...
**Critical Architectural Principle**: Anthropic's prompt caching uses **prefix-matching**—it caches the **beginning** of your message array, not arbitrary sections.

```typescript
// ❌ WRONG: Breaks cache prefix (loses 90% ROI)
const messages = [
  {
    role: 'user',
    content: `Today's date: ${new Date().toISOString()}`  // ⚠️ DYNAMIC DATA FIRST!
  },
  {
    role: 'user',
    content: [
...
// ✅ CORRECT: Static content first (preserves cache)
const messages = [
  {
    role: 'user',
    content: [
...
// Result: Cache HIT on every request → 90% cost savings
```
...
```typescript
// Order matters: Static → Semi-static → Dynamic
const prompt = {
  system: [
    {
      type: 'text',
      text: SYSTEM_INSTRUCTIONS,  // Never changes → Cache Layer 1
      cache_control: { type: 'ephemeral' }
    },
    {
      type: 'text',
      text: TOOL_DEFINITIONS,  // Rarely changes → Cache Layer 2
      cache_control: { type: 'ephemeral' }
    },
    {
      type: 'text',
      text: FEW_SHOT_EXAMPLES,  // Changes weekly → Cache Layer 3
      cache_control: { type: 'ephemeral' }
    }
  ],
  messages: [
    {
      ...
      content: [
        {
          type: 'text',
          text: LARGE_CODEBASE,  // Changes per user → Cache Layer 4
          cache_control: { type: 'ephemeral' }
        },
        {
          type: 'text',
          text: currentUserQuery  // Always dynamic → NOT cached
        }
      ]
    }
  ]
...
// Without caching: 100 × 60K tokens × $3/MTok = $18
// With hierarchical caching:
//   - First request: $0.18 (cache write)
//   - Next 99 requests: 99 × 60K tokens × $0.30/MTok = $1.78
//   - Total: $1.96 (89% savings)
```

**Pattern 2: The Cache-Breaking Mistake**
```typescript
// ❌ Common mistake: Dynamic data in system prompt
const systemPrompt = `
You are a helpful assistant.
Current time: ${new Date().toISOString()}  // ⚠️ Changes every second
Session ID: ${sessionId}  // ⚠️ Changes per user
`
// Result: Cache invalidated on EVERY request

// ✅ Fixed: Move dynamic data to user message
...
```typescript
// ❌ Breaks cache: Millisecond precision
const timestamp = new Date().toISOString()  // 2026-02-05T18:32:45.123Z

// ✅ Preserves cache: Minute precision (60x longer cache window)
const timestamp = new Date().toISOString().slice(0, 16)  // 2026-02-05T18:32

// ✅ Best: No timestamp in cached section at all
...
## 🎯 Architectural Scenario: Production Decision-Making
...
const data = JSON.parse(response.content[0].text)  // ⚠️ Might fail
```

**Cost Analysis**:
- Cost per request: $0.045 (Opus)
- Monthly cost (100K requests): **$4,500** ❌ Over budget
- Expected JSON failures: ~8% (high temp = unpredictable)
- Monthly failure cost: 8,000 failures × $50 = **$400,000** ❌ Catastrophic

---

#### Option B: Low Temperature + JSON Schema + Caching ✅ OPTIMAL
...
- Cached requests: $0.0015 (90% cheaper) ✅
- Average cost (90% cache hit rate): $0.003/request
- Monthly cost (100K requests): **$300** ✅ 10x under budget
- Expected JSON failures: **~0%** (schema enforcement) ✅
- Monthly failure cost: **$0** ✅ No support tickets
- P95 latency: ~280ms ✅ Under 500ms target
...
- Monthly cost (100K requests): **$1,200** ❌ 4x over optimal
- Expected JSON failures: ~3% (no schema enforcement)
- Monthly failure cost: 3,000 failures × $50 = **$150,000** ❌ Catastrophic
- P95 latency: ~420ms ⚠️ Acceptable but higher
...
**Cost Analysis**:
- Cost per request (average): $0.0015 × 1.3 retries = $0.002
- Monthly cost (100K requests): **$200** ✅ Under budget
- Expected JSON failures: ~1% (after 3 retries) ⚠️ Still 1,000 failures
- Monthly failure cost: 1,000 failures × $50 = **$50,000** ❌ Unacceptable
- P95 latency: ~680ms ❌ Over 500ms target (retries add latency)
- Wasted API calls: 30% overhead from retries ❌ Inefficient
...
1. **Guarantees 100% valid JSON** (schema enforcement) → $0 failure cost
2. **Minimizes cost** (90% cache hit rate) → $300/month vs $4,500 budget
3. **Meets latency target** (280ms P95) → Under 500ms requirement
4. **Production-ready pattern** (low temp + caching + validation) → Reliable at scale
...
## 🏗️ Advanced Project: Self-Healing JSON System
...
**Strategy**: Execute → Validate (Zod/Pydantic) → Heal → Resolve
...
CRITICAL: Return ONLY the JSON object. No markdown formatting, no preamble, no explanation.`;
  ...
  try {
    const response = await anthropic.messages.create({
      model: "claude-sonnet-4-5-20251101",
      max_tokens: 1024,
      temperature: 0,  // ✅ Zero randomness for JSON tasks
      messages: [
        {
          role: "user",
          content: rawInput
...
```typescript
// ❌ Common failure: Model returns this
const modelOutput = `
\`\`\`json
{
...
JSON.parse(modelOutput);  // ❌ SyntaxError: Unexpected token
...
```typescript
// ❌ BAD: Temperature adds randomness
const response = await anthropic.messages.create({
  model: 'claude-sonnet-4-5-20251101',
  max_tokens: 1024,
  temperature: 0.7,  // ⚠️ Can introduce syntax errors
  messages: [{ role: 'user', content: 'Extract data as JSON...' }]
});
// Result: 8% failure rate, unpredictable errors

// ✅ GOOD: Zero temperature for deterministic output
const response = await anthropic.messages.create({
  model: 'claude-sonnet-4-5-20251101',
  max_tokens: 1024,
  temperature: 0,  // ✅ Deterministic, consistent structure
  messages: [{ role: 'user', content: 'Extract data as JSON...' }]
});
...
// ⚠️ That 1% failure rate costs MORE than all successful requests combined!
```
  ...
  it('heals all corrupted inputs into valid schema', async () => {
    ...
    for (const testCase of CORRUPTED_INPUTS) {
      ...
      try {
        ...
        console.log(`  ✅ Healed successfully: ${result.triage_level}`);
        ...
        console.log(`  ❌ Failed to heal: ${error instanceof Error ? error.message : error}`);
...
✅ **100% Healing Rate**: All 6 corrupted inputs must be healed into valid schema

✅ **Latency**: &lt;3s per request (including retry attempts)

✅ **Cost Efficiency**: &lt;3 retries per request on average

✅ **Schema Compliance**: Every output passes Zod validation

✅ **Error Logging**: All healing attempts are logged for debugging
...
#   ✅ Healed successfully: MEDIUM
# Testing Challenge 2: Instruction injection attempt with fake JSON
#   ✅ Healed successfully: EMERGENCY
# ...
# ✓ heals all corrupted inputs into valid schema (4523ms)
# ✓ logs healing attempts and final resolution
# ✓ escalates to architectural failure after max retries
# ✓ completes healing within performance SLA
```
...
<Callout type="success" title="Architect Tier Mastery">
This isn't a participation trophy. You achieve **"Architect Tier"** mastery by meeting four **quantified performance benchmarks**. These are the same metrics used by production systems processing millions of requests per day.
</Callout>

A student achieves "Architect Tier" mastery by meeting the following four performance benchmarks:

### 1. Schema Adherence (The "Zero-Crash" Test)

**Metric**: 100% success rate in producing valid JSON across **50 adversarial test cases** (inputs designed to trigger markdown wrapping, nested object hallucinations, or schema bypass attempts).

**Verification Method**:
- Use Zod or Pydantic schema validator
- **Any uncaught `JSON.parse` error = FAIL**
- Test suite must include all adversarial patterns:
...
**Success Criteria**:
```typescript
✅ PASS: 50/50 test cases produce valid schema (100%)
❌ FAIL: 49/50 test cases produce valid schema (98%)
```

**Why This Matters**: In production, a 98% success rate means **2,000 failures per 100K requests**. At $50/failure for manual intervention, that's **$100K/month** in losses. Architect Tier requires **zero tolerance** for schema violations.

**Implementation Example**:
```typescript
describe('Schema Adherence - Zero Crash Test', () => {
  it('achieves 100% success rate across 50 adversarial cases', async () => {
    const testCases = generateAdversarialCases(50);
    let successCount = 0;
    ...
    for (const testCase of testCases) {
      try {
        ...
        ClinicalSummarySchema.parse(result);
        successCount++;
      } catch (error) {
        console.error(`FAIL: Test case ${testCase.id} - ${error.message}`);
      }
    }

    const successRate = (successCount / testCases.length) * 100;
    console.log(`Schema Adherence: ${successRate}%`);

    // Architect Tier requires 100%, not 98%
    expect(successRate).toBe(100);
  });
});
```
...
### 2. Healing Efficiency (The "Recovery" Rate)

**Metric**: Successful recovery from schema violations within **maximum 2 retry cycles**.

**Verification Method**:
- Monitor the `retryCount` variable in healing loop
- **If system requires a 3rd retry = FAIL**
- Student must refine the "Heal Prompt" to provide better error-trace context

**Success Criteria**:
```typescript
✅ PASS: 95%+ of failures heal within 2 retries
❌ FAIL: 90% of failures heal within 2 retries (requires 3rd retry too often)
```

**Why This Matters**: Each retry costs tokens and latency. A well-designed heal prompt should fix errors immediately. If you need 3+ retries, your error feedback is too vague.

**Implementation Example**:
```typescript
describe('Healing Efficiency - Recovery Rate', () => {
  it('heals within 2 retry cycles for 95%+ of failures', async () => {
    const failureCases = CORRUPTED_INPUTS;
    const healingAttempts: number[] = [];

    for (const testCase of failureCases) {
      let retryCount = 0;

      // Mock the healing function to track attempts
      const result = await getHardenedSummary(testCase.input, retryCount);

      // Track how many retries were needed
      healingAttempts.push(retryCount);
    }

    // Calculate percentage healed within 2 retries
    const within2Retries = healingAttempts.filter(count => count <= 2).length;
    const efficiencyRate = (within2Retries / healingAttempts.length) * 100;

    console.log(`Healing Efficiency: ${efficiencyRate}%`);
    console.log(`Average retries: ${average(healingAttempts)}`);

    expect(efficiencyRate).toBeGreaterThanOrEqual(95);
    expect(average(healingAttempts)).toBeLessThan(1.5);  // Most heal on first retry
  });
});
```

**Architect's Tip**: If your average retry count is &gt;1.5, your heal prompt is too generic. Show the model **exactly** what field failed and why:

```typescript
// ❌ VAGUE: "Your JSON was invalid, please fix it"
const badHealPrompt = "Your previous response was invalid. Try again.";

// ✅ SPECIFIC: "Field 'patient_id' expected UUID format, got 'abc-123'"
const goodHealPrompt = `
ERROR: Field "patient_id" expected UUID format (e.g., "123e4567-e89b-12d3-a456-426614174000")
but received: "${parsedValue.patient_id}"

Please regenerate the JSON with a valid UUID for patient_id.
`;
```

---

### 3. Deterministic Steering (The "Temperature" Check)

**Metric**: Consistent output structure across **5 identical requests** (Variance &lt; 2%).

**Verification Method**:
- Send the same input 5 times
- Confirm `temperature: 0` is hardcoded
- Verify `response_format: { type: "json_object" }` is enabled at API level
- Measure structural variance (field order, key names, value types)

**Success Criteria**:
```typescript
✅ PASS: Variance &lt; 2% (effectively identical structures)
❌ FAIL: Variance > 5% (non-deterministic output)
```

**Why This Matters**: Production systems require **predictability**. If the same input produces different JSON structures, downstream systems break.

**Implementation Example**:
```typescript
describe('Deterministic Steering - Temperature Check', () => {
  it('produces &lt;2% variance across 5 identical requests', async () => {
    const testInput = "Patient John Doe reports severe headache";
    const results: ClinicalSummary[] = [];

    // Run same input 5 times
    for (let i = 0; i &lt; 5; i++) {
      const result = await getHardenedSummary(testInput);
      results.push(result);
    }

    // Calculate structural variance
    const variance = calculateStructuralVariance(results);

    console.log(`Structural Variance: ${variance.toFixed(2)}%`);
    console.table(results);

    expect(variance).toBeLessThan(2);

    // Verify temperature=0 is set
    expect(apiConfig.temperature).toBe(0);
  });
});

function calculateStructuralVariance(results: ClinicalSummary[]): number {
  // Compare triage_level consistency
  const triageLevels = results.map(r => r.triage_level);
  const uniqueLevels = new Set(triageLevels).size;

  // Compare symptom count consistency
  const symptomCounts = results.map(r => r.symptoms.length);
  const avgSymptoms = average(symptomCounts);
  const symptomVariance = symptomCounts.reduce((sum, count) =>
    sum + Math.abs(count - avgSymptoms), 0) / symptomCounts.length;

  // Calculate overall variance (should be near 0% for temp=0)
  const variance = ((uniqueLevels - 1) + symptomVariance) / 2 * 100;
  return variance;
}
```

**Architect's Rule**: If variance is &gt;2%, you're either:
1. Not using `temperature: 0`
2. Not using `response_format: { type: "json_object" }`
3. Relying on stochastic sampling instead of deterministic output

---

### 4. Token Overhead Management

**Metric**: "Heal Loop" token cost stays within **1.5x** of the original request cost.

**Verification Method**:
- Track `input_tokens` and `output_tokens` for each API call
- Calculate total cost across all retries
- **Architects must prove** they're sending only the relevant error snippet back to the model, not re-sending a 10,000-word document

**Success Criteria**:
```typescript
✅ PASS: Total healing cost ≤ 1.5x original request cost
❌ FAIL: Total healing cost &gt; 2.0x original request cost
```

**Why This Matters**: If healing costs **2x** the original request, you're doubling your API bill. At 100K requests/day with a 10% failure rate, inefficient healing costs an extra **$600/day** ($18K/month).

**Implementation Example**:
```typescript
describe('Token Overhead Management', () => {
  it('keeps healing cost within 1.5x of original request', async () => {
    const testCase = CORRUPTED_INPUTS[0];
    let totalInputTokens = 0;
    let totalOutputTokens = 0;
    let attemptCount = 0;

    // Mock API to track token usage
    const mockAPI = jest.spyOn(anthropic.messages, 'create')
    ...
    await getHardenedSummary(testCase.input);

    // Calculate cost (Sonnet pricing)
    const inputCost = (totalInputTokens / 1_000_000) * 3;   // $3/MTok
    const outputCost = (totalOutputTokens / 1_000_000) * 15; // $15/MTok
    const totalCost = inputCost + outputCost;

    // First attempt baseline cost
    const baselineCost = totalCost / attemptCount;
    const overhead = totalCost / baselineCost;
    ...
    expect(overhead).toBeLessThanOrEqual(1.5);

    mockAPI.mockRestore();
  });

  it('sends only error snippet, not full context on retry', async () => {
    const largeContext = "X".repeat(10000);  // 10K character document
    const testInput = `Context: ${largeContext}\n\nExtract data...`;

    // First attempt
    const firstAttemptTokens = estimateTokens(testInput);

    // Heal attempt should NOT include the full 10K context
    const healPrompt = `ERROR: Invalid JSON\nPlease fix and return corrected output.`;
    const healAttemptTokens = estimateTokens(healPrompt);

    console.log(`First attempt tokens: ${firstAttemptTokens}`);
    console.log(`Heal attempt tokens: ${healAttemptTokens}`);

    // Heal prompt should be &lt;20% of original size
    const ratio = healAttemptTokens / firstAttemptTokens;
    expect(ratio).toBeLessThan(0.2);
  });
});
```

**Architect's Tip**: Never re-send the entire original context on retry. Send only:
1. The error message
2. The invalid JSON (or snippet)
3. The schema reminder

```typescript
// ❌ WASTEFUL: Re-sending 10K context on every retry
const badHealPrompt = `
${ORIGINAL_10K_DOCUMENT}  // ⚠️ Wasting 10K tokens per retry!

Your previous JSON was invalid: ${error}
Please try again.
`;

// ✅ EFFICIENT: Send only error context
const goodHealPrompt = `
Your previous JSON was invalid.

ERROR: ${error.message}
INVALID OUTPUT: ${invalidJSON.substring(0, 200)}

Please return corrected JSON following the schema.
`;
```
...
**To achieve Architect Tier, you must pass ALL four benchmarks**:

| Benchmark | Metric | Success Criteria |
|-----------|--------|------------------|
| **Schema Adherence** | Zero-Crash Test | 100% valid JSON (50/50 cases) |
| **Healing Efficiency** | Recovery Rate | 95%+ heal within 2 retries |
| **Deterministic Steering** | Temperature Check | &lt;2% variance across 5 runs |
| **Token Overhead** | Cost Management | ≤1.5x original request cost |

**Your Verification Report**:

```bash
npm run verify:architect-tier

# Output:
🏁 Architect's Verification Report
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✅ Schema Adherence:       100% (50/50 cases)
✅ Healing Efficiency:     98% (within 2 retries)
✅ Deterministic Steering: 0.8% variance
✅ Token Overhead:         1.3x (within budget)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎓 Status: ARCHITECT TIER ACHIEVED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

This verification proves you can build AI systems that are:
✓ Financially sustainable (1.3x cost overhead vs 2-3x industry avg)
✓ Technically reliable (100% schema compliance vs 95% industry avg)
✓ Production-ready (0.8% variance vs 5% industry avg)

Add this report to your portfolio. Any CTO will understand what it means.
```

---

### 🚀 Integration: The "Director-Tier" Portfolio

On your landing page, you can now add this specific "Verification" highlight:

<Callout type="info" title="Stop Guessing. Start Verifying.">
In our labs, we don't just check your code; we run your systems through an **Architect's Stress Test**.

You'll be measured on:
- **Recovery Rates**: Can your system self-heal from failures?
- **Latency Overheads**: Does healing stay within cost budgets?
- **Deterministic Variance**: Is your output predictable in production?

You leave the course with a **verified performance report** that proves to any CTO you can build AI that is financially and technically sustainable.

**No participation trophies. Only quantified proof.**
</Callout>

This completes the technical loop for Week 1. With these benchmarks, your students aren't just "learning"—they are building a **quantified professional reputation**.
