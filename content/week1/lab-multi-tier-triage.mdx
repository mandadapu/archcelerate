...
async function testClassifier() {
  ...
  console.log(accuracy >= 95 ? '✅ PASSED' : '❌ FAILED - Needs tuning')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
}
...
**Expected Output**:
```
✅ Expected: simple, Got: simple (98.5% confidence)
   Query: "How do I reset my password?"
   Reasoning: Clear administrative request with no medical content
   Time: 421ms

✅ Expected: complex, Got: complex (94.2% confidence)
...
✅ Expected: critical, Got: critical (99.8% confidence)
...
export class TriageMonitor {
  ...
  getMetrics(): TriageMetrics {
    ...
    const estimatedMonthlyCost = costPerQuery * 1_500_000  // 50K/day × 30 days
  ...
  printReport(): void {
    ...
    console.log(`\nRouting Breakdown:`)
    console.log(`  🟢 Haiku:  ${((haiku.count/total) * 100).toFixed(1)}% (${haiku.count.toLocaleString()} queries, $${haiku.cost.toFixed(2)}, ${haiku.avgLatency.toFixed(0)}ms avg)`)
    console.log(`  🟡 Opus:   ${((opus.count/total) * 100).toFixed(1)}% (${opus.count.toLocaleString()} queries, $${opus.cost.toFixed(2)}, ${opus.avgLatency.toFixed(0)}ms avg)`)
    console.log(`  🔴 Human:  ${((human.count/total) * 100).toFixed(1)}% (${human.count.toLocaleString()} queries)`)
    console.log(`\nCost Analysis:`)
    ...
    console.log(`\nTarget: <$30,000/month`)
    console.log(metrics.estimatedMonthlyCost < 30000 ? '✅ ON TRACK' : '❌ OVER BUDGET')
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')
  }
}
```
...
async function runSimulation() {
  ...
  for (const query of TEST_QUERIES) {
    ...
    console.log(`  → Routed to: ${result.routedTo} ($${result.cost.toFixed(4)}, ${result.latency}ms)\n`)
...
Routing Breakdown:
  🟢 Haiku:  42.9% (6 queries, $0.05, 428ms avg)
  🟡 Opus:   42.9% (6 queries, $0.80, 4105ms avg)
  🔴 Human:  14.3% (2 queries)
...
async function optimizeThresholds() {
  console.log('🎯 Optimizing Confidence Thresholds\n')
  ...
  for (const threshold of THRESHOLDS_TO_TEST) {
    ...
    console.log(monthlyCost < 30000 && accuracy >= 95 ? '  ✅ MEETS TARGETS' : '  ❌ FAILS TARGETS')
  }
}
...
export const TRIAGE_CONFIG = {
  ...
  // Safety thresholds
  criticalEscalationThreshold: 0.70,  // &lt;70% confidence → human review
...
async function validateProduction() {
  ...
  console.log(metrics.estimatedMonthlyCost < TRIAGE_CONFIG.targets.monthlyCostLimit ? '  ✅ PASSED' : '  ❌ FAILED')

  console.log(`\nLatency Target: <${TRIAGE_CONFIG.targets.p95Latency}ms p95`)
  console.log(`  Actual: ${metrics.avgLatency.toFixed(0)}ms avg`)
  console.log(metrics.avgLatency < TRIAGE_CONFIG.targets.p95Latency ? '  ✅ PASSED' : '  ❌ FAILED')
...
**Architecture Wins**:
- **Cost Reduction**: 80% savings ($225K → $30K/month) through intelligent routing
- **Latency Improvement**: 57% faster (4.2s → 1.8s p95) by using Haiku for simple queries
...
- ✅ Confidence threshold optimized for cost/safety balance
- ✅ Human escalation for uncertain or critical queries
- ✅ Cost tracking per query with anomaly alerts
- ✅ Latency monitoring with p95 targets
- ✅ Medical safety validation (no dosage recommendations, no definitive diagnoses)
