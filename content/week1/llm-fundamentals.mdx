...
**The Problem**: A global telehealth platform receives 50,000 patient queries daily. Using Opus 4.5 for every request (~1K input + 500 output tokens per query, 1.5M queries/month) costs **$78,750/month**—financially unsustainable. But downgrading all queries to Haiku risks missing critical medical symptoms that require deep reasoning.
...
function classifyQuery(query: string): Classification {
  ...
  // Critical keywords → always escalate
...
async function routeQuery(query: string, classification: Classification): Promise<RouteResult> {
  ...
  // COMPLEX → high-quality model with fallback
...
async function callModel(query: string, tier: 'haiku' | 'sonnet' | 'opus'): Promise<RouteResult> {
  ...
  // If Haiku expresses uncertainty → escalate to Sonnet
  if (tier === 'haiku' && Math.random() < 0.05) { // 5% uncertainty rate
    console.log('  🔄 Haiku expressed uncertainty → escalating to Sonnet')
    return callModel(query, 'sonnet')
  }
...
async function demo() {
  const testQueries = [
    'When is my next appointment?',                         // → Haiku (simple)
    'What are the side effects of metformin with lisinopril?', // → Opus (complex)
    'I am having chest pain and shortness of breath',       // → HUMAN (critical)
    'Tell me something about health',                       // → Sonnet (low confidence)
  ]

  console.log('🏥 Medical Query Router Demo')
  ...
  console.log('\n' + '─'.repeat(60))
  console.log(`💰 Total cost for 4 queries: $${totalCost.toFixed(4)}`)
...
**Try it yourself** — copy the code above into a file and run `npx ts-node medical-router-demo.ts`. You'll see each query classified, routed to the appropriate model tier, and the cost savings calculated.

**Production Impact**:
- **Cost**: $78.7K → ~$5.4K/month (93% reduction)
- **Latency**: 4.2s → 1.8s P95 (57% faster) by routing 85% to Haiku
...
function sanitizeInFlight(query: string): SanitizedQuery {
  ...
  // SSN, names, addresses → [PATIENT_ID_A], [PATIENT_ID_B], etc.
  const piiPatterns = {
...
async function processWithHaikuAndGuardrail(query: string): Promise&lt;MedicalResponse&gt; {
  ...
  // 3. If flagged → discard Haiku output, re-route to Opus
...
> Monthly spend: **$78,750 → ~$5,500** (93% reduction) — well under the $15,000 target.

---

### The Sovereign Audit Log: Immutable Accountability

In a medical context, an auditor doesn't just ask *what* the AI said — they ask **why the system decided to use a cheaper model** for a specific patient. If you cannot prove the reasoning was sound, the cost-saving measure becomes a legal liability.

#### Architectural Pattern: Cryptographic Hash Chaining

We implement a Merkle Tree-inspired structure where every governance event (a routing decision, a PII block, or a safety veto) is linked to the previous one.
...
interface AuditEntry {
  timestamp: string
  tenantId: string
  queryHash: string           // SHA-256 of the original query
  ...
  previousHash: string        // Hash link to prior entry
  currentHash: string         // SHA-256(previousHash + currentEventData)
}

function createAuditEntry(
  event: Omit<AuditEntry, 'currentHash'>
): AuditEntry {
  const eventData = JSON.stringify({
    timestamp: event.timestamp,
    tenantId: event.tenantId,
    queryHash: event.queryHash,
    routerConfidence: event.routerConfidence,
    modelSelected: event.modelSelected,
    policyId: event.policyId,
    piiEntitiesRedacted: event.piiEntitiesRedacted,
    safetyCheckPassed: event.safetyCheckPassed,
    previousHash: event.previousHash,
  })

  // Cryptographic Seal: SHA-256(previousHash + currentEventData)
  const currentHash = createHash('sha256')
    .update(event.previousHash + eventData)
    .digest('hex')

  return { ...event, currentHash }
}

function verifyChainIntegrity(entries: AuditEntry[]): boolean {
  for (let i = 1; i < entries.length; i++) {
    const recomputed = createAuditEntry({
      ...entries[i],
      previousHash: entries[i - 1].currentHash,
    })

    if (recomputed.currentHash !== entries[i].currentHash) {
      console.error(`Chain broken at entry ${i}: tampering detected`)
      return false  // A single altered byte breaks the entire chain
    }
  }
  return true  // 100% Chain Integrity
}
```

#### The HIPAA-Compliant Decision Record

When the Model Cascade routes a query to a smaller model, the Sovereign Audit Log records a **Sovereign Manifest**:

```typescript
// Example: Haiku handles a low-risk administrative query
const manifest: AuditEntry = createAuditEntry({
  timestamp: new Date().toISOString(),
  tenantId: 'clinic-west-001',
  queryHash: createHash('sha256').update(sanitizedQuery).digest('hex'),
  routerConfidence: 0.98,
  modelSelected: 'claude-haiku-4-5-20251001',
  policyId: 'ROUTING-v2.3-low-risk-admin',
  piiEntitiesRedacted: 0,           // PII-Scrubber active; 0 entities leaked
  safetyCheckPassed: true,           // Safety check passed
  safetyCheckLatencyMs: 45,
  previousHash: lastAuditEntry.currentHash,
})

// Proof chain for auditors:
// 1. Routing Logic: "Confidence 0.98; no high-risk keywords"
// 2. Safety Signature: "Safety check passed (45ms)"
// 3. Data Integrity: "0 PII entities leaked to third-party API"
// 4. Cryptographic Seal: SHA-256 hash linking to previous entry
```

#### From Cost-Center to Trust-Center

By hardening the audit layer, you transform AI infrastructure from a risky experiment into a **Sovereign Asset**:

| Enterprise Risk | Architectural Solution | Audit Proof |
|----------------|----------------------|-------------|
| Data Leakage | Pre-Inference PII Gateway | Log of 0 leaked entities |
| Medical Error | Confidence-Based Escalation | Record of high-confidence routing |
| Systemic Failure | Multi-Provider Failover | Uptime report across providers |
| Audit Failure | Hash-Chained Audit Logs | 100% Chain Integrity Report |

> **The Sovereign Architect's Mandate:** Your telehealth platform now routes 93% cheaper ($78,750 → ~$5,500), with clinically safe confidence-based escalation, and every decision cryptographically signed for HIPAA/GDPR/SOC2 auditors. This is the difference between building a chatbot and building a **Sovereign Intelligence System**.
...
### ⚠️ Warning: Tokenization Edge Cases
...
// But with spaces...
"A p p l e" → 5 tokens (5x the cost!)
...
"1234" → 1 token
"12345" → 1 token
"123456" → 2 tokens  // ⚠️ Boundary crossed
"1234567" → 2 tokens

// Scientific notation
"1.5e10" → 4 tokens ("1", ".", "5e", "10")

// Architectural impact: Dollar amounts
"$1,234.56" → 5 tokens (",", "1", ",", "234", ".")
"$1234.56" → 3 tokens  // 40% fewer tokens without commas
```
...
// Production impact:
// Sending 1000 code snippets/day × 5 extra tokens
// = 5K tokens/day × $3/MTok = $0.015/day = $0.45/month
...
"COPD" → 2 tokens ("CO", "PD")  // Chronic Obstructive Pulmonary Disease
"COVID" → 2 tokens ("COV", "ID")
"MRI scan" → 3 tokens ("M", "RI", "scan")

// But...
"CT scan" → 2 tokens ("CT", "scan")  // More common, single token

// Why this matters for reasoning:
// LLMs trained on "COVID" as 1 semantic unit
// But tokenizer splits it → model must "reassemble" meaning
...
async function analyzeTokenization(terms: string[]) {
  ...
  for (const term of terms) {
    ...
    console.log(`"${term}" → ${result.input_tokens} tokens`)

    // Flag if fragmentation is high
    const chars = term.length
    const tokensPerChar = result.input_tokens / chars
    if (tokensPerChar > 0.5) {
      console.warn(`  ⚠️ High fragmentation: ${(tokensPerChar * 100).toFixed(0)}% of chars are tokens`)
    }
  }
}
...
## 🏗️ Prompt Caching: The Architect's First Line of Defense
...
```typescript
// ❌ Without caching: Full cost every time
...
// Cost: 1000 calls × 5000 tokens × $3/MTok = $15
```
...
// 1000 calls: $0.015 + (999 × $0.0015) = $1.50 (vs $15)
```
...
```typescript
// Cache structure: System → Tools → Few-shot → Dynamic context
const prompt = {
  system: [
    {
      type: 'text',
      text: SYSTEM_INSTRUCTIONS,  // 2K tokens - ✅ Cacheable (static)
      cache_control: { type: 'ephemeral' }
    },
    {
      type: 'text',
      text: TOOL_DEFINITIONS,     // 3K tokens - ✅ Cacheable (static)
      cache_control: { type: 'ephemeral' }
    },
    {
      type: 'text',
      text: FEW_SHOT_EXAMPLES,    // 10K tokens - ✅ Cacheable (mostly static)
      cache_control: { type: 'ephemeral' }
    }
  ],
  messages: [
    // User history (dynamic) - ❌ Not cached
    ...conversationHistory,
...
**Without caching**:
- Cost: 100K × 28K × $3/MTok = **$8,400/day** = **$252K/month**

**With caching** (90% cache hit rate):
- First call: 100K × 28K × $3/MTok = $8,400
- Cached calls: 100K × 28K × $0.30/MTok × 0.90 = $756
...
- **Example**: 2000ms → 500ms for cached system prompts

**Production Impact**:
```typescript
// Without cache: 2.5s P95 latency
// With cache: 0.8s P95 latency → 68% faster
// User experience: Feels "instant" vs "thinking"
```

### Implementation Checklist

✅ **Before deploying prompt caching**:
...
## 🎯 Context Truncation Strategies: When You Hit the Limit
...
|----------|-------------------------|----------|----------------------|
| **FIFO (First-In, First-Out)** | Drop the oldest messages in the array as new ones arrive | Short, transactional chatbots | ⚠️ **High Risk**: The model loses the initial "System Instructions" or the original user goal. **Never use this pattern for safety-critical systems.** |
| **Middle-Out Truncation** | Keep the System Prompt and the most recent 10 messages; delete the middle history | Long, multi-turn technical support | ✅ **Optimal**: Preserves the "Mission" (system prompt) and the "Current State" (recent messages) while clearing the bulk. **Production default for most applications.** |
| **Rolling Summarization** | Use a cheaper model (Haiku) to summarize the first 50 messages into a 200-token "State Summary" | Complex, multi-day research agents | ⚠️ **High Latency**: Adds a summarization step (300-800ms), but maintains 100% of the semantic history. **Best for high-value sessions.** |
| **Vector-Backed Memory** | Move "inactive" context to a Vector DB and retrieve only what is relevant to the current query | Enterprise Knowledge Bases, AI agents with long-term memory | ⚠️ **High Complexity**: Requires a RAG infrastructure but offers effectively "infinite" memory. **Only viable at scale (&gt;10K sessions/month).** |

**Cost-Quality-Latency Matrix**:
```
FIFO              → Free, 0ms, ❌ Dangerous (loses system prompt)
Middle-Out        → Free, 0ms, ✅ Good (preserves mission + current state)
Summarization     → $0.001-0.01, 300-800ms, ✅ Excellent (semantic preservation)
Vector-Backed     → $0.01-0.05, 50-200ms, ✅ Best (infinite memory, high complexity)
```

---

### Strategy #1: FIFO (First-In, First-Out) — ❌ NOT RECOMMENDED

**How it works**: Drop oldest messages as new ones arrive.

```typescript
// ❌ DANGEROUS: This pattern can delete system instructions
function fifoTruncation(
  ...
  while (currentTokens > maxTokens) {
    messages.shift()  // ⚠️ Can delete system prompt!
...
### Strategy #2: Middle-Out Truncation (Production Default) — ✅ RECOMMENDED
...
- Can break multi-turn reasoning ("As I mentioned earlier..." → model has no memory)
...
// Cost analysis
// Old context: 180K tokens × $3/MTok = $0.54 per request (dropped)
...
// Embedding cost: 2M messages × 1K tokens × $0.0001/MTok = $0.20/month
// Storage cost: 2M vectors × $0.50/month = $1,000/month (Pinecone Pro)
// Query cost: 10K queries/day × $0.001/query = $300/month
// Total: $1,300/month for effectively infinite memory
// vs. Summarization: $0.01 × 2M sessions = $20,000/month
```
...
### ⚠️ The "Instruction Anchor" — Critical Safety Pattern
...
```typescript
// ❌ DANGEROUS: System prompt can be truncated
const messages = [
...
messages.shift()  // ⚠️ System prompt DELETED
// Model no longer knows it must follow HIPAA rules!
```

**✅ Safe Implementation: Immutable Index 0**
...
function productionTruncate(state: ConversationState): Message[] {
  ...
  console.log(`History length: ${state.history.length} → ${TARGET_LENGTH}`)
...
// ❌ WRONG: If system prompt is truncated...
// Model: "Here are some home remedies for chest pain" → DANGEROUS

// ✅ CORRECT: System prompt is anchored at Index 0
// Model sees: "ESCALATE chest pain" → "This requires immediate medical attention. I'm alerting your care team."
```
...
|----------|------|---------|---------|----------------|---------------|
| **FIFO** | Free | 0ms | ❌ Dangerous | None | **Never use** |
| **Middle-Out** | Free | 0ms | ✅ Good | None | Simple Q&A, stateless tasks |
| **Rolling Summarization** | $0.001-0.01 | 300-800ms | ✅ Excellent | None | Long sessions, agents, support |
| **Vector-Backed** | $0.01-0.05 | 50-200ms | ✅ Best | Vector DB required | Enterprise, infinite memory |
...
async function hybridTruncation(
  ...
  if (messageCount % SUMMARIZATION_INTERVAL === 0) {
    console.log('📝 Triggering periodic summarization')
    return await summarizeOldContext(messages, maxTokens)
  }
...
// Cost impact:
// Without: 50 messages × 180K tokens × $3/MTok = $27
// With hybrid: 49 × 5K tokens + 1 × summarization = $0.735 + $0.56 = $1.30
// Savings: 95% ($25.70 per 50-message session)
```

**Architect's Rule**: Truncation strategy should match conversation value:
- **High-value** (sales, support) → LLM summarization
- **Mid-value** (general chat) → Hybrid (drop-middle + periodic summarization)
- **Low-value** (one-off queries) → Drop-middle or no truncation needed
...
## 🎓 Test Your Architectural Thinking

> These aren't beginner questions—they're **real-world architectural scenarios** you'll face in production. Think through the tradeoffs before checking the solutions.
...
**Option A (Long-Context)**: ❌ Over budget
- 100 queries × 5M tokens × $15/MTok = $7,500/month
- **75K tokens output** (summaries) × $75/MTok = $5,625
- **Total: $13,125/month** (26x over budget!)

**Option B (MapReduce)**: ❌ Complex, but still expensive
- Split: 5M tokens / 100 pages = 50K tokens per chunk = 100 chunks
- Process each: 100 chunks × 50K tokens × $0.25/MTok × 100 queries = $1,250/month
- Merge step (Sonnet): 100 intermediate results × 10K tokens × $3/MTok × 100 queries = $3,000/month
- **Total: $4,250/month** (8.5x over budget)

**Option C (Hybrid RAG)**: ✅ Within budget
- Vector search: $50/month (embedding cost: 500M tokens × $0.0001/MTok)
- Retrieve top 50 pages: 50 × 500 tokens = 25K tokens per query
- Sonnet processing: 100 queries × 25K tokens × $3/MTok = $75/month (input)
- Output: 100 queries × 5K tokens × $15/MTok = $75/month
- **Total: $200/month** (40% of budget, leaves room for growth)

**Option D (Incremental)**: ⚠️ Quality risk
- Haiku on 5M tokens: 100 × 5M × $0.25/MTok = $125/month
- But: Haiku quality insufficient for legal citations (50% hallucination rate on long context)
- Escalation rate: 80% → Sonnet cost: $4,000/month
- **Not viable due to quality constraints**

**Architectural Insights**:
- RAG reduces context from 5M → 25K tokens (99.5% reduction!)
...
**Latency (P95 &lt; 500ms)**:
- Opus: 4-8s P95 → ❌ Fails (10x too slow)
- Sonnet: 1-3s P95 → ❌ Fails (3x too slow)
- Haiku: 300-800ms P95 → ✅ Meets requirement
- Self-hosted Llama: 50-200ms → ✅ Meets requirement

**Quality (95%+ accuracy)**:
- Opus: 99% → ✅ Overkill
- Sonnet: 97% → ✅ Meets requirement
- Haiku: 92% on simple, 88% on nuanced → ⚠️ Borderline
- Llama 7B fine-tuned: 93% → ✅ Possible with good training data

**Cost (&lt;$0.0001/post)**:
- Opus: $15/MTok × 200 tokens = $0.003/post → ❌ 30x over budget
- Sonnet: $3/MTok × 200 tokens = $0.0006/post → ❌ 6x over budget
- Haiku: $0.25/MTok × 200 tokens = $0.00005/post → ✅ Meets budget
- Llama self-hosted: $0.00001/post (amortized GPU) → ✅ Meets budget
...
async function moderateContent(post: string) {
  ...
  // → Returns: { toxicity: 'high' | 'medium' | 'low', confidence: number }
...
// 0.90 × $0.00005 + 0.10 × $0.0006 = $0.000105/post
// → Slightly over target, but acceptable tradeoff for quality
...
- **Total cost**: $4000+/month → only viable at &gt;40M posts/month
...
**Option D (Agentic Loop)**: ✅ Best (highest quality, acceptable cost)
```typescript
...
// Cost breakdown:
// Iteration 1: 50K tokens (RAG) × $15/MTok = $0.75
// Iteration 2: 50K + 30K new files × $15/MTok = $1.20
// Iteration 3: 80K + 15K more files × $15/MTok = $1.43
...
1. **RAG alone is insufficient** for complex codebases—LLMs need ability to "explore"
...
4. **Tool use** is the key primitive—don't just send context, let LLM request it

**Real-World Tradeoff**:
- **Option C**: $1.05, 85% success → $1.24/successful refactor (15% require human fix)
- **Option D**: $3.38, 98% success → $3.45/successful refactor
- **Value**: Option D costs 3x more but saves 2 hours of debugging → $3.38 vs $200 (engineer time)
...
### Scenario 4: The Medical Safety Truncation Dilemma 🏥
...
<details>
<summary><strong>💡 Solution & Safety Analysis</strong></summary>

**Correct Answer: B) Middle-Out Truncation**

**Why Each Option Succeeds or Fails**:

**Option A (FIFO)**: ❌ **CATASTROPHICALLY DANGEROUS**
```typescript
...
messages.shift()  // ⚠️ DELETES SAFETY GUIDELINES
messages = [
...
**Option B (Middle-Out)**: ✅ **CORRECT - Safety Preserved**
```typescript
...
**Option C (Clear Cache/Fresh Session)**: ⚠️ **Wasteful but Safe**
```typescript
...
**Option D (Increase Temperature)**: ❌ **Completely Wrong**
```typescript
...
**Why this fails**:
- **Temperature doesn't restore lost context**—it only controls randomness of token selection
