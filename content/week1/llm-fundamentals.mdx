...
### Strategy #1: Drop the Middle (Fastest, Simplest)
...
### Strategy #2: Drop the Beginning (Rare, Risky)

**How it works**: Keep system prompt + recent messages, discard oldest.

```typescript
function dropBeginningStrategy(
  messages: Message[],
  maxTokens: number
): Message[] {
  ...
  while (currentTokens > maxTokens && messages.length > 10) {
...
**Pros**:
- Maintains full recent context
- Natural for "sliding window" applications

**Cons**:
- **Loses important setup**: Original user intent, key details
- Breaks long-running sessions (loses user preferences set early)
- System prompt grows stale if not refreshed

**Best For**: Rarely used—only when initial context is truly disposable

---

### Strategy #3: LLM-Based Summarization (Highest Quality, Costly)
...
| Strategy | Cost | Latency | Quality | Best Use Case |
|----------|------|---------|---------|---------------|
| **Drop Middle** | Free | 0ms | ⚠️ Moderate | Simple Q&A, stateless tasks |
| **Drop Beginning** | Free | 0ms | ❌ Poor | Rarely appropriate |
| **LLM Summarization** | $0.001-0.01 | 300-800ms | ✅ High | Long sessions, agents, support |
