---
title: "Architectural ROI & Model Selection"
description: "Unit economics, model selection matrix, and quantified decision-making"
estimatedMinutes: 45
---
...
export class ComplexityAwareRouter {
  ...
  async route(prompt: string): Promise<{
    ...
    console.log(`✅ Routed to ${complexity.recommendedModel} (score: ${complexity.score}, cost: $${actualCost.toFixed(4)}, latency: ${latency}ms)`)
...
> "The router should learn from production traffic. If Haiku consistently handles score=45 queries well, raise your Haiku threshold from 40→50. This incremental tuning compounds into massive savings."

#### Production Results: Real-World Impact

**Scenario**: Customer support chatbot processing 100K requests/day

**Before Dynamic Router** (all requests → Sonnet):
- Cost: 100K × $0.009 = $900/day = **$27,000/month**
...
// Cost calculation
Daily cost:
  Haiku:  78,200 × $0.0006 = $46.92
  Sonnet: 18,500 × $0.009  = $166.50
  Opus:   3,300  × $0.045  = $148.50
...
function calculateVPT(
  ...
  const conversionLift = qualityDelta * 0.5  // Conservative: 6% quality → 3% conversion lift
...
- Cost: $0.009/request
- Monthly cost: 50K × 30 × $0.009 = **$13,500**
...
- Cost: $0.045/request
- Monthly cost: 50K × 30 × $0.045 = **$67,500**
...
- Revenue increase: 50K × 30 × 0.03 × 0.025 × $75 = $16,875/month
...
- Monthly cost: 5K × $0.045 = **$225**
- Misdiagnosis reduction: 12% × 5,000 = 600 fewer errors/month
...
const medicalVPT = {
  ...
  costSavings: 600 × $15,000 = $9,000,000/month (avoided wasted treatments),
...
export class VPTOptimizedRouter {
  ...
  analyzeFeatureVPT(
    ...
    pairs.forEach(([baseline, upgraded]) => {
      ...
      console.log(`\n${baseline} → ${upgraded}:`)
      console.log(`  Quality Δ: +${analysis.qualityDelta}%`)
      console.log(`  Cost Δ: $${analysis.costDelta.toLocaleString()}/month`)
      console.log(`  Revenue Δ: $${analysis.revenueImpact.toLocaleString()}/month`)
...
> "Don't guess quality improvements. Run A/B tests with real traffic. 'Opus feels better' is not a VPT input. 'Opus increased CTR from 4.2% → 4.9%' is."
...
export class FinancialCircuitBreaker {
  ...
  async checkBudget(userId: string, estimatedCost: number = 0.01): Promise<BudgetCheckResult> {
    ...
    if (budget.currentHourlySpend > config.hourlySoftWarn) {
      warning = `⚠️  Approaching hourly limit: $${budget.currentHourlySpend.toFixed(2)}/$${config.hourlyHardCap}`
    }
  ...
  private async tripCircuit(userId: string, reason: string, limit: number) {
    ...
    console.error(`🚨 CIRCUIT BREAKER TRIPPED: User ${userId} - ${reason} (limit: $${limit})`)
  }
  ...
  private async resetCircuit(userId: string) {
    ...
    console.log(`✅ Circuit breaker RESET: User ${userId}`)
  }
...
> "If a user's session cost exceeds $2.00 in an hour, the Safety Proxy should trip a 'Financial Circuit Breaker' to prevent infinite loops or automated 'wallet-draining' attacks. This is not optional—it's architectural self-defense."
...
**The Challenge**: A global telehealth platform receives **50,000 patient queries per day**. Some are simple ("How do I reset my password?"), others are complex medical symptom questions requiring careful analysis. Using Opus 4.5 for all queries costs **$225K/month**—making the product unprofitable.
...
class TriageMonitor {
  ...
  async processQueryWithTracking(query: string): Promise<TriageResult> {
    ...
    // Alert on anomalies
    if (result.cost &gt; 0.50) {
      console.warn(`⚠️  High-cost query: $${result.cost.toFixed(4)} - "${query.substring(0, 50)}..."`)
    }

    if (result.latency &gt; 5000) {
      console.warn(`⚠️  Slow query: ${result.latency}ms - "${query.substring(0, 50)}..."`)
    }
  ...
  getDailyReport(): string {
    ...
    const report = [
      ...
      '',
      'Routing Breakdown:',
      `  🟢 Haiku:  ${((haiku/total) * 100).toFixed(1)}% (${haiku.toLocaleString()} queries)`,
      `  🟡 Opus:   ${((opus/total) * 100).toFixed(1)}% (${opus.toLocaleString()} queries)`,
      `  🔴 Human:  ${((human/total) * 100).toFixed(1)}% (${human.toLocaleString()} queries)`,
      '',
...
**Before Triage System** (all queries → Opus 4.5):
- **Cost**: $78,750/month (1.5M queries × ~1K input + 500 output tokens at $15/$75 per MTok)
- **Latency**: 4.2s p95
- **Quality**: 98% accuracy

**After Triage System** (intelligent routing):
- **Cost**: $15,000/month (81% reduction)
  - 80% simple queries → Haiku (~$0.001/query)
  - 15% complex queries → Opus (~$0.10/query)
  - 5% critical queries → Human review ($0/direct API cost)
...
// Simple queries (80% = 1,200,000) → Haiku
// ~1K input ($0.25/MTok) + 500 output ($1.25/MTok) = $0.000875/query
const simpleCost = 1_200_000 * 0.001  // ~$1,200/month

// Complex queries (15% = 225,000) → Opus
// ~2K input ($15/MTok) + 1K output ($75/MTok) = $0.105/query
const complexCost = 225_000 * 0.105   // ~$23,625/month

// Critical queries (5% = 75,000) - human review
const criticalCost = 0  // Separate budget

// Total API Cost (before optimization)
const totalCost = 1_200 + 23_625      // ~$24,825/month
```

**Actual Results** (after optimization):
- Implemented caching for FAQs (30% of simple queries): **$360/month savings**
- Fine-tuned Haiku confidence threshold (0.9 → 0.85): **10% more queries to Haiku**
- **Final Cost**: **~$15,000/month** (81% reduction from original)

**Business Impact**:
- **Gross Margin**: Increased from 30% → 78%
...
// Implementation cost
const engineeringCost = 40 hours × $200/hr = $8,000
...
**Current Metrics**:
- Model: Claude Sonnet 4.5
- Monthly cost: $2,500 (10K users × 25 summaries/month × $0.01/summary)
- Summary quality: 90% user satisfaction
- Free → Paid conversion: 1% (100 conversions/month)
- Paid tier revenue: $20/month × 100 users = $2,000/month
...
- Estimated conversion lift: +20% relative (1.0% → 1.2%)
- Additional revenue: 20 extra conversions × $20 = $400/month
...
### The Correct Answer: **C** — Run an A/B test
...
// Highly unlikely that 8% quality drop → 100%+ conversion drop
```
...
interface ABTestConfig {
  ...
  successMetric: string  // Free → Paid conversion rate
  costThreshold: number  // If Haiku conversion &lt; 0.5%, abort
}

// Expected outcomes:
// Scenario 1 (80% probability): Haiku conversion = 0.95%-1.05%
//   → Quality drop doesn't impact conversion
//   → Switch to Haiku: Save $2,250/month
//   → ROI: $27K/year savings

// Scenario 2 (15% probability): Haiku conversion = 0.70%-0.94%
//   → Quality drop causes 6-30% conversion loss
//   → Calculate breakeven: Is cost savings > revenue loss?
//   → If yes: Switch. If no: Stay on Sonnet or optimize further.

// Scenario 3 (5% probability): Haiku conversion &lt; 0.70%
//   → Quality drop causes &gt;30% conversion loss
//   → Stay on Sonnet, investigate other optimizations (caching, tier system)
```
...
**Why D is wrong**: Opus costs 5x more ($12,500) for a 6% quality improvement. Even if Opus increases conversions by 20% (1.0% → 1.2%), that's only +$400/month revenue vs. +$10,000 cost. **VPT is massively negative.**
...
3. **Scales learning**: The A/B test data informs future decisions (e.g., if Haiku drops conversion by 10%, you know the quality → conversion elasticity for this feature).

4. **Production discipline**: Directors don't make $27K/year decisions ($2,250 × 12 months) based on gut feel. They run experiments.
...
- ❌ "Haiku is cheaper, let's use it"
- ❌ "Sonnet has better quality, worth the cost"
- ❌ "Opus will increase conversions"

You say:
- ✅ "I ran a 30-day A/B test with 10K users. Haiku reduced cost by 90% ($2,250/month) with statistically insignificant conversion impact (&lt;2% drop). **VPT is +$2.29/token**. Recommendation: Switch to Haiku for this feature, monitor conversion for 90 days."
...
### Scenario: Digital Health - Multi-Tier Triage System

**Business Context**

A global telehealth platform receives 50,000 patient queries daily ranging from simple prescription refills to complex symptom analysis requiring specialist consultation. The platform currently routes all queries to Claude Sonnet 4.5 at an average cost of $0.15 per query, resulting in $7,500/day ($225K/month) in AI costs.

The business problem: **80% cost reduction target while maintaining 100% accuracy on safety-critical queries** (e.g., chest pain, suicidal ideation). A single misclassified high-risk case could result in patient harm and regulatory penalties. However, the majority of queries are routine ("Can I take ibuprofen with my medication?") and don't require frontier model reasoning.

The architectural challenge: Build a **multi-tier triage system** that routes simple queries to Haiku 4.5 ($0.01/query), escalates complex medical reasoning to Opus 4.5 ($0.30/query), and maintains a human review fallback for uncertainty. The system must achieve cost savings without compromising patient safety.

**Success Criteria**

- **Cost Target**: Reduce average cost per query from $0.15 to &lt;$0.03 (80% reduction = $180K/month savings)
- **Safety Requirement**: 100% accuracy on high-risk symptom detection (zero false negatives for chest pain, severe bleeding, suicidal ideation)
- **Quality Threshold**: ≥95% patient satisfaction score on query responses
- **Latency Target**: p95 response time &lt;3s (Haiku queries &lt;1s, Opus queries &lt;5s)
- **Compliance**: HIPAA-compliant audit logging for all escalations and human reviews

**Architecture Decision: Why Multi-Tier Routing Solves This**

The single-model approach (Sonnet for everything) treats all queries as equal complexity, leading to systematic overspending. The solution is a **cascade architecture** that applies cost-first model selection:

1. **Haiku Classifier** (95% of queries, $0.01 each): Handle routine queries that don't require complex reasoning
2. **Opus Escalation** (4% of queries, $0.30 each): Complex medical symptoms requiring differential diagnosis
3. **Human Review** (1% of queries): High-risk cases or low-confidence Opus responses

This architecture exploits the **Pareto distribution of query complexity**: most queries are simple, few require expensive models. By routing intelligently, we achieve 80% cost savings while upgrading quality for the 4% of queries that need it.
...
// Configuration: Claude 4.5 series pricing (Feb 2026)
const MODEL_CONFIG = {
  haiku: {
    model: 'claude-haiku-4.5-20250201',
    costPerInputToken: 0.00000025,  // $0.25 per MTok
    costPerOutputToken: 0.00000125, // $1.25 per MTok
    avgTokensPerQuery: 2000,        // 1500 input + 500 output
    avgCost: 0.01
  },
  sonnet: {
    model: 'claude-sonnet-4.5-20250201',
    costPerInputToken: 0.000003,    // $3 per MTok
    costPerOutputToken: 0.000015,   // $15 per MTok
    avgTokensPerQuery: 2500,
    avgCost: 0.15
  },
  opus: {
    model: 'claude-opus-4.5-20251101',
    costPerInputToken: 0.000015,    // $15 per MTok
    costPerOutputToken: 0.000075,   // $75 per MTok
    avgTokensPerQuery: 3000,
    avgCost: 0.30
  }
}

// Safety-critical symptoms requiring immediate escalation
const HIGH_RISK_KEYWORDS = [
  'chest pain', 'shortness of breath', 'suicidal', 'self-harm',
  'severe bleeding', 'stroke symptoms', 'seizure', 'unconscious',
  'severe allergic reaction', 'anaphylaxis', 'overdose'
]

class HealthcareTriage {
  private client: Anthropic
  private metricsStore: QueryMetrics[] = []
   ...
   * Routes query through multi-tier system
   */
  async triageQuery(userQuery: string, userId: string): Promise<TriageResult> {
    const startTime = Date.now()

    // Safety check: Immediate escalation for high-risk keywords
    const safetyFlags = this.detectSafetyFlags(userQuery)
    if (safetyFlags.length > 0) {
      console.log(`⚠️  Safety flags detected: ${safetyFlags.join(', ')}. Escalating to human review.`)
      return {
        classification: 'critical',
        confidence: 1.0,
        routedTo: 'human',
        cost: 0,
        latency: Date.now() - startTime,
        escalationReason: 'Safety-critical keywords detected',
        safetyFlags
      }
    }

    // Step 1: Haiku classification
    const haikuResult = await this.classifyWithHaiku(userQuery)

    // Step 2: Route based on Haiku confidence
    if (haikuResult.classification === 'simple' && haikuResult.confidence >= 0.85) {
      // High confidence simple query → Haiku handles it
      const response = await this.generateHaikuResponse(userQuery)
      const latency = Date.now() - startTime

      this.logMetrics({
        timestamp: new Date(),
        userId,
        queryType: haikuResult.classification,
        modelUsed: 'haiku',
        cost: MODEL_CONFIG.haiku.avgCost,
        latency,
    ...
    // Step 3: Complex or low-confidence → Escalate to Opus
    if (haikuResult.classification === 'complex' || haikuResult.confidence < 0.85) {
      const opusResponse = await this.generateOpusResponse(userQuery)
      const latency = Date.now() - startTime

      // Step 4: Check Opus confidence for human review
      if (opusResponse.confidence < 0.70) {
        console.log(`⚠️  Opus confidence low (${opusResponse.confidence}). Escalating to human review.`)
        ...
        this.logMetrics({
          ...
          queryType: 'complex',
          modelUsed: 'opus',
          cost: MODEL_CONFIG.opus.avgCost,
          latency,
          confidence: opusResponse.confidence,
          escalated: true,
          humanReviewed: true
        })
        ...
        return {
          ...
          confidence: opusResponse.confidence,
          routedTo: 'human',
          response: opusResponse.response,
          cost: MODEL_CONFIG.opus.avgCost,
          latency,
          escalationReason: 'Opus confidence below threshold (0.70)',
          safetyFlags: []
        }
      }
    ...
    throw new Error('Triage logic error: no route matched')
  }

  /**
   * Haiku classification: Simple vs Complex
   * Fast, cheap determination of query complexity
   */
  private async classifyWithHaiku(userQuery: string): Promise<{
    classification: 'simple' | 'complex'
    confidence: number
  }> {
    const classificationPrompt = `You are a medical query classifier for a telehealth platform.

Classify this patient query as either:
- "simple": Routine questions about medications, refills, basic health advice
- "complex": Symptoms requiring differential diagnosis, multi-condition analysis, specialist knowledge
...
Respond ONLY with JSON:
{
  ...
  "reasoning": "brief explanation"
    ...
    const response = await this.client.messages.create({
      model: MODEL_CONFIG.haiku.model,
      max_tokens: 200,
      messages: [{
        role: 'user',
    ...
    console.log(`📊 Haiku classification: ${result.classification} (confidence: ${result.confidence})`)
    return {
  ...
  /**
   * Generate response using Haiku for simple queries
   */
  private async generateHaikuResponse(userQuery: string): Promise<string> {
    const response = await this.client.messages.create({
      model: MODEL_CONFIG.haiku.model,
      max_tokens: 500,
      messages: [{
        role: 'user',
        content: `You are a helpful medical assistant. Answer this patient query clearly and concisely:\n\n${userQuery}`
      }]
    })
  ...
  /**
   * Generate response using Opus for complex queries
   * Returns both response and confidence score
   */
  private async generateOpusResponse(userQuery: string): Promise<{
    response: string
    confidence: number
  }> {
    const prompt = `You are an expert medical assistant specializing in complex symptom analysis.

Patient Query: "${userQuery}"

Provide a detailed, evidence-based response. After your response, on a new line, include:
CONFIDENCE: &lt;0.0-1.0>

Where confidence represents your certainty in the diagnosis/recommendation.`

    const response = await this.client.messages.create({
      model: MODEL_CONFIG.opus.model,
      max_tokens: 1500,
      messages: [{
        role: 'user',
        content: prompt
    ...
    const confidenceMatch = fullResponse.match(/CONFIDENCE:\s*(0?\.\d+|1\.0)/)
  ...
  /**
   * Safety check: Detect high-risk keywords
   */
  private detectSafetyFlags(query: string): string[] {
    const lowerQuery = query.toLowerCase()
    return HIGH_RISK_KEYWORDS.filter(keyword => lowerQuery.includes(keyword))
  }

  /**
   * Log metrics for cost attribution and monitoring
   */
  private logMetrics(metrics: QueryMetrics): void {
    this.metricsStore.push(metrics)

    // In production, send to observability platform (Datadog, New Relic, etc.)
    console.log(`📈 Query metrics: ${JSON.stringify(metrics)}`)
  }

  /**
   * Cost analysis across all queries
   */
  getCostAnalysis(): {
    totalQueries: number
    totalCost: number
    ...
    const totalQueries = this.metricsStore.length
    const totalCost = this.metricsStore.reduce((sum, m) => sum + m.cost, 0)
    const avgCost = totalCost / totalQueries

    const costByModel = this.metricsStore.reduce((acc, m) => {
      if (!acc[m.modelUsed]) {
        acc[m.modelUsed] = { count: 0, totalCost: 0 }
      }
      acc[m.modelUsed].count++
      acc[m.modelUsed].totalCost += m.cost
      return acc
    }, {} as Record<string, { count: number, totalCost: number }>)

    const escalatedQueries = this.metricsStore.filter(m => m.escalated || m.humanReviewed).length
    const escalationRate = escalatedQueries / totalQueries
...
// Example usage
async function main() {
  const triage = new HealthcareTriage(process.env.ANTHROPIC_API_KEY!)

  // Example 1: Simple query (routes to Haiku)
  console.log('\n=== Example 1: Simple Query ===')
  const result1 = await triage.triageQuery(
    "Can I take ibuprofen with my blood pressure medication?",
    "user-123"
  )
  console.log(`Result: ${result1.routedTo} model, cost: $${result1.cost.toFixed(3)}`)

  // Example 2: Complex query (escalates to Opus)
  console.log('\n=== Example 2: Complex Query ===')
  const result2 = await triage.triageQuery(
    "I've had intermittent chest tightness for 3 days, worse with exertion. Also experiencing occasional dizziness. No family history of heart disease. What could this be?",
    "user-456"
  )
  console.log(`Result: ${result2.routedTo} model, cost: $${result2.cost.toFixed(3)}`)

  // Example 3: Safety-critical query (immediate human escalation)
  console.log('\n=== Example 3: Safety-Critical Query ===')
  const result3 = await triage.triageQuery(
    "I'm having severe chest pain radiating to my left arm",
    "user-789"
  )
  console.log(`Result: ${result3.routedTo}, safety flags: ${result3.safetyFlags.join(', ')}`)

  // Cost analysis after 1000 queries
  console.log('\n=== Cost Analysis ===')
  const analysis = triage.getCostAnalysis()
  console.log(`Total queries: ${analysis.totalQueries}`)
  console.log(`Average cost: $${analysis.avgCost.toFixed(3)}/query`)
  console.log(`Escalation rate: ${(analysis.escalationRate * 100).toFixed(1)}%`)
}
```

**Outcome Metrics**

**Cost Reduction (vs. Sonnet-only baseline)**:
- **Baseline**: 50,000 queries/day × $0.15 = $7,500/day ($225K/month)
- **Multi-Tier System**:
  - Haiku (95%): 47,500 queries × $0.01 = $475/day
  - Opus (4%): 2,000 queries × $0.30 = $600/day
  - Human review (1%): 500 queries × $0 = $0/day (internal staff)
  - **Total**: $1,075/day ($32K/month)
- **Savings**: $193K/month (86% cost reduction, exceeding 80% target)

**Quality Metrics**:
- **Safety**: 100% high-risk symptom detection (zero false negatives on test set)
- **Patient satisfaction**: 96% (up from 94% with Sonnet-only)
- **Escalation accuracy**: 92% of Opus escalations confirmed as complex by clinician review

**Performance**:
- **p95 latency**: 2.1s (Haiku queries: 0.8s, Opus queries: 4.2s)
- **Throughput**: 50,000 queries/day handled with 99.9% uptime

**Business Impact**:
- **Annual savings**: $2.3M (vs. Sonnet baseline)
- **ROI**: Implementation cost ($150K engineering + 3 months) paid back in &lt;1 month
- **Compliance**: HIPAA audit passed with full escalation logging

**Key Architectural Insight**: The 95/4/1 distribution (Haiku/Opus/Human) demonstrates the **power law of query complexity**. By routing intelligently, we upgraded quality for the 4% of queries that need it while achieving massive cost savings on the 95% that don't. This is cost-first architecture in practice.
