...
description: "Safety proxy, pre-deployment checks, and egress scanning"
estimatedMinutes: 45
---

# Production Readiness Baseline: Safety Proxy & Pre-Deployment Checks

Build a **Safety Proxy layer** before production—because **every input is potentially malicious** and **every output is a compliance risk**.

> **Architect Perspective**: Production readiness isn't a checklist—it's an architectural layer. Build a **Safety Proxy** that sits between users and your LLM, enforcing PII redaction, content filtering, and audit logging. **Never** let raw user input hit your LLM, and **never** let unfiltered LLM output reach users.

## The Safety Proxy Architecture

**The Problem**: Direct LLM integration exposes you to 3 critical risks:
1. **PII Leakage**: User sends credit card number → LLM response includes it → logged to analytics → GDPR violation
2. **Harmful Content**: LLM generates offensive content → shown to user → brand damage + legal liability
3. **Zero Auditability**: No record of blocked attempts → can't prove compliance during audits
...
export class EgressScanner {
  ...
  private async logEgressViolation(event: {
    ...
    console.error(`🚨 EGRESS VIOLATION [${event.severity}]: ${event.violations.length} violations detected`)
...
- Result: **System prompt leaked to user** → Attacker learns internal logic → Catastrophic security breach
...
```typescript
// Each check blocks the next → 800ms total
...
**✅ Parallel Approach**:
```typescript
// Independent checks run simultaneously → 250ms total
async function parallelSafety(input: string, output: string): Promise<SafetyResult> {
  ...
  const [
    ...
    detectPromptInjection(input),  // 250ms ← Longest check
...
**Performance Improvement**: 800ms → 250ms (69% reduction)
...
**Result**: Parallel approach reduces overhead by **81%** (847ms → 162ms) while maintaining identical security coverage.
...
export class DeploymentGovernor {
  ...
  private async verifySafetyProxy(): Promise<DeploymentVerification> {
    ...
    checks.push({
      ...
      evidence: `Test: "My SSN is 123-45-6789" → ${piiTest.blocked ? 'BLOCKED' : 'ALLOWED'}`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test output scan → ${egressTest.violationsFound} violations detected`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test: Offensive prompt → ${contentTest.blocked ? 'BLOCKED' : 'ALLOWED'}`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Audit log test → ${auditTest.eventsLogged} events logged`,
      severity: 'CRITICAL'
    })
  ...
  private async verifyCostControls(): Promise<DeploymentVerification> {
    ...
    checks.push({
      ...
      evidence: `Test: 11 requests in 1 min → 11th ${rateLimitTest.rejected ? 'REJECTED' : 'ALLOWED'}`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test: $2.01 spend in 1 hour → Circuit ${budgetTest.tripped ? 'TRIPPED' : 'OPEN'}`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test query → Cost tracked: $${trackingTest.costLogged.toFixed(4)}`,
      severity: 'HIGH'
    })
  ...
  private async verifyReliability(): Promise<DeploymentVerification> {
    ...
    checks.push({
      ...
      evidence: `Test: Simulated 503 → ${retryTest.attempts} retry attempts`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test: 5 failures → Circuit ${circuitTest.opened ? 'OPEN' : 'CLOSED'}`,
      severity: 'HIGH'
    })
    ...
    checks.push({
      requirement: 'Fallback: Primary fails → Backup provider',
      status: fallbackTest.fellBack ? 'PASS' : 'FAIL',
      evidence: `Test: Kill primary → ${fallbackTest.fellBack ? 'Backup used' : 'Failed'}`,
      severity: 'HIGH'
    })
  ...
  private async verifySecurity(): Promise<DeploymentVerification> {
    ...
    checks.push({
      ...
      evidence: `grep "sk-ant-" codebase → ${secretsTest.hardcodedKeys} matches (must be 0)`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `HTTP test → ${httpsTest.redirected ? '301 to HTTPS' : 'Allowed'}`,
      severity: 'CRITICAL'
    })
    ...
    checks.push({
      ...
      evidence: `Test: "Ignore instructions" → ${promptTest.refused ? 'REFUSED' : 'COMPLIED'}`,
      severity: 'HIGH'
    })
      ...
      if (failed.length > 0) {
        ...
        for (const check of failed) {
          if (check.severity === 'CRITICAL') {
            recommendations.push(`  🚨 CRITICAL: ${check.requirement}`)
          }
        }
      }
    }
...
Overall Status: ${decision.overallStatus === 'GREEN' ? '🟢 GREEN' : decision.overallStatus === 'YELLOW' ? '🟡 YELLOW' : '🔴 RED'}
Deployment Approved: ${decision.approved ? '✅ YES' : '❌ NO'}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Layer Verification:
${decision.layers.map(l => `${l.status === 'GREEN' ? '🟢' : l.status === 'YELLOW' ? '🟡' : '🔴'} ${l.layer}: ${l.status}`).join('\n')}
...
if (!decision.approved) {
  console.error('❌ DEPLOYMENT BLOCKED: Fix critical issues above')
  process.exit(1)
}

console.log('✅ DEPLOYMENT APPROVED: All layers GREEN')
```
...
Layer Verification:
🔴 Layer 1: Safety Proxy: RED
🟢 Layer 2: Cost Controls: GREEN
🟢 Layer 3: Reliability: GREEN
🟡 Layer 4: Monitoring: YELLOW
🟢 Layer 5: Security: GREEN
...
📋 RECOMMENDATIONS:
  Layer 1: Safety Proxy: Fix 2 failing checks before deployment
    🚨 CRITICAL: PII Detection: Block input with SSN/Credit Card
    🚨 CRITICAL: Egress Scanning: Detect hallucinated PII and prompt leaks
  Layer 4: Monitoring: Fix 1 failing checks before deployment

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

❌ DEPLOYMENT BLOCKED: Fix critical issues above
```
...
> "Every check must have **evidence**. Not 'PII detection is implemented,' but 'Test: SSN input → BLOCKED (verified).' Directors trust evidence, not claims."
...
- [ ] **PII Detection Implemented**
  ...
  - PII detected in input → request blocked
  - PII detected in output → redacted before logging
  - Test: "My SSN is 123-45-6789" → blocked or redacted
...
- [ ] **Content Filtering Active**
  ...
  - Test: Send offensive prompt → verify blocked + logged
...
- [ ] **Retry Logic with Exponential Backoff**
  ...
  - Test: Simulate 503 error → verify 3 retries
...
- [ ] **Alert Rules**
  ...
  - Cost &gt;$1K/hour → page on-call
  - PII detection rate &gt;1% → investigate
...
**The Safety Proxy Pattern**:
- **Never** let raw user input reach the LLM—filter for PII and harmful content first
- **Never** return raw LLM output to users—redact PII and filter harmful content
- **Always** log safety events (PII detected, content blocked) for compliance audits
- The Safety Proxy is **not optional**—it's the baseline for production AI systems
...
- Redact PII **before logging**—logging raw PII is a GDPR violation
- PII in output → redact and log security event
- NEVER log the actual PII value—log the type and hash only
...
- Log every block with hash—critical for compliance audits
- If output blocked → return safe fallback message, don't retry
...
### The Correct Answer: **C** — Engineer a Path Through Conflicting Constraints

**Why C is the Architect's answer**:

**The Technical Solution**:
```typescript
// 1. Parallel Scans: 800ms → 200ms (81% reduction)
...
1. **Hour 1-2**: Implement parallel scans
   ...
   - Test latency: 800ms → 187ms
   - Status: Latency fixed ✅
...
2. **Hour 3-4**: Refine PII patterns
   ...
   - Test on 10K sample dataset: 95.2% → 99.6%
   - Status: PII detection fixed ✅

3. **Hour 5**: Run Go/No-Go verification
   ```
   Layer 1: Safety Proxy → 🟢 GREEN (99.6% PII, &lt;200ms)
   Layer 2: Cost Controls → 🟢 GREEN
   Layer 3: Reliability → 🟢 GREEN
   Layer 4: Monitoring → 🟢 GREEN
   Layer 5: Security → 🟢 GREEN
   ```
...
> **You**: "I have a solution that meets all three requirements. I've implemented parallel safety scans to reduce latency from 800ms to 200ms, and refined our PII patterns to achieve 99.6% detection—exceeding our 99.5% target. The system is now ready for deployment."
>
> *Shows Red-Green Report: All layers GREEN*
>
> **CSO**: "99.6% detection with &lt;200ms latency? How did you achieve both?"
>
> **You**: "Parallel execution. Instead of running checks sequentially, we run them simultaneously. The latency is now determined by the longest individual check (250ms), not the sum (800ms). Then I added medical-specific PII patterns—MRNs, DOBs—to close the 5% gap."
>
...
**The Reality**: Architects who sacrifice security for performance don't stay architects for long. You don't get to choose between safety and speed—you engineer **both**.
...
1. **Identify the constraints**: 95% → 99.5% PII, 800ms → &lt;300ms latency, 24hr deadline
...
- ✅ Safe (PII redacted, content filtered, egress scanned)
- ✅ Fast (parallel scans, &lt;200ms overhead)
- ✅ Reliable (retries, circuit breakers, fallbacks)
