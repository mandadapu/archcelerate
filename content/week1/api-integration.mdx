---
title: "API Resilience & Integration"
description: "Fault tolerance, model cascades, and production-grade reliability patterns"
estimatedMinutes: 45
---
...
## 🏗️ Infrastructure-Level Resilience Patterns
...
export class CircuitBreakerWithFailover {
    ...
    try {
      ...
      for (const provider of this.providers.slice(1)) {
        ...
        try {
          ...
          console.log(`✅ Failover successful - ${provider.name} is now primary`);
  ...
  private evaluateCircuitState(providerName: string) {
    ...
    if (errorRate >= this.FAILURE_THRESHOLD) {
      circuit.state = 'OPEN';
      console.warn(`⚠️ Circuit breaker TRIPPED for ${providerName} - error rate ${(errorRate * 100).toFixed(1)}%`);
    }
  }
...
async function chatCompletion(messages: Message[]) {
  ...
  console.log(`✅ Served by ${provider}`);
...
export class SemanticCache {
  ...
  async get(query: string): Promise<{
    ...
    if (bestMatch && bestMatch.similarity >= this.SIMILARITY_THRESHOLD) {
      ...
      console.log(`✅ Semantic cache HIT (similarity: ${(bestMatch.similarity * 100).toFixed(1)}%, latency: ${Date.now() - startTime}ms)`);
    ...
    console.log(`❌ Semantic cache MISS (latency: ${Date.now() - startTime}ms)`);
    return { hit: false };
  }
...
// Cache HIT - semantically similar (200ms) ✅ 15x faster
```

**Cost Impact**:
- **Without semantic cache**: 100K requests/day → 100K LLM calls → $300/day
- **With semantic cache** (60% hit rate): 100K requests → 40K LLM calls + 60K embeddings → $120/day + $1.20 = **$121.20/day** → **60% cost savings**
...
export class StatefulTaskManager {
  ...
  async createTask(params: {
    ...
    console.log(`✅ Created task ${task.id} with ${params.totalSteps} steps`);
    return task;
  }
  ...
  async executeWithCheckpoints(
    ...
    console.log(`▶️  Starting task ${taskId} from step ${startStep}/${steps.length}`);
    ...
    try {
      for (let i = startStep; i < steps.length; i++) {
        ...
        console.log(`   ✅ Checkpoint saved (${executionTime}ms)`);
      }
      ...
      console.error(`❌ Task ${taskId} failed at step ${task.currentStep}:`, error);
  ...
  async resumeTask(taskId: string): Promise<void> {
    ...
    console.log(`🔄 Resuming task ${taskId} from step ${task.currentStep}`);
...
async function runResearchTask(userId: string, topic: string) {
  ...
  try {
    ...
    console.error('❌ Task failed - can resume later:', error);
...
**Cost & Time Savings**:
- **Without checkpointing**: Task fails at Step 9/10 → Restart all 10 steps → 2x tokens, 2x time
- **With checkpointing**: Task fails at Step 9/10 → Resume from Step 9 → 10% extra tokens, minimal time loss
...
## 🎯 Architect Challenge: High-Availability Scenario
...
**Analysis**:
- ✅ Simple code change
- ❌ Creates "Retry Storm" - hammers failing service with 10x traffic
- ❌ Increases user latency (10 × 30s = 5 minutes timeout)
- ❌ Wastes API quota on doomed requests
- **Result**: Makes outage WORSE, not better

---

**Option B: Implement Automated Model Cascade with Pre-Warmed Backup** ✅ OPTIMAL
...
**Analysis**:
- ✅ **Eliminates single point of failure** - Different cloud provider
- ✅ **Instant failover** - Circuit breaker trips at 50% error rate
- ✅ **Maintains user experience** - Transparent provider switch
- ✅ **Cost-effective** - Only uses backup during outages
- ✅ **Auto-recovery** - Returns to Claude when health restored
...
- ❌ **Breaks user trust** - Your product appears unreliable
- ❌ **Revenue loss** - Features unavailable = customer churn
- ❌ **Competitive disadvantage** - Competitors with failover keep running
...
**Analysis**:
- ✅ Better perceived latency for successful requests
- ❌ **Doesn't fix the 87% error rate** - Users still see failures
- ❌ **Streaming fails too** - 503 errors occur before streaming starts
- ❌ **Misleading solution** - Treats symptom, not cause
...
## 🏗️ Production Implementation: The Resilient AI Service

<Callout type="success" title="Cornerstone Pattern">
This is the **cornerstone of API resilience** - a complete, deployable service demonstrating all patterns working together: Model Cascade, Circuit Breaker, and Automatic Failover.
</Callout>

**The Problem**: Relying on a single provider is a single point of failure. When Claude goes down, your product stops working.

**The Solution**: Implement a Model Cascade that automatically attempts the request with a secondary model (Gemini, GPT) before returning an error to the user.

**Strategy**: Primary Attempt → Error Detection → Circuit Breaker Check → Fallback

### ResilientAIService Implementation

```typescript
/**
 * ARCHCELERATE MASTER PATTERN: The Resilient Model Cascade
 * Strategy: Primary Attempt -> Error Detection -> Circuit Breaker Check -> Fallback
 */
...
export class ResilientAIService {
  private primary = new Anthropic({
    apiKey: process.env.ANTHROPIC_API_KEY
  });

  private secondary = new GoogleGenerativeAI(
  ...
  async generateWithFallback(prompt: string): Promise<{
    ...
    if (circuitOpen) {
      console.warn("⚠️ Circuit Open: Routing directly to Secondary Provider (Gemini)");

      const result = await this.callSecondary(prompt);
      ...
      return {
        ...
        metrics: {
          ...
          cost: this.calculateCost('google', result.tokensUsed),
    ...
    try {
      // 2. Primary Attempt (Claude 3.5 Sonnet)
      ...
      console.error(`❌ Primary Provider Failed (${error.status || error.code})`);
      console.log(`   Failure count: ${this.failureCount}/${this.FAILURE_THRESHOLD}`);

      // 3. Fallback Logic
      // Trigger for 429 (Rate Limit) or 5xx (Provider Down) errors
      ...
      // Permanent errors (401 Unauthorized, 400 Bad Request) should still fail
      throw error;
    }
  }

  /**
   * Call secondary provider (Gemini)
   */
  private async callSecondary(prompt: string): Promise<{
    text: string;
    tokensUsed: number;
  }> {
    const model = this.secondary.getGenerativeModel({
      model: "gemini-1.5-pro"
    });
  ...
  private isCircuitOpen(): boolean {
    ...
    if (this.failureCount >= this.FAILURE_THRESHOLD) {
      // Just tripped - start cooldown
      this.circuitOpenUntil = Date.now() + this.CIRCUIT_COOLDOWN_MS;
      console.warn(`🔴 Circuit Breaker TRIPPED - Cooldown for ${this.CIRCUIT_COOLDOWN_MS / 1000}s`);
      return true;
    }
  ...
  private resetCircuit() {
    if (this.failureCount > 0) {
      console.log('✅ Primary Provider Recovered - Circuit Breaker Reset');
    }
    this.failureCount = 0;
  ...
  private calculateCost(
    ...
    const costPer1MTokens = {
      anthropic: 3.00,    // Claude Sonnet 4.5 average (input + output)
      google: 1.25,       // Gemini 1.5 Pro average
      openai: 5.00        // GPT-4 average
    };
  ...
  getCircuitStatus() {
    return {
      ...
      cooldownEndsAt: this.circuitOpenUntil > Date.now()
...
export async function POST(req: Request) {
  ...
  try {
    const { text, metrics } = await aiService.generateWithFallback(prompt);

    console.log(`✅ Response from ${metrics.provider}`);
    console.log(`   Latency: ${metrics.latencyMs}ms`);
    console.log(`   Cost: $${metrics.cost.toFixed(6)}`);
    console.log(`   Fallback: ${metrics.fallbackTriggered ? 'Yes' : 'No'}`);

    return Response.json({
      text,
      metrics,
      circuitStatus: aiService.getCircuitStatus()
    });
...
<Callout type="warning" title="Three Production Principles">
These principles separate hobby projects from production-grade resilience.
</Callout>

#### 1. Provider Diversity (Cloud Infrastructure)

**Never use the same cloud provider for your fallback**. If AWS US-East-1 goes down, and both your models are hosted there, your fallback is useless.

**Smart Routing**:
- **Primary**: Anthropic Claude (AWS/GCP)
- **Secondary**: Google Gemini (GCP different region) OR OpenAI GPT (Azure)

```typescript
// ✅ GOOD: Different cloud providers
const resilientService = new ResilientAIService();
// Primary: Anthropic (AWS US-East-1)
// Fallback: Gemini (GCP US-Central) or OpenAI (Azure East-US)

// ❌ BAD: Same cloud provider, same region
// Primary: Anthropic (AWS US-East-1)
// Fallback: AWS Bedrock Claude (AWS US-East-1)  // ⚠️ No redundancy!
```

**Real-World Example**: During the December 2023 AWS US-East-1 outage, services with Anthropic → OpenAI failover maintained uptime. Services with Anthropic → Bedrock (both on AWS) went down together.

#### 2. The "Vibe" Shift (Response Consistency)

**The Problem**: Gemini and Claude have different "personalities." A fallback response might feel different to the user (tone, verbosity, formatting).

**The Solution**: Use a **Standardized System Prompt** across both models to minimize the delta.

```typescript
const STANDARDIZED_SYSTEM_PROMPT = `You are a professional assistant.
Follow these rules strictly:
- Be concise and direct (2-3 sentences max)
- Use bullet points for lists
- Avoid conversational filler ("Great question!", "I'd be happy to help!")
- Match the user's tone (formal/casual)`;

// Apply to both providers
async generateWithFallback(prompt: string) {
  const fullPrompt = `${STANDARDIZED_SYSTEM_PROMPT}\n\nUser query: ${prompt}`;

  // Now both Claude and Gemini will follow same guidelines
  // ...
}
```

**Before Standardization**:
- Claude: "I'd be happy to help! Here are three key points: 1) ..."
- Gemini: "Sure thing! Let me break this down for you in a fun way: ..."

**After Standardization**:
- Claude: "Three key points: 1) ... 2) ... 3) ..."
- Gemini: "Three key points: 1) ... 2) ... 3) ..."

**User Impact**: Reduced "jarring" transitions when fallback occurs. Users don't notice the provider switch.

#### 3. Cost Monitoring (Emergency Traffic Auditing)

**The Problem**: Fallback models can sometimes be more expensive. Without tracking, you might discover unexpected costs.

**The Solution**: Track `provider_name` in your cost monitoring so you can audit "Emergency Traffic" spending.

```typescript
// Track costs per provider
async function trackProviderCost(userId: string, metrics: GenerationMetrics) {
  await prisma.usage.create({
    data: {
      userId,
      provider: metrics.provider,        // ✅ Track which provider
      model: metrics.model,
      tokensUsed: metrics.tokensUsed,
      cost: metrics.cost,
      fallbackTriggered: metrics.fallbackTriggered,  // ✅ Was this emergency traffic?
      timestamp: new Date()
    }
  });
}

// Query emergency traffic costs
const emergencyCosts = await prisma.usage.aggregate({
  where: {
    fallbackTriggered: true,
    timestamp: {
...
console.log(`Emergency fallback costs (Feb 2026): $${emergencyCosts._sum.cost}`);
```

**Cost Analysis Dashboard**:
```typescript
// Example output
Primary (Anthropic):  $1,200  (80,000 requests)
Fallback (Gemini):    $180    (12,000 requests, 15% of total)
Emergency %:          13%     (within acceptable range)
```

**Alert Thresholds**: If fallback traffic exceeds 20%, investigate:
- Is primary provider having reliability issues?
- Are rate limits set too low?
- Is circuit breaker threshold too aggressive?

---

## 🏁 Week 1 Lab: The Resilience Test

<Callout type="success" title="Chaos Engineering">
This lab teaches students to **prove** their system is resilient by simulating real production failures. Used by Netflix, Amazon, and Google to validate infrastructure.
</Callout>

### The Challenge

Build a system that maintains **100% successful response rate** despite a **30% simulated failure rate** in the primary provider.
...
/**
 * Chaos Monkey: Randomly injects failures to test resilience
 * Used in testing/staging environments to validate fallback logic
 */

export class ChaosMonkey {
  private readonly failureRate: number;

  constructor(failureRate: number = 0.3) {  // 30% default
    this.failureRate = failureRate;
  }

  /**
   * Wrap an API call to randomly inject failures
   */
  async inject<T>(
    fn: () => Promise<T>,
    errorTypes: Array<'rate_limit' | 'server_error' | 'timeout'> = ['rate_limit', 'server_error']
  ): Promise<T> {
    // Randomly decide if this call should fail
    if (Math.random() < this.failureRate) {
      const errorType = errorTypes[Math.floor(Math.random() * errorTypes.length)];

      console.log(`🐵 Chaos Monkey: Injecting ${errorType} error`);
  ...
  getStats() {
    return {
      ...
      expectedFailures: `${(this.failureRate * 100).toFixed(0)}%`
    };
  }
}
```

### Integrate Chaos Monkey into ResilientAIService

```typescript
// For testing only - inject chaos into primary provider
export class ResilientAIService {
  private chaosMonkey?: ChaosMonkey;

  enableChaos(failureRate: number = 0.3) {
    this.chaosMonkey = new ChaosMonkey(failureRate);
    console.log(`🐵 Chaos Monkey enabled with ${failureRate * 100}% failure rate`);
  }
  ...
  async generateWithFallback(prompt: string) {
    ...
    try {
      // Inject chaos if enabled (testing only!)
      const response = this.chaosMonkey
        ? await this.chaosMonkey.inject(() => this.primary.messages.create({
            model: "claude-sonnet-4-5-20251101",
            max_tokens: 1024,
            messages: [{ role: "user", content: prompt }]
          }))
        : await this.primary.messages.create({
            model: "claude-sonnet-4-5-20251101",
            max_tokens: 1024,
            messages: [{ role: "user", content: prompt }]
          });
      ...
      // Fallback logic handles both real and injected failures
      // ...
    }
  }
}
...
import { ResilientAIService } from '@/lib/resilient-ai-service';

describe('Week 1 Lab: Resilience Test', () => {
  const aiService = new ResilientAIService();

  /**
   * VERIFICATION METRIC 1: Uptime
   * System must maintain 100% successful response rate
   * despite 30% simulated failure rate
   */
  it('maintains 100% uptime with 30% primary failure rate', async () => {
    // Enable Chaos Monkey
    aiService.enableChaos(0.30);  // 30% failure rate

    const testCases = Array(100).fill('Test query');
    const results: Array<{ success: boolean; provider: string; latency: number }> = [];

    for (const query of testCases) {
      try {
        const { text, metrics } = await aiService.generateWithFallback(query);
    ...
    const fallbackCount = results.filter(r => r.provider === 'google').length;
    const avgLatency = results.reduce((sum, r) => sum + r.latency, 0) / results.length;

    console.log('\n📊 Resilience Test Results:');
    console.log(`   Total requests: ${testCases.length}`);
    console.log(`   Successful: ${successCount} (${(successCount / testCases.length * 100).toFixed(1)}%)`);
    console.log(`   Failed: ${failureCount}`);
    console.log(`   Fallback triggered: ${fallbackCount} times`);
    console.log(`   Average latency: ${avgLatency.toFixed(0)}ms`);

    // VERIFICATION: 100% uptime required
    expect(successCount).toBe(testCases.length);  // All 100 requests must succeed
    expect(failureCount).toBe(0);

    // Should have triggered fallback (with 30% failure rate)
    expect(fallbackCount).toBeGreaterThan(0);

  }, 60000);  // 60s timeout for 100 requests

  /**
   * VERIFICATION METRIC 2: Latency
   * Fallback triggers must occur in under 500ms of primary failure
   */
  it('triggers fallback within 500ms of primary failure', async () => {
    aiService.enableChaos(1.0);  // 100% failure rate - force fallback

    const startTime = Date.now();

    const { text, metrics } = await aiService.generateWithFallback('Test query');

    const totalLatency = Date.now() - startTime;

    console.log(`\n⏱️  Fallback Latency Test:`);
    console.log(`   Provider used: ${metrics.provider}`);
    console.log(`   Total latency: ${totalLatency}ms`);
    console.log(`   Fallback triggered: ${metrics.fallbackTriggered}`);

    // Verify fallback was used (since primary has 100% failure)
    expect(metrics.provider).toBe('google');
    expect(metrics.fallbackTriggered).toBe(true);

    // Verify latency is acceptable
    expect(totalLatency).toBeLessThan(500);  // Must fail over in &lt;500ms
  });

  /**
   * VERIFICATION METRIC 3: Circuit Breaker
   * After 3 consecutive failures, circuit should open and route directly to secondary
   */
  it('opens circuit after failure threshold and routes to secondary', async () => {
    const freshService = new ResilientAIService();
    freshService.enableChaos(1.0);  // 100% failure to trip circuit

    // Make 3 requests to trip circuit breaker
    for (let i = 0; i &lt; 3; i++) {
      await freshService.generateWithFallback('Test query');
    }

    const status = freshService.getCircuitStatus();
    console.log('\n🔴 Circuit Breaker Status:', status);

    expect(status.circuitOpen).toBe(true);
    expect(status.failureCount).toBe(3);

    // Next request should go directly to secondary (no primary attempt)
    const { metrics } = await freshService.generateWithFallback('Test query');
    expect(metrics.provider).toBe('google');
    expect(metrics.circuitBreakerOpen).toBe(true);
  });

  /**
   * VERIFICATION METRIC 4: Cost Tracking
   * Emergency traffic costs should be tracked separately
   */
  it('tracks emergency fallback costs separately', async () => {
    aiService.enableChaos(0.30);

    const requests = 50;
    const costs: Array<{ provider: string; cost: number; fallback: boolean }> = [];

    for (let i = 0; i < requests; i++) {
      const { metrics } = await aiService.generateWithFallback('Test query');
      costs.push({
        provider: metrics.provider,
        cost: metrics.cost,
    ...
    const emergencyCost = costs
      .filter(c => c.fallback)
      .reduce((sum, c) => sum + c.cost, 0);
    const emergencyPercentage = (emergencyCost / totalCost) * 100;

    console.log('\n💰 Cost Analysis:');
    console.log(`   Total cost: $${totalCost.toFixed(6)}`);
    console.log(`   Emergency traffic cost: $${emergencyCost.toFixed(6)} (${emergencyPercentage.toFixed(1)}%)`);
    console.log(`   Primary (Anthropic) calls: ${costs.filter(c => c.provider === 'anthropic').length}`);
    console.log(`   Fallback (Gemini) calls: ${costs.filter(c => c.provider === 'google').length}`);

    // Verify emergency costs are tracked
    expect(emergencyCost).toBeGreaterThan(0);
    expect(emergencyPercentage).toBeLessThan(50);  // Should be minority of traffic
...
**To pass the Week 1 Resilience Test**, students must demonstrate:

✅ **100% Uptime**: All requests succeed despite 30% primary failure rate

✅ **&lt;500ms Fallback Latency**: Circuit breaker triggers within latency budget

✅ **Circuit Breaker Behavior**: After 3 failures, routes directly to secondary

✅ **Cost Tracking**: Emergency traffic costs tracked separately
...
🔴 Circuit Breaker Status:
   Circuit Open: true ✅
   Failure count: 3
   Routes directly to secondary

💰 Cost Analysis:
   Total cost: $0.004521
   Emergency traffic cost: $0.001204 (26.6%) ✅
   Primary (Anthropic) calls: 72
   Fallback (Gemini) calls: 28

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ ALL VERIFICATION METRICS PASSED
🎓 Status: RESILIENCE TEST COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your system demonstrates production-grade resilience:
✓ Survives provider outages (100% uptime)
✓ Fast failover (&lt;500ms)
✓ Intelligent circuit breaking
✓ Cost-aware emergency routing
```

**Architect's Lesson**: "Chaos engineering proves your system is resilient before production traffic does. Netflix runs Chaos Monkey in production. You run it in testing. Same principle: prove it works before customers suffer."
