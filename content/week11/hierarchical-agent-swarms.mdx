...
## 🔬 Real-World Challenge: The Oncology Research Swarm
...
```
  ...
  │ The Scout │   │Statistician│   │ Reviewer  │
  └───────────┘   └───────────┘   └───────────┘
```
...
- Makes final inclusion decision (Quality score ≥ 7)
...
async function scoutAgent(
  ...
  console.log(`\n🔍 Scout: Searching ${source} for oncology papers`)
...
async function statisticianAgent(
  ...
  console.log(`\n📊 Statistician: Reviewing "${paper.title}"`)
...
async function reviewerAgent(
  ...
  console.log(`\n👨‍⚕️ Reviewer: Assessing clinical relevance`)
...
async function supervisorAgent(
  ...
  console.log(`\n🎯 Supervisor: Making final decision`)
...
async function runOncologyResearchSwarm(
  ...
  console.log('\n🚀 Starting Oncology Research Swarm')
  ...
  for (const paper of allPapers) {
    ...
    if (briefs.length >= 100) {
      console.log('  ⚠️ Budget limit reached, stopping early')
      break
    }
  }
...
- Researcher time: 8 hours/day × 2 researchers = 160 hours/month
...
- Cost: $1,200/month (API costs: $0.40/paper × 3,000)
...
- **Researcher time freed**: 320 → 10 hours/month (97% reduction)
...
// - Clear separation of concerns (Scout → Stats → Review)
...
const circuitBreakers = {
  ...
  qualityThreshold: 7,  // Only include papers scoring ≥7
  humanReviewFlag: true  // Escalate ambiguous cases
}

if (totalCost >= circuitBreakers.budgetLimit) {
  console.log('⚠️ Budget limit reached, stopping swarm')
  return briefs
}
```
...
async function feedbackLoop(briefs: ResearchBrief[]) {
  ...
  if (disagreements.length > briefs.length * 0.2) {
    console.log('⚠️ High disagreement rate, triggering recalibration')
...
- Papers with breakthrough novelty + moderate quality → Human review
- Contradictions between agents → Human review
- Any paper flagged by 2+ agents → Human review
...
| **Scalability** | Hard (n² connections) | Easy (add workers, not connections) |
    ...
    await Promise.all(
      tasks.map(async (task) => {
        ...
        try {
          ...
          console.log(`🔄 ${worker.name} executing: ${task.description}`)
          ...
          console.log(`✅ ${worker.name} completed: ${task.description}`)
          ...
          if (task.retry_count < 2) {
            console.log(`🔄 Retrying task: ${task.description}`)
...
Synthesize these results into a coherent final deliverable that achieves the goal.`
  ...
  async execute(goal: string): Promise<string> {
    console.log(`\n🎯 Manager Agent: Starting workflow for goal: "${goal}"\n`)
    ...
    console.log(`\n✅ All tasks completed (${results.size}/${tasks.length})\n`)
...
**Output:**
```
🎯 Manager Agent: Starting workflow for goal: "Launch a new AI-powered customer support chatbot product..."

📋 Decomposed into 4 tasks

🔄 Market Researcher executing: Analyze the customer support chatbot market...
🔄 Tech Lead executing: Design technical architecture for AI chatbot...
🔄 Marketing Writer executing: Create go-to-market strategy and messaging...
🔄 Financial Analyst executing: Create budget and ROI projections...

✅ Market Researcher completed: Analyze the customer support chatbot market...
✅ Tech Lead completed: Design technical architecture for AI chatbot...
✅ Marketing Writer completed: Create go-to-market strategy and messaging...
✅ Financial Analyst completed: Create budget and ROI projections...
...
**Scenario:** Manager agent delegates task to Worker A → Worker A asks for clarification → Manager delegates to Worker B → Worker B asks for clarification → **Infinite loop!**
...
8. **LOOP!** Manager ↔ Product Manager ↔ Finance ↔ Manager...
...
class SafeManagerAgent extends ManagerAgent {
  ...
  protected async executeWorkerTask(worker: WorkerAgent, task: Task): Promise<string> {
    ...
    if (circuitCheck.break) {
      throw new Error(`🛑 Circuit breaker opened: ${circuitCheck.reason}`)
    }
  ...
  protected async executeWorkerTask(worker: WorkerAgent, task: Task): Promise<string> {
    ...
    while (revisionCount <= this.maxRevisions) {
      ...
      if (verification.passed) {
        console.log(`✅ Verification passed for: ${task.description}`)
        return result
      }

      // Request revision
      console.log(`⚠️ Verification failed: ${verification.reason}`)
      console.log(`🔄 Requesting revision (${revisionCount + 1}/${this.maxRevisions})`)
...
Please revise your work addressing the issues above.`
    ...
    console.warn(`⚠️ Max revisions reached for: ${task.description}`)
    return result!
  }
...
// Output:
// 🔄 Market Researcher executing: Analyze the AI chatbot market...
// ⚠️ Verification failed: Output lacks specific numbers and sources
// 🔄 Requesting revision (1/2)
// 🔄 Market Researcher executing: [REVISED TASK]
// ✅ Verification passed for: Analyze the AI chatbot market...
```
