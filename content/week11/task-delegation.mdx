...
week: 11
concept: 2
description: "Build enterprise orchestration with Dynamic Replanning, DAG Execution Engines, and Queue-Based Load Balancing for self-healing multi-agent systems"
estimatedMinutes: 65
---
...
**The Problem**: The `analyzeTask` function above produces a static execution plan. But in production, agents fail. Research times out. APIs return errors. An LLM produces a "High Uncertainty" flag on its output. A static plan treats these as terminal failures — the entire workflow crashes because one subtask couldn't complete.
...
class DynamicPlanner {
  ...
  async evaluateAndReplan(
    ...
    // Case 1: Task succeeded with high confidence — continue as planned
    if (completedTask.status === 'completed'
        && completedTask.confidence >= this.confidenceThreshold) {
      console.log(`✅ Task ${completedTask.taskId} completed (confidence: ${completedTask.confidence})`)
      return { updatedPlan: remainingPlan }
    }

    // Case 2: Task failed — trigger replanning
    if (completedTask.status === 'failed') {
      return this.handleFailure(completedTask, remainingPlan, availableAgents)
    }

    // Case 3: Low confidence — agent is uncertain about its output
  ...
  private async handleFailure(
    ...
    if (failedTask.attempts < this.maxRetries) {
      ...
      console.log(`🔄 Retrying ${failedTask.taskId} (attempt ${failedTask.attempts + 1})`)
    ...
    if (alternativeAgent) {
      ...
      this.mutations.push(mutation)
      console.log(`🔀 Delegating ${failedTask.taskId} to ${alternativeAgent.role}`)
      return { updatedPlan: remainingPlan, mutation }
    }
    ...
    this.mutations.push(mutation)
    console.log(`⚠️ Skipping ${failedTask.taskId} with fallback`)
    return { updatedPlan: remainingPlan, mutation }
  }
  ...
  private async handleLowConfidence(
    ...
    console.log(`🔍 Decomposed uncertain task into ${mutation.newPlan!.length} subtasks`)
...
|----------|------------|-------------------|
| **Agent failure** | Entire workflow crashes | Retry → Delegate → Fallback |
...
**Architect's Insight**: "Don't manually manage Promise.all. Build a Dependency Graph. The orchestrator identifies which tasks have zero dependencies and fires them off in parallel. As tasks finish, it 'unlocks' the next tier of the graph. This maximizes throughput — running Research and Coding simultaneously — while ensuring the Synthesis stage never starts with empty inputs."
...
class DAGExecutor {
  ...
  async execute(
    ...
    while (this.completedNodes.size < this.nodes.size) {
      ...
      console.log(`⚡ Executing Tier ${tierNum}: ${readyNodes.length} tasks in parallel`)

      // Execute all ready nodes in parallel
      const tierResults = await Promise.allSettled(
        readyNodes.map(async (node) => {
          node.status = 'running'
          console.log(`  → Running: ${node.task.description}`)
      ...
      for (let i = 0; i < tierResults.length; i++) {
        if (tierResults[i].status === 'rejected') {
          ...
          console.log(`  ✗ Failed: ${readyNodes[i].task.description}`)
        }
      }
    }
...
//   Tier 1 (parallel):
//     code: Write code examples ← [research]
//     benchmarks: Gather benchmarks ← [research]
//   Tier 2 (parallel):
//     draft: Write blog post ← [research, code, benchmarks]
//   Tier 3 (parallel):
...
>
> **You:** "We use a DAG execution engine. Each subtask declares its dependencies, and the engine builds a directed acyclic graph. It assigns execution tiers via topological sort — tier 0 has zero dependencies and fires immediately in parallel. As tier-0 tasks complete, they unlock tier-1 nodes, and so on. This maximizes concurrency automatically without manual Promise.all grouping. At 50 tasks, the DAG engine achieves 65% better throughput than sequential execution because it identifies all parallelizable paths. It also integrates with our Dynamic Planner — when a replanning mutation injects new subtasks, the DAG just adds nodes and recalculates tiers."
...
// Tenant B submits 1 task — gets served ahead of Tenant A's backlog
...
2. **Multi-Factor Routing**: Score agents on capability, load, performance, and speed — not just capability alone
3. **Dependency Validation**: Always check for circular dependencies before executing a decomposed plan
4. **Manager-Worker Pattern**: One coordinator, many specialists — decompose, route, execute, synthesize, validate
...
7. **Dynamic Replanning**: Plans must be mutable — agent failures trigger selective retry, delegation to alternatives, or task decomposition, not full restarts
8. **DAG Execution**: Build dependency graphs with topological sort to maximize concurrent throughput automatically, rather than manually grouping Promise.all batches
9. **Queue-Based Load Balancing**: Fair-share scheduling with priority penalties prevents power users from monopolizing multi-tenant agent pools
10. **Granular Resilience**: Design for selective subtask recovery — one agent failure should cost seconds and cents, not minutes and dollars

---

## 🎯 Architect Challenge: The "Orchestration Crisis" COO Triage
