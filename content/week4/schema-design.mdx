...
```typescript
// ✅ GOOD: Positive + Negative constraints
{
  name: 'get_user_profile',
  description: 'Get user identity and contact information (name, email, phone, address). Use this when user asks about personal details, contact info, or account settings. DO NOT use this for payment methods, invoices, subscription status, or billing history—use get_user_billing instead.',
  input_schema: {
    properties: {
      user_id: {
        type: 'string',
...
{
  name: 'get_user_billing',
  description: 'Get user payment and subscription information (payment methods, invoices, subscription tier, billing history). Use this when user asks about payments, charges, invoices, or subscription status. DO NOT use this for personal details, contact info, or account settings—use get_user_profile instead.',
  input_schema: {
    properties: {
      user_id: {
        type: 'string',
...
interface BoundedToolDefinition extends Anthropic.Tool {
  ...
  boundaries: {
    ...
    alternativesFor: {   // Suggest correct tool for excluded cases
      [excludedCase: string]: string  // excluded case → correct tool name
    }
  }
}
...
> **The Gap**: When you have 20+ tools in a large system, even with boundary definitions, you'll still have tool overlap—multiple tools that *could* handle the same request, leading to "selection flapping" where the LLM randomly picks one.
...
- All return similar data, but different schemas → downstream code breaks
...
```typescript
// ✅ BETTER: Single tool with type discriminator
{
  name: 'get_order_info',
...
for (const overlap of auditResults.overlaps) {
  console.log(`
    🚨 Collision detected: ${overlap.tool1} ↔ ${overlap.tool2}
...
// Output:
// 🚨 Collision detected: get_order_status ↔ track_order
// Similarity: 85%
...
track_order(order_id) → { tracking_number, carrier }
check_order(order_id) → { items, total }
...
**Example**:
```typescript
// ✅ GOOD: Discriminator for variants
get_user_data(user_id, type: 'profile' | 'billing' | 'preferences')

// ❌ BAD: Don't merge unrelated operations
...
```typescript
// ❌ DANGEROUS: Exposes internal structure
{
  name: 'get_file',
...
```typescript
// ✅ SECURE: Opaque UUIDs, constrained paths
{
  name: 'get_file',
...
|----------|----------------|----------------|---------------|--------------|
| **Raw IDs/Paths** | ❌ Vulnerable | ❌ Easy | ❌ Possible | ❌ No trace |
| **Opaque UUIDs** | ✅ Prevented | ✅ Impossible | ✅ Sanitized | ✅ Full audit trail |
...
**The Architect's Solution**: Use the `examples` field in JSON Schema to provide "Boundary Pairs"—one standard example and one edge case.
...
// ❌ BAD: Examples in system prompt (detached from tool)
systemPrompt: `
...
```typescript
// ✅ GOOD: Examples attached to tool definition
{
  name: 'format_currency',
...
### ❌ Option A: System Prompt Instruction (Wrong)
...
### ❌ Option C: Database Fix - Allow Empty Strings (Wrong)
...
### ❌ Option D: Runtime Safety Proxy (Partially Wrong)
...
### ✅ Option B: Schema-Level Constraint (Correct)
...
- Database crashes: 15/week → 0/week (100% elimination)
- SLA credit cost: $12K/month → $0 (100% savings)
...
|----------------|--------|---------|----------------|---------------|
| **Data format** (email, UUID, regex) | ✅ Primary | ✅ Backup | ❌ | ❌ |
| **Data bounds** (minLength, min/max) | ✅ Primary | ✅ Backup | ❌ | ❌ |
| **Enums** (categorical values) | ✅ Primary | ✅ Backup | ❌ | ❌ |
| **Required fields** | ✅ Primary | ✅ Backup | ❌ | ❌ |
| **Authorization** (user permissions) | ❌ | ❌ | ✅ Primary | ❌ |
| **Business rules** (order > $1000 needs approval) | ❌ | ❌ | ✅ Primary | ❌ |
| **Tone/style** (friendly vs formal) | ❌ | ❌ | ❌ | ✅ Only option |
| **Nuanced behavior** (when to escalate) | ❌ | ❌ | ❌ | ✅ Guidance |

**Correct Answer: B** — Update the tool's JSON Schema with `"minLength": 10` and `"description"` with example. An Architect enforces logic at the Contract level using Schema constraints, making the error impossible to generate.
