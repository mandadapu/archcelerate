...
objectives:
  - Understand the function calling loop (request → intent → execution → response)
...
┌─────────────────────────────────────────────────────────────┐
│ 1. USER REQUEST                                             │
│    "What's the weather in London and should I bring an      │
│     umbrella?"                                              │
└────────────────────────┬────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────┐
│ 2. LLM INTENT (Tool Call Request)                           │
│    {                                                        │
│      "tool": "get_weather",                                 │
│      "arguments": {                                         │
│        "location": "London",                                │
...
└────────────────────────┬────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. YOUR CODE EXECUTES (Not the LLM!)                        │
│    const result = await getWeather('London', 'celsius')     │
│    // Returns: { temp: 12, conditions: 'rainy',            │
│    //            humidity: 85 }                             │
└────────────────────────┬────────────────────────────────────┘
                         │
                         ▼
...
│    Tool result: { temp: 12, conditions: 'rainy',           │
│                   humidity: 85 }                            │
└────────────────────────┬────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────┐
│ 5. FINAL RESPONSE (LLM synthesizes answer)                  │
│    "It's 12°C and rainy in London with 85% humidity.       │
...
async function chat(userMessage: string): Promise<string> {
  ...
  while (response.stop_reason === 'tool_use') {
    ...
    for (const toolUse of toolUses) {
      console.log(`🔧 LLM requested tool: ${toolUse.name}`)
...
console.log(answer)
// "It's 12°C and rainy in London with 85% humidity. Yes, you should definitely bring an umbrella!"
```
...
**The Problem**: The sequential loop above processes tools one-by-one. If the LLM requests weather for 3 cities, you wait 300ms × 3 = 900ms total.
...
```typescript
// ❌ SEQUENTIAL: Slow (900ms for 3 cities)
for (const toolUse of toolUses) {
...
```typescript
// ✅ PARALLEL: Fast (300ms for 3 cities)
...
async function executeToolsInParallel(
  ...
  const toolPromises = toolUses.map(async (toolUse) => {
    try {
      ...
      console.log(`✅ ${toolUse.name} completed in ${duration}ms`)
...
async function chatWithParallelTools(userMessage: string): Promise<string> {
  ...
  while (response.stop_reason === 'tool_use') {
    ...
    console.log(`\n📊 Parallel execution: ${toolUses.length} tools`)
...
|----------|---------|---------|
| Sequential | 900ms | 3 × 300ms API calls |
...
- **Latency improvement**: 58% reduction (1,200ms → 500ms avg)
- **P99 improvement**: 72% reduction (3,500ms → 980ms)
...
**Why this works**:
- ✅ Specific name describes exactly what it does
- ✅ Description explains WHEN to use it
- ✅ Enums prevent hallucinated values
- ✅ Constraints (min/max) prevent invalid inputs
- ✅ Units are explicit (kg, not "weight")
...
```typescript
// ❌ BAD: A tool that does everything
{
...
2. **Parameter hallucination**: Too many combinations → model guesses wrong action
...
```typescript
// ✅ GOOD: High-granularity, atomic tools
const tools = [
  {
  ...
  },
  {
    ...
    description: 'Update the status of an existing order. Use after order ID is confirmed. Only allows status transitions (pending → shipped → delivered).',
    input_schema: {
      type: 'object',
      properties: {
        order_id: {
...
|--------|-----------------|--------------|
| **Security** | ❌ Broad permissions | ✅ Least privilege per tool |
| **Reliability** | ❌ 30% parameter errors | ✅ 5% parameter errors |
| **Testing** | ❌ 100+ test cases | ✅ 10 test cases per tool |
| **Debugging** | ❌ "manage_database failed" | ✅ "get_user_by_email failed" |
| **Access Control** | ❌ All-or-nothing API key | ✅ Tool-level API keys |
...
3. **Latency**: Longer responses → slower processing
4. **Cost**: 200K tokens × $0.003/1K = $6 per request
...
async function executeToolWithFiltering(
  ...
  try {
    ...
    console.log(`📊 Result size: ${JSON.stringify(rawResult).length} → ${JSON.stringify(filteredResult).length} chars`)
...
// Token reduction: 200,000 → 500 tokens (99.75% reduction)
// Cost reduction: $6.00 → $0.015 per request (400x cheaper)
```
...
- **Monthly cost savings**: $28,000 (from $30K → $2K)
- **Latency improvement**: 6.5x faster (7,800ms → 1,200ms)

#### Security Benefit: Preventing Data Leakage

```typescript
// ❌ DANGEROUS: Raw database response includes sensitive data
{
  name: 'alice@example.com',
  email: 'alice@example.com',
  password_hash: '$2b$10$...',  // ⚠️ Should never go to LLM
  api_keys: ['sk-live-abc123'],  // ⚠️ Security risk
  ssn: '123-45-6789',  // ⚠️ PII violation
  internal_id: 'usr_abc123xyz',  // ⚠️ Internal identifier
  ip_addresses: ['192.168.1.1']  // ⚠️ Privacy risk
}

// ✅ SAFE: Filtered response includes only what LLM needs
{
  name: 'alice@example.com',
  email: 'alice@example.com',
  tier: 'premium',
...
/**
 * Two-tier architecture: Fast router → Specialized handler
 */
class IntentRouter {
...
**Cost Savings**:
```typescript
// ❌ Without routing: Send all 50 tools every time
// 50 tools × 150 tokens each = 7,500 prompt tokens
// Cost: $0.023 per request (Sonnet input pricing)

// ✅ With routing: Send only 4-5 relevant tools
// Classification: 200 tokens (Haiku) = $0.0002
// Execution: 5 tools × 150 tokens = 750 prompt tokens (Sonnet) = $0.0023
...
**Why This Works**:
1. **Static sections are cacheable** (Anthropic's prompt caching) → 90% cost reduction on repeated calls
2. **Clear separation of concerns** → Easier to A/B test conversational tone vs tool behavior
3. **Dynamic context is small** → Only pay for what changes per request
4. **Reusable components** → Share tool-use prompt across multiple endpoints
...
interface WorkflowState {
  stage: WorkflowStage
  intent?: string
  resolvedEntities: Map<string, string>  // mention → entity_id
...
**Benefits**:
1. **Stage-specific tool sets** → LLM only sees relevant tools at each stage
2. **Clear failure points** → Know exactly where things went wrong
3. **Resumable workflows** → Can retry failed stages without restarting
4. **Cost optimization** → Use Haiku for classification/search, Sonnet for complex reasoning
5. **Testable** → Each stage can be unit tested independently
...
**What goes wrong**:
```typescript
// ❌ BAD: LLM loses order_id between calls
{
  tool_use: [
    {
      name: 'create_order',
  ...
  tool_use: [
    {
      ...
      parameters: { items: [...] }  // ❌ Missing order_id!
    }
  ]
}
```
    ...
    for (const requiredTool of dep.dependsOn) {
      if (!this.completedTools.has(requiredTool)) {
        throw new Error(
          ...
          `Required order: ${dep.dependsOn.join(' → ')} → ${toolName}`
        )
      }
    }
  ...
  while (true) {
    ...
    for (const block of response.content) {
      if (block.type === 'tool_use') {
        try {
          // ✅ Orchestrator automatically injects state and checks dependencies
...
|----------|-------------------|-----|
| Single independent tool call | ❌ No | Overkill for simple cases |
| 2-3 sequential tools (e.g., search → create) | ⚠️ Maybe | Consider if failures are common |
| 4+ sequential tools with ID passing | ✅ Yes | LLM will lose context |
| Parallel tools that merge results | ✅ Yes | Need to coordinate outputs |
| Long-running workflows (>5 turns) | ✅ Absolutely | Essential for reliability |
...
```typescript
// ✅ GOOD: Enum forces specific values
{
  priority: {
    type: 'string',
    enum: ['low', 'medium', 'high', 'urgent']
...
#### ❌ Option A: Prompt Engineering (Wrong)
...
#### ✅ Option B: Adapter Layer (Correct)

**Why it works**:
1. **Semantic translation**: Technical errors → business-meaningful responses
...
#### ❌ Option C: Upgrade Model (Wrong)

**Why it fails**:
- More expensive (Opus is 5x cost of Sonnet)
- Still non-deterministic (smarter ≠ perfect)
...
#### ❌ Option D: Acknowledge Limitation (Wrong)
...
|----------|-----------------|--------|
| API returns 404 for "no data" | ✅ Yes | Translate to semantic success (empty result) |
| API returns 500 for temporary outage | ✅ Yes | Translate to retry-able error |
| API returns 200 with `{"error": "..."}` | ✅ Yes | Normalize error format |
| API returns 100 columns when LLM needs 3 | ✅ Yes | Filter results (see Pattern 4) |
| API requires OAuth refresh | ✅ Yes | Handle auth transparently |
| API returns inconsistent field names | ✅ Yes | Normalize schema |
...
- [ ] Document the mapping (404 → $0 balance) for future maintainers
- [ ] Monitor adapter hit rate (how often translations happen)
- [ ] Alert on unexpected error codes (502, 504, etc.)

**Correct Answer**: **B — Implement an Adapter Layer** that catches the 404 and returns a structured message: `{ "balance": 0, "status": "active" }` so the LLM receives a factual success instead of a technical error.
...
|----------|-------------|-------------|------------------|
| Prompt: "Call get_weather()" | Low (parsing errors) | No | ❌ |
| Function calling | High (structured) | Yes (JSON Schema) | ✅ |
