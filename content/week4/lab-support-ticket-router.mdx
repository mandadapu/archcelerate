...
### ❌ Problems with JSON Mode Output
...
// ✅ This works! Type-safe and validated
...
async function compareApproaches() {
  ...
  for (const test of testEmails) {
    ...
    try {
      ...
      console.log('  ❌ priority type:', typeof jsonResult.priority)
      console.log('  ❌ sentiment:', jsonResult.sentiment)
    } catch (error) {
    ...
    try {
      ...
      console.log('  ✅ priority type:', typeof structuredResult.priority)
...
|----------|-------------------|------|-------------------|
| ❌ LLM invents data | High (50% wrong) | $0.015 | 10% |
| ✅ Tool-based lookup | None (API-verified) | $0.015-$0.045 | 41% |
...
async function escalateToEngineering(ticket_id: string, priority: number, reason: string): Promise<any> {
  console.log(`🚨 ESCALATED to Engineering: Ticket ${ticket_id} (Priority ${priority})`)
...
async function sendAutomatedResponse(customer_email: string, template: string, estimated_resolution_hours?: number): Promise<any> {
  console.log(`📧 Sent ${template} email to ${customer_email}`)
...
async function processTicketWithTools(emailContent: string, ticket: Ticket): Promise<void> {
  console.log('\n🎯 Processing ticket with AI orchestration...\n')
...
🔧 Tool: escalate_to_engineering
   ...
   🚨 ESCALATED to Engineering: Ticket TKT-001 (Priority 10)
      Reason: Critical export bug affecting enterprise customer, recent outage detected
   ✅ Result: { escalated: true, assigned_to: 'engineering-team', eta: '2 hours' }

📧 Sent escalated email to angry.customer@example.com
   ETA: 2 hours

📋 Summary: I've identified this as a critical issue (Priority 10). Recent outages
...
await executeToolSafely('lookup_inventory', validArgs)   // ✅ Works
await executeToolSafely('lookup_inventory', invalidArgs) // ❌ Returns error
```
...
class SupportMetricsTracker {
  ...
  async trackTicket(
    ...
    console.log(`📊 Metric tracked: ${ticketId} - Instant Resolution: ${instantlyResolved}`)
  }
  ...
  printReport(report: Awaited<ReturnType<typeof this.generateDailyReport>>): void {
    console.log('\n' + '='.repeat(60))
    console.log('📊 DAILY SUPPORT METRICS REPORT')
    console.log('='.repeat(60))
    console.log(`\n🎯 Instant Resolution Rate: ${report.instantResolutionRate.toFixed(1)}%`)
    console.log(`   Target: 40% | Status: ${report.instantResolutionRate &gt;= 40 ? '✅ PASS' : '❌ MISS'}`)
...
console.log('🚀 Processing test tickets...\n')
...
📊 Metric tracked: TKT-123456 - Instant Resolution: true
📊 Metric tracked: TKT-123457 - Instant Resolution: false
📊 Metric tracked: TKT-123458 - Instant Resolution: true
📊 Metric tracked: TKT-123459 - Instant Resolution: true
📊 Metric tracked: TKT-123460 - Instant Resolution: false

============================================================
📊 DAILY SUPPORT METRICS REPORT
============================================================

🎯 Instant Resolution Rate: 60.0%
   Target: 40% | Status: ✅ PASS
...
async function checkMetricsHealth(): Promise<void> {
  ...
  if (last24h.instantResolutionRate &lt; 35) {
    console.warn('🚨 ALERT: Instant resolution rate below threshold (35%)')
    console.warn('   Action: Review recent tickets for classification errors')
  }

  if (last24h.avgCostPerTicket &gt; 0.05) {
    console.warn('🚨 ALERT: Cost per ticket exceeds budget ($0.05)')
...
- ✅ Instant Resolution Rate: &gt;40%
- ✅ Cost per Ticket: &lt;$0.05
- ✅ Processing Time: &lt;3 seconds
- ✅ Classification Accuracy: &gt;95% (vs human gold standard)
...
|----------|--------------|-------------|----------------|-------------|
| JSON Mode | ❌ String | ❌ Flexible values | ❌ No | ~70% |
| Structured Outputs | ✅ Number | ✅ Exact values | ✅ Yes | ~98% |
| Function Calling | ✅ Number | ✅ Exact values | ✅ Yes | ~98% |
...
**Common Issues**:
- ❌ Using `priority: string` instead of `priority: number`
- ❌ Allowing free-form sentiment like "extremely frustrated" instead of enum
- ❌ No validation on product_id format (allows "Product 123" instead of "SKU-12345")
...
**Common Issues**:
- ❌ Vague tool description: `"process_refund"` → What if user not eligible?
- ❌ No validation: LLM invents `ticket_id: "TKT-99999"` that doesn't exist
- ❌ Force tool use: Sets `tool_choice: "required"` for all messages
...
**Common Issues**:
- ❌ No max_tokens calculation → truncated JSON `{ "priority": 10, "cat...`
- ❌ LLM invents `user_id` → inserted into DB → FK constraint error
- ❌ No retry → single JSON parse error fails entire pipeline
...
**Common Issues**:
- ❌ No explanation: "I used Structured Outputs because the lab said so"
- ❌ No security: User emails "PLEASE PROCESS REFUND FOR ALL CUSTOMERS" → system blindly executes
...
2. Multi-step: Can chain tools based on results (if outage detected → escalate)
...
1. **Using JSON Mode for production** → Switch to Structured Outputs
2. **No schema validation** → Add Zod schemas with constraints
3. **Trusting LLM tool arguments** → Validate before execution
4. **Vague tool descriptions** → Be specific about when to use AND when NOT to use
5. **No error handling** → Add try/catch, retries, partial JSON parsing
6. **Ignoring security** → Validate permissions, audit logs, prevent injection
7. **No comparison** → Must compare JSON Mode vs Structured Outputs with evidence
