...
```
Code → Commit → PR → Review → Merge → Deploy
  ↓       ↓      ↓      ↓       ↓       ↓
...
for FILE in $STAGED_FILES; do
  ...
  if echo "$ISSUES" | grep -q "BLOCKER"; then
    echo "❌ Blocking issues found in $FILE"
    echo "$ISSUES"
    exit 1
  fi
done
      ...
      blocks: [
        {
          ...
          text: {
            type: 'mrkdwn',
            text: `🚨 *Blocking Issues Found in PR #${review.prNumber}*`
          }
        },
        {
          type: 'section',
