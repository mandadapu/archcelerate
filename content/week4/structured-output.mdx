...
## 🎯 Real-World Challenge: The Enterprise Support Orchestrator
...
- **Zero Hallucinations**: Cannot invent ticket IDs or priority levels—database expects exact enums

**The Architectural Problem**: LLMs return **text**, but your support system needs **typed data**:

```typescript
// ❌ What you get from raw LLM
...
|----------|-------------------|-------------------|-------------------|------------------|
| **JSON Mode** | ❌ Prompt-based (unreliable) | ⚠️ High (invents "very high" priority) | ✅ Required | ⚠️ Risky |
| **Structured Outputs** | ✅ API-level (guaranteed) | ✅ None (rejects invalid values) | ❌ Not needed | ✅ Yes |
...
{ "priority": "very urgent" }  // ❌ Database expects 1-10, gets string

// Structured Outputs (API enforcement)
{ "priority": 10 }  // ✅ LLM forced to use valid integer
```

**[👉 Lab: Build the Support Ticket Router](/curriculum/week-4/labs/support-ticket-router)**
...
```typescript
// ❌ LLM invents subscription tier when it doesn't know
{
  "category": "billing",
  "priority": "high",
...
**Why this works**:
- ✅ LLM only calls expensive Zendesk API when genuinely needed (50% of cases)
- ✅ Zero hallucinations (subscription data comes from real API)
- ✅ Structured output ensures database compatibility
- ✅ 40%+ instant resolution (no human handoff)
- ✅ Cost-optimized ($0.015 for simple tickets, $0.045 for complex)

**Alternative Architecture (Not Recommended)**:
```typescript
// ❌ BAD: Always query Zendesk first
...
```typescript
// ✅ GOOD: Let LLM decide if it needs external data
const response = await anthropic.messages.create({
  tools: [queryZendeskTool, classifyTicketTool],
...
```typescript
// ❌ What tutorials teach (unusable in production)
const response = await anthropic.messages.create({
  model: 'claude-3-5-sonnet-20240620',
  max_tokens: 1024,
  messages: [{
...
// Problem: How do you extract 62 and "partly cloudy" reliably? String parsing? Regex? 😱
```
...
### Level 3: Structured Output (Production-ready) ✅
...
- ✅ Direct integration with APIs/databases
...
```typescript
// ✅ Structured Outputs: Grammar enforcement at decoding
const response = await anthropic.messages.create({
  model: 'claude-3-5-sonnet-20240620',
  tools: [{
    name: 'extract_data',
...
LLM sampling: [vocabulary of 50K tokens]
  ↓
Token 1: "The" (probability: 0.15) ← Conversational fluff
Token 2: "category" (probability: 0.12)
Token 3: "is" (probability: 0.08)
Token 4: "billing" (probability: 0.25)
  ↓
Output: "The category is billing" ← String, not JSON!
```
...
LLM sampling: [vocabulary filtered by grammar]
  ↓
Token 1: Must be "{" (start of JSON object) → probability: 1.0
Token 2: Must be "category" (only valid key) → probability: 1.0
Token 3: Must be ":" (JSON syntax) → probability: 1.0
Token 4: Must be one of ["billing", "technical", "feature"] → pick highest prob
Token 5: Must be "}" (close object) → probability: 1.0
  ↓
Output: {"category": "billing"} ← Guaranteed valid!
```
...
// Success rate: ~97-98%
// Failure modes:
// - "Sure! Here's the data: {...}" ← conversational wrapper
// - { "priority": "very high" } ← string instead of integer
...
const response = await openai.chat.completions.create({
  ...
  response_format: {
    ...
    json_schema: {
      ...
      strict: true,  // ← Enables grammar constraints
      schema: {
        type: 'object',
        properties: {
...
| **Retry API Costs** | $750/month (2,500 retries × $0.003) | $0 |
...
```typescript
// ❌ Expensive: Send all 20 tools every time
const response = await anthropic.messages.create({
  tools: allTools, // 20 tools * 200 tokens each = 4,000 prompt tokens wasted
  messages: [...]
})

// ✅ Optimized: Send only relevant tools
...
```typescript
// ❌ BAD: Assume parsing always works
const data = JSON.parse(response.content[0].text)
```
...
// ❌ LLM invents user_id that doesn't exist
{
  "tool": "process_refund",
  "arguments": {
...
// Your system tries to refund non-existent user → failure
```
...
// ✅ GOOD: Let LLM admit it doesn't know
{
  name: 'process_refund',
  input_schema: {
    properties: {
...
// ❌ LLM's tool definition (outdated)
{
  name: 'create_order',
  input_schema: {
    properties: {
...
// Both LLM and API use same spec → no drift!
```
...
**Architect's Principle**: Your LLM output schema is an **API Contract**. If you change a `priority` field from a string to an integer, you must version your schema and provide migration scripts—just like you would for a REST API.
...
// ❌ Week 4: You realize strings are inefficient
const TicketSchema = {
  type: 'object',
  properties: {
...
// 🚨 PROBLEM: Analytics dashboard expects old schema
...
3. **Testing Migrations**
   ...
   describe('Schema migrations', () => {
     it('should round-trip v1 → v2 → v1 without data loss', () => {
...
- [ ] Write migration function (old → new)
- [ ] Write reverse migration (new → old) for rollback
...
```
┌──────────────────────────────────────────────────────┐
│ Layer 1: PREVENTIVE (Before LLM call)                │
│ - Generous max_tokens buffer                         │
│ - Clear schema in system prompt                      │
│ - Few-shot examples of correct format                │
└──────────────────┬───────────────────────────────────┘
                   │ LLM generates output
                   ▼
┌──────────────────────────────────────────────────────┐
│ Layer 2: REACTIVE (Parse & Validate)                 │
│ - Try JSON.parse()                                   │
│ - If success, validate against Zod schema            │
│ - If fail, proceed to Layer 3                        │
└──────────────────┬───────────────────────────────────┘
                   │ Validation failed
                   ▼
┌──────────────────────────────────────────────────────┐
│ Layer 3: REPAIR (Auto-fix & Retry)                   │
│ - JSON repair (add missing brackets)                 │
│ - Regex cleanup (remove conversational fluff)        │
│ - Feedback loop (send error back to LLM)             │
└──────────────────────────────────────────────────────┘
```
...
class SelfHealingExtractor<T> {
        ...
        try {
          ...
          if (repaired) {
            try {
              ...
              console.log(`✅ Successfully repaired JSON on attempt ${attempts}`)
  ...
  /**
   * Build error feedback for LLM (validation error → retry with hints)
   */
...
async function extractWithFeedback<T>(
  ...
  for (let turn = 1; turn &lt;= maxTurns; turn++) {
    ...
    try {
      ...
      console.log(`✅ Success on turn ${turn}`)
      return validated
    } catch (error) {
...
**When to Alert**:
- Direct success &lt; 90% → Schema/prompt needs improvement
- Repair success &gt; 10% → Underlying issue with LLM behavior
- Total failure &gt; 2% → Production reliability at risk
...
```typescript
// ❌ INFINITE RETRY LOOP (Production Killer)
let attempts = 0
...
```
┌─────────────────────────────────────────────────────────┐
│ Tier 1: Primary LLM (Sonnet, temp=0.7)                  │
│ Creative, flexible, handles complex cases                │
└───────────────────┬─────────────────────────────────────┘
                    │ Validation failed (1st attempt)
                    ▼
┌─────────────────────────────────────────────────────────┐
│ Tier 2: Constrained LLM (Haiku, temp=0.0)               │
│ Strict, deterministic, focused on format correction     │
└───────────────────┬─────────────────────────────────────┘
                    │ Validation failed (2nd attempt)
                    ▼
┌─────────────────────────────────────────────────────────┐
│ Tier 3: Format-Fixer Microservice (Deterministic)       │
│ Coerces any input into valid schema format              │
└───────────────────┬─────────────────────────────────────┘
                    │ Extreme edge case
                    ▼
┌─────────────────────────────────────────────────────────┐
│ Tier 4: Hard-Coded Defaults (Guaranteed Valid)          │
│ Safe defaults for every field, never fails              │
└─────────────────────────────────────────────────────────┘
```
...
async function extractWithFullFallback<T>(
  ...
  try {
    ...
    console.log('✅ Tier 2 (Constrained LLM) succeeded')
  ...
  try {
    ...
    console.log('✅ Tier 3 (Format-Fixer) succeeded')
  ...
  console.log('⚠️ Tier 4 (Hard-Coded Defaults) used')
...
- ✅ Ambiguous input needing interpretation
- ✅ Multi-field dependencies

**Tier 2 (Constrained LLM)**:
- ✅ Format correction (dates, emails, phone numbers)
- ✅ Simple JSON structure fixes
- ✅ Fast fallback (&lt;500ms)

**Tier 3 (Format-Fixer)**:
- ✅ Deterministic coercion required
- ✅ Known input patterns (date formats)
- ✅ Type conversions (string → number)

**Tier 4 (Hard-Coded Defaults)**:
- ✅ Graceful degradation for rare edge cases
- ✅ Partial extraction (some fields valid, others default)
...
**Option B: Use Zod/Pydantic Schema with Regex Validation + Auto-Retry Loop** ✅ **CORRECT**
...
// → { customer_id: 'CUST-5678', category: 'billing', priority: 9 }
...
- **95% success rate** on retry (from initial 85% to 95% × 0.85 = ~99% total)
...
- Cost increase: $15K/month → $75K/month
- Still requires manual intervention for failures
...
class ProductionTicketExtractor {
  ...
  private async escalateToHuman(
    ...
    await slack.send({
      channel: '#ops-alerts',
      text: `🚨 Ticket extraction failed after ${this.maxAttempts} attempts. Manual review required.`
    })
  }
}
```
...
// Alert thresholds:
// - First-try success < 80% → Review system prompt
// - Total failure rate > 1% → Escalate to engineering
// - Specific error > 5% of total → Add explicit validation rule
```
...
1. **Prompts are suggestions, schemas are contracts**
   - LLMs are probabilistic → 85-95% adherence
   - Code validation is deterministic → 100% enforcement
...
|----------|------------------|----------|
| Plain text | ❌ No | Demos, prototypes |
| JSON mode (prompting) | ⚠️ Risky | Quick experiments |
| Structured output + validation | ✅ Yes | Production systems |
