...
class CheckpointedAgent {
  ...
  async loadLatestCheckpoint(): Promise<{
    ...
    console.log(`📂 Loaded checkpoint: Step ${checkpoint.stepNumber} (${checkpoint.stepName})`)
  ...
  async resume(): Promise<AgentState> {
    ...
    console.log(`🔄 Resuming from step ${checkpoint.stepNumber}...`)
  ...
  async execute(userQuery: string): Promise<AgentState> {
    ...
    if (existingThread && existingThread.status === 'running') {
      console.log('🔄 Existing thread found, resuming...')
  ...
  private async executeFrom(state: AgentState, fromStep: number): Promise<AgentState> {
    ...
    for (let i = fromStep; i < steps.length; i++) {
      const step = steps[i]
      console.log(`\n⚙️  Step ${i + 1}/${steps.length}: ${step.name}`)
      ...
      if (Math.random() &lt; 0.1 && i < steps.length - 1) {
        console.log('\n💥 SIMULATED CRASH - Agent stopped mid-execution')
...
try {
  ...
  console.log('✅ Completed:', result.finalOutput)
} catch (error) {
  console.log('❌ Agent crashed, but state is saved')
}

// Later: Resume from checkpoint
const agent2 = new CheckpointedAgent('thread_123', 'user_abc')
const resumedResult = await agent2.resume()
console.log('✅ Resumed and completed:', resumedResult.finalOutput)

/* Example Output:

⚙️  Step 1/5: plan
✅ Checkpoint saved: Step 1 (plan)

⚙️  Step 2/5: research
✅ Checkpoint saved: Step 2 (research)

💥 SIMULATED CRASH - Agent stopped mid-execution

--- Server restarts ---

🔄 Existing thread found, resuming...
📂 Loaded checkpoint: Step 2 (research)
🔄 Resuming from step 2...

⚙️  Step 3/5: analyze
✅ Checkpoint saved: Step 3 (analyze)

⚙️  Step 4/5: write
✅ Checkpoint saved: Step 4 (write)

⚙️  Step 5/5: finalize
✅ Checkpoint saved: Step 5 (finalize)
...
|-----------------------|--------------------|
| Agent crashes → restart from scratch | Agent crashes → resume from last checkpoint |
| User logs out → lose progress | User logs out → state persisted, resume anytime |
| 30-minute workflow crashes at 29min → waste 29min | Crash at 29min → resume from 25min checkpoint |
...
class TimeTravelDebugger {
  ...
  async replayFrom(
    ...
    console.log(`⏪ Time-traveling to step ${stepNumber} (${checkpoint.stepName})`)
    ...
    console.log('🔁 Replaying execution with modified state...')
...
⏪ Time-traveling to step 2 (research)
🔁 Replaying execution with modified state...
⚙️  Step 3/5: analyze
✅ Checkpoint saved: Step 3 (analyze)
⚙️  Step 4/5: write
✅ Checkpoint saved: Step 4 (write)
...
> **The Gap**: Basic time-travel lets you inspect checkpoints, but in production, you need to **edit agent memory** at a specific point and **replay from there**. An agent might fail at Step 10 because of a hallucination at Step 3—you need to fork the state, fix Step 3, and replay Steps 4-10.
...
Step 1: Load contract PDF ✅
Step 2: Extract clauses ✅
Step 3: Identify liability terms ❌ HALLUCINATION: Missed critical indemnity clause
Step 4: Analyze risk (based on incomplete Step 3) ❌ WRONG
Step 5: Generate summary (based on wrong Step 4) ❌ WRONG
...
- Re-execution cost: 60 minutes × $0.50 (LLM) = $0.50 per mistake
...
class StateForkingDebugger extends TimeTravelDebugger {
  ...
  async createFork(
    ...
    console.log(`\n🍴 Creating state fork from Step ${forkPoint}`)
...
```typescript
// Agent execution:
Step 1: Validate order ✅ (Checkpoint saved)
Step 2: Check inventory ✅ (Checkpoint saved)
Step 3: Charge credit card ✅ (Tool executes: $500 charged)
💥 CRASH (before checkpoint saved)

// Resume from Step 2 checkpoint:
Step 3: Charge credit card ❌ RUNS AGAIN → Double charge ($1,000 total)
```

**Production impact**:
- E-commerce platform: 3% crash rate during payment processing
- Volume: 10K orders/month
- Double charges: 300 orders/month × $200 avg = $60K/month
- Customer disputes: 300 × $25 chargeback fee = $7,500/month
...
class IdempotentToolRegistry {
  ...
  async executeIdempotent(
    ...
    if (cached) {
      console.log(`♻️  Idempotent replay: ${toolName} (request_id: ${requestId})`)
    ...
    console.log(`🔧 Executing tool: ${toolName} (request_id: ${requestId})`)
    const startTime = Date.now()
    ...
    try {
      ...
      console.log(`✅ Tool executed successfully (${latency}ms)`)
...
async function testIdempotentCrashRecovery() {
  ...
  try {
    ...
    console.log('\n💥 Agent crashed after tool execution but before checkpoint')
  }
  ...
  const resumeResult = await agent.executeToolSafely(
    'charge_credit_card',
    { orderId: 'order_789', amount: 500 },
    3  // Same step number → same request_id
  )

  console.log(`\n✅ Idempotency worked: ${resumeResult.wasReplay ? 'Cached result returned' : 'NEW CHARGE (BUG!)'}`)
}

/* Example Output:

--- First execution ---
🔧 Executing tool: charge_credit_card (request_id: thread_payment_123_step_3_charge_credit_card)
💳 Charging $500 for order order_789
✅ Tool executed successfully (42ms)
Payment result: {"transactionId":"txn_1738790123456"}

💥 Agent crashed after tool execution but before checkpoint

--- Resume from checkpoint ---
♻️  Idempotent replay: charge_credit_card (request_id: thread_payment_123_step_3_charge_credit_card)
   Original execution: 2025-02-05T18:23:45.123Z
   Returning cached result (no side effects)

✅ Idempotency worked: Cached result returned
*/
```
...
- Double charges: 300 × $200 avg = $60K/month
- Chargeback fees: 300 × $25 = $7,500/month
- Customer support: 300 × $50 = $15K/month
- **Annual cost**: $990K
...
4. Log successful executions (NOT failures—allow retry)
...
❌ **MUST use idempotency keys** (side effects):
...
48:01 - 💥 Cloud provider outage kills server
...
50:03 - ❌ DUPLICATE COMMENT posted to legal database
```
...
if (cached) {
  console.log('♻️  Tool already executed, returning cached result')
...
5. **Idempotent behavior**: Returns cached result, NO duplicate post ✅
...
**Why**: This misunderstands the architecture. The agent doesn't "remember" via LLM memory—it loads state from the database checkpoint. The checkpoint at 45:00 shows Step 9 completed, Step 10 pending. The agent correctly determines it should execute Step 10. The issue isn't memory—it's that the tool execution (48:00) and the checkpoint confirming it (48:01, never happened) are not atomic.
...
- 98% storage reduction (10.1MB → 213.5KB per 100-turn thread)
...
- ✅ State Checkpointing (database-backed persistence)
- ✅ Time-Travel Debugging (inspect any historical checkpoint)
- ✅ State Forking (HITL intervention, edit & replay)
- ✅ Delta Checkpointing (incremental snapshots, 98% storage savings)
- ✅ Tool Idempotency (exactly-once execution, no duplicates)
- ✅ Monitoring (checkpoint frequency, resume latency, replay rate, idempotent cache hits)

**Cost & Reliability Analysis**:
```typescript
// No persistence
- Crash at Step 9/10 → Re-run all 10 steps
- Cost: $0.30 + $0.30 (rerun) = $0.60
- Double executions: 3% crash rate × 10K workflows = 300 incidents/month
- Double-charge cost: 300 × $200 = $60K/month

// With checkpointing only (no idempotency)
- Crash at Step 9 → Resume from Step 8
...
- Crash at Step 9 → Resume from Step 8, idempotent tool returns cached result
