...
class OptimizedCheckpointedAgent extends CheckpointedAgent {
  ...
  async saveCheckpoint(
    ...
    if (stepNumber % 5 === 0 || state.finalOutput !== null) {
      ...
      console.log(`💾 Synced to Postgres: Step ${stepNumber}`)
    } else {
      console.log(`⚡ Cached in Redis: Step ${stepNumber}`)
    }
  }
  ...
  async loadLatestCheckpoint() {
    ...
    if (cached) {
      ...
      console.log(`⚡ Loaded from Redis: Step ${stepNumber}`)
      return { state, stepNumber, stepName: `step_${stepNumber}` }
    }

    console.log('📂 Redis miss, loading from Postgres...')
...
- 10 steps = (9 × 2ms) + (2 × 30ms) = 78ms total overhead
...
```typescript
// No checkpointing
- Agent crashes at step 9 of 10 → re-run all 10 steps
- LLM cost: 10 steps × $0.03 = $0.30
- Re-execution after crash: $0.30 again = $0.60 total

// With checkpointing
- Agent crashes at step 9 → resume from step 8 checkpoint
- LLM cost: 10 steps × $0.03 = $0.30
- Re-execution after crash: 2 steps × $0.03 = $0.06
- Database cost: 10 checkpoints × $0.0001 = $0.001
- Total: $0.30 + $0.06 + $0.001 = $0.361

// Savings on crash: $0.60 - $0.361 = $0.239 (39.8% reduction)
// With 5% crash rate: $0.60 → $0.372 (38% cost reduction)
```
