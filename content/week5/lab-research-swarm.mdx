...
**Success Criteria**:
- ✅ Find and extract 10-15 relevant papers from PubMed/ArXiv
- ✅ Validate statistical methodology (p-values, sample sizes, bias detection)
- ✅ Generate executive briefing citing only approved papers
- ✅ Zero hallucinated citations (all DOIs verified)
- ✅ 100% supervisor approval before HITL review
...
export class SearcherAgent {
  async search(
    ...
    console.log(`🔍 Searcher Agent: Searching for "${query}"`)
    ...
    console.log(`   ✅ Found ${papers.length} papers in ${(searchTime / 1000).toFixed(1)}s`)
...
Output JSON:
{
  "terms": "(immunotherapy[MeSH] OR immune checkpoint inhibitors) AND (CD47 OR SIRPα) AND neoplasms[MeSH]",
  "filters": [
    ...
    */
    ...
    return [
      ...
      },
      {
        ...
        abstract: 'Dual immune checkpoint blockade targeting CD47-SIRPα and PD-1/PD-L1 pathways represents a rational therapeutic strategy. We evaluated AO-176 (anti-CD47) plus atezolizumab (anti-PD-L1) in treatment-naive advanced NSCLC (N=156). Primary endpoint: ORR by RECIST 1.1. Results: ORR 58% for combination vs 31% for atezolizumab monotherapy (p&lt;0.001). Median PFS 10.2 vs 5.6 months (HR=0.52, p=0.003). Response rates were higher in PD-L1 high expressors. Grade ≥3 immune-related adverse events: 28% combination vs 18% monotherapy. Mechanistic studies revealed enhanced tumor-infiltrating lymphocyte activation and reduced myeloid-derived suppressor cells in combination arm.',
...
If data not found, omit that field. ONLY return JSON.`
      ...
      if (jsonMatch) {
        try {
          ...
          console.warn(`⚠️ Failed to parse metadata for ${result.pmid}`)
        }
      }
...
**Expected Output**:
```
✓ finds relevant papers for oncology query (3842ms)
✓ extracts sample sizes and p-values (2156ms)
✓ verifies all papers have DOIs (1892ms)
✓ completes search within 30 seconds (4234ms)
...
export class CriticAgent {
  async critique(papers: Paper[]): Promise<CritiqueResult> {
    console.log(`\n📊 Critic Agent: Reviewing ${papers.length} papers`)
    ...
    for (const paper of papers) {
      ...
      console.log(`   → ${critique.recommendation.toUpperCase()} (Score: ${critique.methodologyScore}/10)`)
    }
...
- **include**: Score ≥ 7, no major flaws, results credible
...
export class WriterAgent {
  async synthesize(
    ...
    console.log(`\n✍️ Writer Agent: Synthesizing briefing from ${papers.length} papers`)
    ...
    console.log(`   ✅ Briefing generated: ${wordCount} words, ${approvedPapers.length} citations`)
...
- Example: "CD47-SIRPα axis: 3 Phase II trials show enhanced anti-tumor immunity"
...
export class SupervisorAgent {
  ...
  async run(query: string): Promise<WorkflowResult> {
    ...
    console.log('🚀 SUPERVISOR: Starting research workflow')
    ...
    console.log(`\n👔 SUPERVISOR REVIEW: ${supervisorReview.quality}`)
...
Rules:
- If ANY hallucinated citations → needs_revision
- If avg methodology score < 7 → requiresHumanApproval = true
- If < 3 papers cited → needs_revision (insufficient coverage)
- If ≥ 5 papers cited AND no issues → approved`
  ...
  private async humanApprovalGate(
    ...
    console.log(`\n🧑‍⚕️ Chief Scientist Decision: ${approved ? '✅ APPROVED' : '❌ REJECTED'}\n`)
...
async function main() {
  ...
  console.log(`Papers: ${result.papers.length} found → ${result.briefing.papersCited} cited`)
...
1. **Supervisor Pattern for Sequential Dependencies**: Research requires Searcher → Critic → Writer flow
...
- Cost: $125K/year → $8.5K/year (93% reduction)
...
🎉 **Lab Complete!** You've built a production-grade multi-agent research system.
