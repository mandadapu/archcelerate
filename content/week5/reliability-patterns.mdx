...
async function reflectiveAgent(
  ...
  console.log('🤖 Generating initial output...')
...
${criteria.map((c, i) => `${i + 1}. ${c.dimension}: ${c.question} (Score 0-10, must score ≥${c.threshold})`).join('\n')}
...
Output JSON:
{
  ...
  "overallQuality": "approved" or "needs_revision"
    ...
    if (attempt &gt;= maxRevisions) {
      console.log('❌ Max revisions reached, escalating to human review')
      result.finalQuality = 'needs_human_review'
      break
    }

    // Step 3: Revise based on critique
    console.log('⚙️  Revising output based on critique...')
...
🔍 Reflection attempt 1...
  - Accuracy: 9/10 ✅
  - Completeness: 5/10 ❌ (missing storage layer explanation)
  - Clarity: 8/10 ✅
⚙️  Revising output based on critique...
...
> **The Gap**: When an agent critiques its own work, it suffers from **self-confirmation bias**—it's blind to its own hallucinations. If Claude generates a hallucinated fact, Claude's critique will often accept it as valid because both use the same reasoning pattern.
...
// ❌ HALLUCINATION: Paper shows 78% in Phase II, Phase III not yet complete
...
// ❌ SELF-CONFIRMATION: Same model fails to catch own hallucination
```
...
Critic Model (GPT-4o or specialized reward model)
   ...
   [Critique with different reasoning architecture]
         ↓
If critique fails → Generator revises (now aware of blind spot)
```
...
async function crossModelReflection(
  ...
  while (attempt < maxRevisions) {
    attempt++
    console.log(`\n🔄 Cross-Model Reflection - Attempt ${attempt}`)

    // Generate with Claude
    console.log('  🤖 Generator (Claude 3.5 Sonnet)...')
    ...
    if (critique.hallucinations.length > 0) {
      console.log(`     🚨 Hallucinations Found: ${critique.hallucinations.length}`)
    ...
    if (passed) {
      console.log('  ✅ Cross-model critique PASSED')
      return {
    ...
    console.log('  ⚙️  Revising based on cross-model critique...')
  }
...
- Manual review cost: 360 × $75/hour × 30min = $13,500/month
...
- Manual review cost: 48 × $75/hour × 30min = $1,800/month
- Additional LLM cost: 5K × $0.03 (GPT-4o critique) = $150/month
...
|----------|--------------|---------------------|
| Low-stakes content (blog posts) | ✅ Sufficient | ⚠️ Overkill |
| Factual content (news, research) | ❌ Risky | ✅ Required |
| High-stakes (medical, legal) | ❌ Dangerous | ✅ Mandatory |
| Cost-sensitive (&lt;$0.10 budget) | ✅ Acceptable | ❌ Too expensive |
| Liability-sensitive (&gt;$1M risk) | ❌ Insufficient | ✅ Essential |
...
class HITLGate {
  ...
  async requestApproval(
    ...
    console.log(`\n⏸️  HITL: Agent ${agentId} requesting approval for ${action.type}`)
    ...
    return new Promise((resolve) => {
      const checkInterval = setInterval(() => {
        ...
        if (Date.now() - startTime > this.config.timeout) {
          ...
          clearInterval(checkInterval)
          console.log('⏱️  HITL timeout: Auto-rejecting action')
          resolve('timeout')
          return
        }
        ...
        if (request.status === 'approved') {
          clearInterval(checkInterval)
          console.log('✅ HITL: Human approved action')
          resolve('approved')
        } else if (request.status === 'rejected') {
          clearInterval(checkInterval)
          console.log(`❌ HITL: Human rejected action. Reason: ${request.rejectionReason}`)
          resolve('rejected')
        }
      }, 1000)
    })
...
async function agentWithHITL(userRequest: string): Promise<string> {
  ...
  if (hitlConfig.requireApproval(action)) {
    ...
    console.log('🚀 Executing low-stakes action without approval...')
...
⏸️  HITL: Agent agent_cleanup_001 requesting approval for database_write
   Description: Delete 1,500 user records older than 2 years (GDPR compliance)
...
|-------------|-------------|------|-----------------|
| Database read | N/A | Free | ❌ No |
| Send email to 1 user | Yes (send apology) | $0.01 | ❌ No (agent can proceed) |
| Send email to 10K users | No (reputation damage) | $100 | ✅ Yes (HITL required) |
| Approve $50 refund | Yes (can reverse transaction) | $50 | ❌ No (set threshold at $100) |
| Approve $500 refund | Partially (dispute risk) | $500 | ✅ Yes (above threshold) |
| Delete user data | No (GDPR violation if wrong) | $10K+ fine | ✅ Yes (irreversible) |
| Deploy code to production | Yes (can rollback) | $0 | ✅ Yes (high stakes) |
...
Calculate risk score = f(confidence, disagreement, impact)
         ↓
Risk score > threshold? → HITL
Risk score ≤ threshold? → Auto-proceed
```
...
async function uncertaintyTriggeredHITL(
  ...
  console.log('🤖 Generating decision with confidence tracking...')
  ...
  console.log(escalate ? '⏸️  ESCALATING to human' : '✅ Auto-proceeding')
...
- Human review cost: 3,500 × $15 = $52,500/month
- Error rate (auto-approved): 8% on &lt;$1K claims
- Error cost: 800 × $500 avg = $400K/month errors
...
- Human review cost: 1,800 × $15 = $27,000/month
- Error rate: 1.2% (85% reduction, caught by uncertainty detection)
- Error cost: 120 × $500 avg = $60K/month
- Additional LLM cost: 10K × $0.02 (cross-model check) = $200/month
...
function logprobsToConfidence(logprob: number): number {
  ...
  // logprob = -0.01 → ~99% confidence
  // logprob = -0.5 → ~60% confidence
  // logprob = -2.0 → ~13% confidence
...
async function nVersionConsensus<T>(
  ...
  console.log(`🔄 Running ${n}-version consensus...`)

  // Run N agents in parallel
  const promises = Array.from({ length: n }, async (_, i) => {
    console.log(`  🤖 Agent ${i + 1} starting...`)
    ...
    console.log(`  ✅ Agent ${i + 1} completed`)
    return parsedOutput
  })
  ...
  if (maxCount === n) {
    ...
    console.log(`\n✅ UNANIMOUS CONSENSUS (${n}/${n} agents agree)`)
    ...
    console.log(`\n⚖️  MAJORITY CONSENSUS (${maxCount}/${n} agents agree)`)
...
${patientNote}`
  ...
  if (result.consensus === 'unanimous') {
    console.log('✅ All agents agree - safe to proceed')
    return result.finalOutput
  } else if (result.consensus === 'majority') {
    console.log('⚠️  Majority consensus - proceeding with majority output')
    console.log(`   Discarded minority outputs: ${result.minorityOutputs.length}`)
    return result.finalOutput
  } else {
    console.log('🚨 NO CONSENSUS - escalating to human pharmacist')
...
- Review cost: 10K × $25 = $250K/month
- **Annual cost**: $3M in human review

**After 3-Version Consensus**:
- Unanimous agreement: 82% (8,200 cases) → Auto-proceed
- Majority consensus: 13% (1,300 cases) → Auto-proceed with flag
- No consensus: 5% (500 cases) → Human review required
- Accuracy on unanimous: 99.7% (near-perfect)
- Accuracy on majority: 96% (acceptable)
- Manual review: 5% (500 cases/month)
- Review cost: 500 × $25 = $12,500/month
- LLM cost: 10K × 3 agents × $0.02 = $600/month
- **Annual cost**: $150K (review) + $7.2K (LLM) = $157.2K
- **Savings**: $2.84M/year (95% reduction)

**Critical insight**: Unanimous consensus (82% of cases) has 99.7% accuracy → safe to automate
...
|----------|--------------|---------------------|
| Low-stakes (blog summaries) | ✅ Sufficient | ❌ Overkill (3x cost) |
| Critical data (medical, legal) | ❌ Too risky | ✅ Required |
| High-volume (&gt;10K/month) | ⚠️ Maybe | ✅ Yes (can automate 80%) |
| 100% review currently | ❌ Expensive | ✅ Reduces review 95% |
| Regulatory compliance | ❌ Insufficient | ✅ Meets audit standards |
...
> "This pattern comes from avionics and medical devices, where a single bug can kill people. Run three independent flight control systems; if one disagrees, it's outvoted. Apply the same principle to AI: run three agents in parallel. If all three hallucinate the same fact, you're unlucky—but statistically unlikely. If one hallucinates and two don't, majority rules. If all three disagree, you have **systemic uncertainty**—escalate to human. This is the gold standard for mission-critical AI."

**Implementation checklist**:
- ✅ Use N=3 (sweet spot: cost vs reliability)
- ✅ Run agents in parallel (not sequential)
- ✅ Use moderate temperature (0.5) for diversity
- ✅ Auto-proceed on unanimous (99.7% accuracy)
- ✅ Auto-proceed on majority (96% accuracy, flag for audit)
- ✅ Escalate on no consensus (5% of cases)
- ✅ Log all minority outputs for post-hoc analysis
...
- Monthly error cost: 800 × $2,500 = **$2M/month**
...
- New monthly error cost: 200 × $2,500 = **$500K/month**
...
**Reasoning**: If one reflection loop improves accuracy from 92% → 98%, a second loop should get us closer to 100%. Stack two reflection loops in series.

**Estimated impact**:
- Second reflection cost: +$0.05 (total $0.10/claim)
- Accuracy improvement: 98% → 99.2% (diminishing returns)
- Error rate: 0.8% (80 claims/month)
- Error cost: 80 × $2,500 = $200K/month

**Verdict**: ⚠️ **WRONG APPROACH**

**Why**: Diminishing returns. The second loop only catches 1.2% more errors (120 claims) but costs $500/month (10K × $0.05). You're spending $500K to save $300K. Plus, you **still haven't eliminated the liability risk**—you still have 0.8% errors on high-value claims.

**Architect's mistake**: Trying to solve a **risk elimination problem** with a **probabilistic improvement**. Reflection loops are statistical—they reduce errors but can't guarantee zero.
...
**Cost Analysis**:
- Reflection cost: 10K × $0.05 = $500/month
...
**Verdict**: ❌ **CATASTROPHICALLY WRONG (in regulated industries)**
...
- Monthly LLM savings: 10K × $0.025 = $250/month
- Error cost: Still $500K/month
- **Net**: Save $250, still lose $500K

**Verdict**: ❌ **WRONG**

**Why**: You're trading **$250/month in LLM savings** for **$500K/month in error costs**. This is penny-wise, pound-foolish. Worse, Haiku likely has **lower accuracy** than Sonnet, so error rate might increase from 2% → 4%, doubling the loss to $1M/month.
...
3. **Composite Architecture**:
   ```
   All claims → Reflection Loop (98% accuracy)
                       ↓
   High-risk claims → HITL Gate (100% review)
                       ↓
   Low-risk claims → Auto-approve (acceptable error rate)
   ```
...
- 5x improvement in hallucination detection (40% → 92%)
...
You **own** reliability and liability. If your agent makes a $500K error that could have been prevented with a $15 human review, **you failed to implement proper gates**. If your system has systematic 2% errors and you call it "acceptable," **you're not an architect—you're negligent**. Reliability is not optional in production systems.

**Production Requirements**:
- ✅ Cross-Model Verification (different architectures for generator vs critic)
- ✅ Uncertainty-Triggered HITL (dynamic thresholds based on confidence)
- ✅ N-Version Consensus (3 agents for mission-critical tasks)
- ✅ Hybrid Gates (reflection + HITL for liability elimination)
- ✅ Financial Firewall (bound maximum error cost with hard gates)
- ✅ Monitoring (track hallucination rate, escalation rate, error cost)
