...
week: 7
concept: 1
description: "Monitor traces, evaluations, and unit economics to prevent silent failures, with semantic drift detection via embedding centroid monitoring"
estimatedMinutes: 55
objectives:
  ...
  - Monitor semantic drift via embedding centroid analysis to catch provider-side model changes
---
...
```typescript
// ❌ Without tracing, you see:
User query → [BLACK BOX] → Response (5 seconds)

// ✅ With tracing, you see:
User query
  ├─ Query embedding (200ms, 500 tokens, $0.0015)
  ├─ Vector search (1200ms, 10 results)
  ├─ Reranking (800ms, top 3 results)
  ├─ LLM call (2500ms, 2000 tokens, $0.045)
  └─ Response (5000ms total, $0.0465 total)
```
...
|---|---|---|
| &gt; 0.15 | Strong signal — top result is clearly the best match | Serve with high confidence |
| 0.05 - 0.15 | Ambiguous — multiple documents are nearly equal | Flag for re-ranking or consider returning multiple answers |
| &lt; 0.05 | Weak signal — no document is clearly relevant | Trigger fallback (ask for clarification, refuse to answer) |
...
### Semantic Drift Detection: Embedding Centroid Monitoring

Monitoring "System Health" (uptime, latency, error rates) is necessary but insufficient. You must also monitor **"Model Health"** — the behavioral consistency of your model's outputs over time.

**The Problem**: Cloud LLM providers regularly update models behind the same API endpoint. When Anthropic or OpenAI ships a "minor update," your system stays "up" but the model's **personality, reasoning style, or output distribution** may shift. Users report the AI "feels different" or "got dumber," but your dashboards show green across the board.

```
┌─────────────────────────────────────────────────────────────┐
│            SEMANTIC DRIFT DETECTION                         │
│                                                             │
│  Traditional Monitoring          Semantic Monitoring        │
│  ────────────────────            ─────────────────────      │
│  ✅ API is up                    📐 Output embeddings       │
│  ✅ Latency is normal             are shifting              │
│  ✅ No errors                                               │
│  ✅ Tokens are flowing           ⚠️  Centroid moved 0.15    │
│                                      in 24 hours            │
│  Status: ALL GREEN               Status: MODEL DRIFTING     │
│                                                             │
│  Day 1 Centroid: [0.12, 0.45, 0.78, ...]                  │
│  Day 2 Centroid: [0.12, 0.44, 0.77, ...]  ← Normal jitter │
│  Day 3 Centroid: [0.27, 0.31, 0.62, ...]  ← DRIFT 🚨      │
│                                                             │
│  The model was silently updated by the provider.            │
│  Your system is "working" but producing different outputs.  │
└─────────────────────────────────────────────────────────────┘
```

**How It Works**: Embed every model output over a rolling window. Calculate the **centroid** (average vector) of those embeddings. If the centroid shifts beyond a threshold between windows, the model's output distribution has changed — even if every individual response "looks fine."

```typescript
// src/week7/observability/semantic-drift.ts
import { embed } from '@/lib/embeddings'

/**
 * Embedding Centroid Monitor
 *
 * Tracks the "behavioral coordinates" of the model by computing
 * the centroid of output embeddings over rolling time windows.
 * A significant centroid shift indicates the model's output
 * distribution has changed — the only way to catch "Silent
 * Quality Collapse" before users notice.
 */
...
interface CentroidSnapshot {
  ...
  sampleCount: number
  stdDevMagnitude: number    // How spread out the outputs are
}

interface DriftAlert {
  detected: boolean
  magnitude: number          // Cosine distance between centroids
  threshold: number
  previousWindow: CentroidSnapshot
  currentWindow: CentroidSnapshot
  recommendation: string
}

/**
 * Calculate the centroid (average embedding) of a set of output texts
 */
async function calculateCentroid(outputs: string[]): Promise<{
  centroid: number[]
  stdDevMagnitude: number
}> {
  // Embed all outputs
  const embeddings = await Promise.all(
    outputs.map(text => embed(text))
  )

  // Calculate centroid (element-wise average)
  ...
  const distances = embeddings.map(emb => cosineSimilarity(emb, centroid))
  const avgDistance = distances.reduce((a, b) => a + b, 0) / distances.length
  const stdDevMagnitude = Math.sqrt(
    distances.reduce((sum, d) => sum + Math.pow(d - avgDistance, 2), 0) / distances.length
  )

  return { centroid, stdDevMagnitude }
}

/**
 * Detect semantic drift between two time windows
 */
async function detectSemanticDrift(
  currentWindow: CentroidSnapshot,
  previousWindow: CentroidSnapshot,
  driftThreshold: number = 0.08  // Cosine distance threshold
): Promise<DriftAlert> {
  const magnitude = 1 - cosineSimilarity(
    currentWindow.centroid,
    previousWindow.centroid
  )

  const detected = magnitude > driftThreshold

  let recommendation = ''
  if (magnitude > driftThreshold * 2) {
    recommendation = 'CRITICAL: Major behavioral shift detected. ' +
      'Likely cause: provider model update. ' +
      'Action: Run full regression suite immediately, pause deployments.'
  } else if (detected) {
    recommendation = 'WARNING: Moderate drift detected. ' +
      'Action: Run targeted eval on high-priority test cases. ' +
      'Check provider changelog for model updates.'
  } else {
    recommendation = 'Normal operational variance. No action required.'
  }
...
/**
 * Production monitoring loop: Run every 6 hours
 * Compares current 24h window against previous 24h window
 */
async function semanticDriftMonitor() {
  // Fetch outputs from current and previous windows
  const now = new Date()
  const windowSize = 24 * 60 * 60 * 1000 // 24 hours

  const currentOutputs = await prisma.queryLog.findMany({
    where: {
      timestamp: {
        gte: new Date(now.getTime() - windowSize),
        lte: now
      }
    },
    select: { response: true },
    take: 500  // Sample up to 500 outputs per window
  })

  const previousOutputs = await prisma.queryLog.findMany({
    where: {
      timestamp: {
        gte: new Date(now.getTime() - 2 * windowSize),
        lt: new Date(now.getTime() - windowSize)
      }
    },
    select: { response: true },
    take: 500
  ...
  if (currentOutputs.length &lt; 50 || previousOutputs.length &lt; 50) {
    console.log('⚠️ Insufficient samples for drift detection (need 50+ per window)')
    return
  }

  // Calculate centroids
  const current = await calculateCentroid(currentOutputs.map(o => o.response))
  const previous = await calculateCentroid(previousOutputs.map(o => o.response))

  const currentSnapshot: CentroidSnapshot = {
    windowStart: new Date(now.getTime() - windowSize),
    windowEnd: now,
    centroid: current.centroid,
    sampleCount: currentOutputs.length,
    stdDevMagnitude: current.stdDevMagnitude
  }

  const previousSnapshot: CentroidSnapshot = {
    windowStart: new Date(now.getTime() - 2 * windowSize),
    windowEnd: new Date(now.getTime() - windowSize),
    centroid: previous.centroid,
    sampleCount: previousOutputs.length,
    stdDevMagnitude: previous.stdDevMagnitude
  }

  // Detect drift
  const alert = await detectSemanticDrift(currentSnapshot, previousSnapshot)

  // Store for trending
  await prisma.semanticDriftLog.create({
    data: {
      timestamp: now,
      driftMagnitude: alert.magnitude,
      threshold: alert.threshold,
      detected: alert.detected,
      sampleCountCurrent: currentSnapshot.sampleCount,
      sampleCountPrevious: previousSnapshot.sampleCount,
      stdDevCurrent: currentSnapshot.stdDevMagnitude,
      stdDevPrevious: previousSnapshot.stdDevMagnitude
    }
  })

  console.log(`📐 Semantic Drift Check:`)
  console.log(`   Current window: ${currentSnapshot.sampleCount} samples`)
  console.log(`   Previous window: ${previousSnapshot.sampleCount} samples`)
  console.log(`   Drift magnitude: ${alert.magnitude.toFixed(4)} (threshold: ${alert.threshold})`)
  console.log(`   Status: ${alert.detected ? '🚨 DRIFT DETECTED' : '✅ Stable'}`)

  if (alert.detected) {
    await alertOpsTeam({
      severity: alert.magnitude > alert.threshold * 2 ? 'critical' : 'warning',
      title: 'Semantic Drift Detected',
      message: alert.recommendation,
      dashboard: 'semantic-drift',
      metadata: {
        magnitude: alert.magnitude,
        threshold: alert.threshold,
        currentSamples: currentSnapshot.sampleCount,
        previousSamples: previousSnapshot.sampleCount
      }
    })

    // Auto-trigger regression suite on critical drift
    if (alert.magnitude > alert.threshold * 2) {
      console.log('🔬 Auto-triggering full regression suite...')
      await triggerRegressionSuite('semantic-drift-detected')
    }
  }
}
```
...
| Drift Magnitude | Severity | Likely Cause | Action |
|-----------------|----------|--------------|--------|
| &lt; 0.04 | Normal | Day-to-day variance in outputs | None |
| 0.04 - 0.08 | Low | Topic distribution shift in user queries | Monitor |
| 0.08 - 0.15 | Warning | Possible provider-side model update | Run targeted evals |
| &gt; 0.15 | Critical | Confirmed model behavioral shift | Full regression + pause deployments |

> **Architect's Tip**: "Don't just monitor errors; monitor the vector space. Calculate the average embedding of your model's outputs over 24 hours. If the 'Centroid' of those embeddings shifts significantly, your model is drifting. This is the only way to catch a 'Silent Quality Collapse' before users start reporting that the AI feels 'different' or 'dumber'."
...
// ✅ HTTP 200, no errors
// ❌ WRONG - enterprise customers have 90-day returns!
```
...
Running LLM-as-a-Judge on **every request** in the hot path can double your latency and cost. In production, you need **Probabilistic Evaluation Sampling** — a pattern where 100% of requests are served immediately, but a configurable percentage are **asynchronously** evaluated by a "Teacher Model" in the background.
...
class ShadowEvaluationProxy {
  ...
  // Called on EVERY request — decides whether to sample
...
**Background Worker — The Teacher Model Evaluates**:
...
| No eval | +0ms | +0% | Blind — no quality signal |

With 10,000 daily requests and a 10% sample rate, you evaluate 1,000 requests per day — more than enough for statistically significant quality monitoring with a ±3% confidence interval.
...
├─ decompose_query (0.5s)
├─ sub_task_0 (2.8s)
│  └─ technical_agent (2.7s)
│     └─ technical_rag (2.5s)
│        ├─ embed_query (0.2s)
│        ├─ vector_search (0.8s)
│        ├─ rerank (0.5s)
│        └─ llm_call (1.0s)
├─ sub_task_1 (3.5s) ← BOTTLENECK!
│  └─ medical_agent (3.4s)
│     └─ medical_rag (3.2s)
│        ├─ embed_query (0.2s)
│        ├─ vector_search (2.5s) ← SLOW!
│        ├─ rerank (0.3s)
│        └─ llm_call (0.2s)
└─ synthesize (1.4s)
```
...
async function monitorDistributedTraces() {
  ...
  for (const trace of recentTraces) {
    ...
    if (metrics.total_latency_ms &gt; 10000) {
      await alertSlack({
        channel: '#ai-alerts',
        text: `⚠️ Slow distributed trace: ${metrics.total_latency_ms}ms
...
Trace ID: ${trace.traceId}`
    ...
    if (metrics.parallel_efficiency &lt; 0.5) {
      await alertSlack({
        channel: '#ai-alerts',
        text: `⚠️ Poor parallel efficiency: ${(metrics.parallel_efficiency * 100).toFixed(1)}%
...
Trace ID: ${trace.traceId}`
    ...
    for (const [agent, latency] of Object.entries(metrics.agent_latencies)) {
      if (latency &gt; agentAvgLatencies[agent] * 2) {
        await alertSlack({
          channel: '#ai-alerts',
          text: `⚠️ Agent ${agent} is 2x slower than average: ${latency}ms vs ${agentAvgLatencies[agent]}ms
Trace ID: ${trace.traceId}`
        })
      }
    }
  ...
  for (const tenant of tenants) {
    ...
    if (monthlyCosts.total_cost &gt; budgetLimit * 1.2) { // 120% of budget
      await alertSlack({
        ...
        text: `⚠️ Tenant ${tenant.name} is over budget:
...
An Architect doesn't just look at a total bill. You must link **Token Attribution to SaaS Tiers** to identify **Toxic Tenants** — users whose complex queries cost more in tokens than they pay in their monthly subscription.
...
**A)** Traces — You need to check if the database is slow. Latency spikes are probably causing bad user experience.

**B)** Unit Economics — You need to check if you're spending enough on tokens. Maybe the model was downgraded to save costs and it's producing worse answers.

**C)** Evaluations — You were monitoring "System Health" but not "Semantic Health." You need to implement LLM-as-a-Judge to track Answer Relevance scores over time to detect "Model Drift" or "Context Degradation" that doesn't trigger standard errors.

**D)** Logs — You need to read more user chat histories to manually identify bad responses.

<details>
<summary>Correct Answer</summary>

**C — Evaluations (Semantic Health Monitoring)**
...
This is the **Silent Quality Collapse** — the most dangerous failure mode in AI:
...
**None of these trigger errors.** None of them affect latency. None of them appear in traditional APM dashboards. The only way to detect them is through **continuous semantic evaluation** — monitoring Faithfulness, Answer Relevance, and Context Precision over time using the Shadow Evaluation Proxy pattern.

**Why the other answers are wrong**:
- **A (Traces)**: Latency is 150ms — traces would confirm the system is fast. Speed is not the problem.
...
2. **Silent Failures**: AI systems fail without errors — "Up" does not mean "Correct"
...
4. **Shadow Evaluation Proxy**: Sample 10% of production traffic for async Teacher Model evaluation — statistically significant quality monitoring without impacting P99 latency
