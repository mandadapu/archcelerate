...
week: 7
concept: 3
description: "Build automated evaluation systems using stronger models to grade weaker models at scale with cross-model hierarchies, negative sampling, artifact manifests, and uncertainty-triggered human escalation"
estimatedMinutes: 60
objectives:
  ...
  - Build logprob-based escalation for uncertainty-triggered human-in-the-loop routing
---
...
async function runRegressionTests(
  ...
  console.log(`🔬 Running Compliance Regression Tests`)
...
async function testPromptChange() {
  ...
  // ⚠️ Missing explicit disclaimer requirement!
...
- Cost: $450 per release ($0.90 per test × 500 tests, including judge model)
...
**Metrics**:
- **Time Reduction**: 2 weeks → 2 hours = **99.4% reduction**
- **Cost Savings**: $15,000 → $450 = **$14,550 per release**
- **Annual Savings**: $14,550 × 12 releases = **$174,600/year**
- **Deployment Frequency**: 12x increase (monthly → weekly)
...
│                                                         │
│  Production Model          Judge Model                  │
│  ────────────────          ──────────────               │
│  Claude Haiku 4.5    →    Claude Sonnet 4.5             │
│  GPT-4o mini         →    GPT-4o / Claude Sonnet 4.5    │
│  Claude Sonnet 4.5   →    OpenAI o1 / DeepSeek R1       │
│  GPT-4o              →    OpenAI o1 / Claude Opus       │
│                                                         │
│  ⚠️  INVALID: Same-tier evaluation                      │
│  Claude Haiku  →  Claude Haiku   ❌ (self-grading)      │
│  Sonnet 4.5    →  Sonnet 4.5     ❌ (blind spots match) │
│                                                         │
│  KEY: Judge must evaluate the LOGIC PATH,               │
  ...
  if (candidates.length === 0) {
    ...
    if (reasoningModels.length === 0) {
      throw new Error(
        ...
        `Cannot self-evaluate — add a reasoning model to the registry.`
      )
    }
...
async function reasoningLayerEval(
  ...
  const judgePrompt = `You are a reasoning evaluator. Your task is NOT to check if the answer "sounds right" — it is to trace the LOGICAL CHAIN from context to conclusion.
...
|-----------------|-------------|-----------------|----------------|----------|
| Haiku 4.5 ($0.001/1K) | Sonnet 4.5 ($0.015/1K) | ~$0.003 | Fast → Frontier | High-volume customer support |
| GPT-4o mini ($0.0015/1K) | GPT-4o ($0.015/1K) | ~$0.005 | Fast → Frontier | Chatbot QA |
| Sonnet 4.5 ($0.015/1K) | o1 ($0.06/1K) | ~$0.02 | Frontier → Reasoning | RAG for regulated industries |
| GPT-4o ($0.015/1K) | DeepSeek R1 ($0.014/1K) | ~$0.015 | Frontier → Reasoning | Code generation review |
...
**The Problem**: In traditional software, you version code. In AI, you must version the **Inference Context** — the complete combination of Prompt + Model + Retrieval Configuration that determines system behavior.
...
│  "v1.2.3 passed 95% of Golden Dataset,                     │
│   but FAILED on 3 edge cases where v1.2.2 passed."         │
│                                                             │
│  → Director can make informed risk-based deploy decision.   │
└─────────────────────────────────────────────────────────────┘
```
...
const v1_2_3 = generateManifest(
  ...
  {
    strategy: 'hybrid-search',       // ← Changed strategy
    chunkSize: 512,
    chunkOverlap: 100,               // ← Changed overlap
    topK: 5,
    reranker: 'cohere',              // ← Added reranker
    embeddingModel: 'text-embedding-3-small'
  },
  {
    faithfulnessAvg: 0.80,           // ⚠️ DROPPED from 0.95
    relevanceAvg: 0.92,              // ✅ Improved
    safetyPassRate: 1.0,
    compliancePassRate: 0.97,        // ⚠️ DROPPED from 1.0
    costPerQuery: 0.018,             // ⚠️ Increased
    latencyP95Ms: 1800               // ✅ Improved
  },
...
async function differentialTest(
  ...
  console.log(`📊 Differential Test: v${manifestA.version} vs v${manifestB.version}`)
  ...
  console.log(`  v${manifestA.version} → v${manifestB.version}`)
  console.log(`${'═'.repeat(60)}`)
  console.log(`  Total Tests:   ${report.summary.totalTests}`)
  console.log(`  Both Passed:   ${report.summary.bothPassed}`)
  console.log(`  Improvements:  ${report.summary.improvements} ✅`)
  console.log(`  Regressions:   ${report.summary.regressions} ${regressions > 0 ? '🚨' : '✅'}`)
  console.log(`${'─'.repeat(60)}`)

  if (regressionDetails.length > 0) {
    console.log(`  REGRESSION DETAILS:`)
    for (const detail of regressionDetails) {
      console.log(`    [${detail.testId}] ${detail.dimension}: ${detail.versionAScore.toFixed(2)} → ${detail.versionBScore.toFixed(2)}`)
...
// ──────────────────────────────────────────────────────────
//   REGRESSION DETAILS:
//     [INV-001] faithfulness: 0.95 → 0.70
//       Query: "Should I invest all my savings in tech stocks?"
//     [REG-045] compliance: 1.00 → 0.80
//       Query: "What are the tax implications of selling my mutual fund?"
//     [BAL-112] faithfulness: 0.98 → 0.65
...
## Uncertainty-Triggered Human-in-the-Loop: Architecture by Exception

Most HITL systems route to humans based on **static categories** (e.g., "all refund requests over $500 go to a human"). An Architect builds **self-aware systems** that escalate based on the model's own **confidence signals**.

### The Problem with Static Escalation

```
┌─────────────────────────────────────────────────────────────┐
│         STATIC vs DYNAMIC ESCALATION                        │
│                                                             │
│  STATIC ESCALATION (Current):                               │
│  ─────────────────────────────                              │
│  if (topic === 'refund' && amount > 500) → HUMAN            │
│  if (topic === 'legal') → HUMAN                             │
│  else → AI handles it                                       │
│                                                             │
│  Problem: AI confidently handles a $50 refund question      │
│  it doesn't know the answer to. No escalation triggered.    │
│                                                             │
│  DYNAMIC ESCALATION (Hardened):                             │
│  ──────────────────────────────                             │
│  if (model.confidence < threshold) → HUMAN                  │
│  if (judge.disagrees_with_generator) → HUMAN                │
│  if (perplexity > anomaly_threshold) → HUMAN                │
│  else → AI handles it                                       │
│                                                             │
│  The AI admits: "I'm not sure about this one."              │
└─────────────────────────────────────────────────────────────┘
```

### Logprob-Based Confidence Scoring

When an LLM generates tokens, each token has a **log probability** — the model's confidence that this is the right next token. Low logprobs (high perplexity) mean the model is "guessing," not "knowing."

```typescript
// src/week7/evals/logprob-escalation.ts
import Anthropic from '@anthropic-ai/sdk'
import OpenAI from 'openai'

/**
 * Logprob-Based Escalation: "Architecture by Exception"
 *
 * Only use expensive human labor when the AI admits it's confused.
 * Three escalation signals:
 * 1. Low token-level confidence (high perplexity)
 * 2. Cross-model disagreement (judge vs generator)
 * 3. Hedging language detection (linguistic uncertainty markers)
 */

interface ConfidenceAssessment {
  overallConfidence: number      // 0-1, aggregated confidence score
  tokenPerplexity: number        // Average perplexity across output tokens
  hedgingScore: number           // 0-1, how much hedging language detected
  crossModelAgreement: number    // 0-1, agreement between judge and generator
  escalationRequired: boolean
  escalationReason?: string
  escalationPriority: 'none' | 'low' | 'medium' | 'high' | 'critical'
}

/**
 * Signal 1: Token-level confidence from logprobs
 *
 * OpenAI API exposes logprobs directly. For Anthropic, we use
 * a proxy approach with hedging language detection.
 */
async function getTokenConfidence(
  query: string,
  context: string,
  systemPrompt: string
): Promise<{
  ...
  minLogprob: number
  perplexity: number
  lowConfidenceSpans: { text: string; logprob: number }[]
}> {
  ...
  const logprobData = choice.logprobs?.content || []

  // Calculate token-level statistics
  const logprobs = logprobData.map(t => t.logprob)
  ...
  // Find low-confidence spans (tokens where model is uncertain)
  const lowConfidenceSpans: { text: string; logprob: number }[] = []
  for (const token of logprobData) {
    if (token.logprob &lt; -2.0) {  // Less than ~13% confidence
      lowConfidenceSpans.push({
        text: token.token,
...
/**
 * Signal 2: Cross-model disagreement detection
 *
 * Run the same query through both the generator (student) and
 * a judge (teacher). If they produce meaningfully different
 * answers, the system should escalate.
 */
async function detectCrossModelDisagreement(
  query: string,
  context: string,
  generatorResponse: string
): Promise<{
  agreement: number        // 0-1, semantic agreement
  disagreementType?: 'factual' | 'interpretation' | 'completeness'
  judgeResponse: string
  explanation: string
}> {
  const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY })

  // Step 1: Get independent judge response
  const judgeCompletion = await anthropic.messages.create({
...
If the context doesn't contain enough information, say "INSUFFICIENT_CONTEXT".`
  ...
  const judgeResponse = judgeCompletion.content[0].type === 'text'
    ? judgeCompletion.content[0].text
    : ''

  // Step 2: Compare generator and judge responses
  const comparisonResult = await anthropic.messages.create({
    model: 'claude-sonnet-4-5-20250514',
    max_tokens: 512,
    temperature: 0.0,
    messages: [{
      role: 'user',
      content: `Compare these two responses to the same question. Do they agree on the key facts?

Question: ${query}
Context: ${context}

Response A (Generator): ${generatorResponse}
Response B (Judge): ${judgeResponse}

Rate agreement 0.0-1.0 and classify any disagreement.

Output JSON:
{
  "agreement": 0.0-1.0,
  "disagreementType": "factual" | "interpretation" | "completeness" | null,
  "explanation": "Brief explanation"
}`
    }]
  })

  const text = comparisonResult.content[0].type === 'text'
    ? comparisonResult.content[0].text
    : '{}'
  const result = JSON.parse(text.match(/\{[\s\S]*\}/)?.[0] || '{"agreement": 1.0}')
 ...
 *
 * When the model is uncertain, it uses hedging phrases.
 * This works as a proxy for logprobs when the API doesn't expose them.
 */
function detectHedgingLanguage(response: string): {
  hedgingScore: number
  hedgingPhrases: string[]
} {
  const HEDGING_PATTERNS = [
    { pattern: /I('m| am) not (entirely |completely )?sure/i, weight: 0.9 },
    { pattern: /I think/i, weight: 0.3 },
    { pattern: /I believe/i, weight: 0.4 },
    { pattern: /it('s| is) possible that/i, weight: 0.5 },
    { pattern: /might be/i, weight: 0.4 },
    { pattern: /could be/i, weight: 0.3 },
    { pattern: /may (or may not|not)/i, weight: 0.6 },
    { pattern: /if I('m| am) not mistaken/i, weight: 0.7 },
    { pattern: /to the best of my knowledge/i, weight: 0.5 },
    { pattern: /approximately|roughly|around/i, weight: 0.2 },
    { pattern: /it seems (like|that)/i, weight: 0.4 },
    { pattern: /I('d| would) recommend (checking|verifying|confirming)/i, weight: 0.6 },
    { pattern: /please (verify|confirm|check) (with|this)/i, weight: 0.7 },
    { pattern: /I don('t| do not) have (enough|sufficient) (information|context|data)/i, weight: 0.9 },
  ]
  ...
  // Normalize to 0-1 (cap at 1.0 if many hedging phrases)
  const hedgingScore = Math.min(totalWeight / 2.0, 1.0)

  return { hedgingScore, hedgingPhrases }
}

/**
 * Complete Confidence Assessment Pipeline
 *
 * Combines all three signals into a single escalation decision.
 */
async function assessConfidence(
  ...
  const {
    ...
    perplexityThreshold = 15.0,   // Perplexity above this = uncertain
    agreementThreshold = 0.7,     // Agreement below this = disagreement
    hedgingThreshold = 0.5        // Hedging above this = uncertain
  } = options

  // Signal 1: Hedging language (always available, free)
  const hedging = detectHedgingLanguage(generatorResponse)

  // Signal 2: Token confidence (requires OpenAI for logprobs)
  let tokenPerplexity = 1.0
  if (enableLogprobs) {
    try {
      const tokenResult = await getTokenConfidence(query, context, '')
      tokenPerplexity = tokenResult.perplexity
    } catch {
      // Logprobs not available, rely on other signals
      tokenPerplexity = 1.0
    }
  }

  // Signal 3: Cross-model disagreement (expensive, use selectively)
  let crossModelAgreement = 1.0
  if (enableCrossModel && (hedging.hedgingScore > 0.3 || tokenPerplexity > 10.0)) {
    // Only run cross-model check if other signals suggest uncertainty
    const disagreement = await detectCrossModelDisagreement(
      query, context, generatorResponse
    )
    crossModelAgreement = disagreement.agreement
  }

  // Aggregate confidence (weighted combination)
  const overallConfidence = (
    (1.0 - hedging.hedgingScore) * 0.3 +                              // Hedging signal
    Math.min(1.0, 5.0 / tokenPerplexity) * 0.3 +                      // Perplexity signal
    crossModelAgreement * 0.4                                           // Cross-model signal
  )

  // Determine escalation
  let escalationRequired = false
  let escalationReason: string | undefined
  let escalationPriority: ConfidenceAssessment['escalationPriority'] = 'none'

  if (crossModelAgreement &lt; agreementThreshold) {
    escalationRequired = true
    escalationReason = `Cross-model disagreement: generator and judge agree only ${(crossModelAgreement * 100).toFixed(0)}%`
    escalationPriority = crossModelAgreement &lt; 0.4 ? 'high' : 'medium'
  } else if (tokenPerplexity > perplexityThreshold) {
    escalationRequired = true
    escalationReason = `High perplexity: ${tokenPerplexity.toFixed(1)} (threshold: ${perplexityThreshold})`
    escalationPriority = tokenPerplexity > perplexityThreshold * 2 ? 'high' : 'medium'
  } else if (hedging.hedgingScore > hedgingThreshold) {
    escalationRequired = true
    escalationReason = `Hedging language detected: "${hedging.hedgingPhrases.join('", "')}"`
  ...
  return {
    overallConfidence,
    tokenPerplexity,
    hedgingScore: hedging.hedgingScore,
    crossModelAgreement,
    escalationRequired,
    escalationReason,
    escalationPriority
...
/**
 * Production integration: Route responses based on confidence
 */
async function confidenceAwareResponse(
  query: string,
  context: string
): Promise<{
  response: string
  routed: 'ai' | 'human_review' | 'human_immediate'
  ...
  // Assess confidence
  const confidence = await assessConfidence(query, context, response, {
    enableLogprobs: true,
    enableCrossModel: true
  })

  if (!confidence.escalationRequired) {
    // High confidence — serve directly
    return { response, routed: 'ai', confidence }
  }

  if (confidence.escalationPriority === 'high' || confidence.escalationPriority === 'critical') {
    // Low confidence — pause and escalate immediately
    await createHumanReviewTicket({
      query,
      context,
      aiResponse: response,
      reason: confidence.escalationReason!,
      priority: confidence.escalationPriority,
      confidenceScore: confidence.overallConfidence
    })

    return {
      response: 'I want to make sure I give you the most accurate answer. ' +
        'I\'ve flagged this for our team to review, and you\'ll hear back shortly.',
      routed: 'human_immediate',
      confidence
    }
  }

  // Medium/low confidence — serve response but queue async review
  await createHumanReviewTicket({
    query,
    context,
    aiResponse: response,
  ...
  return { response, routed: 'human_review', confidence }
}

async function createHumanReviewTicket(ticket: {
  query: string
  context: string
  aiResponse: string
  reason: string
  ...
  console.log(`   Confidence: ${(ticket.confidenceScore * 100).toFixed(0)}%`)
...
| Signal | Source | Cost | Latency | Best For |
|--------|--------|------|---------|----------|
| **Hedging Detection** | Regex on response text | Free | &lt;1ms | Quick proxy for uncertainty |
| **Token Perplexity** | OpenAI logprobs API | Included in generation | 0ms (piggybacked) | Token-level confidence |
| **Cross-Model Disagreement** | Second model call | ~$0.003/query | ~500ms | Catching confident hallucinations |
...
│                                                             │
│  Query → Generate Response → Assess Confidence              │
│                                   │                         │
│                    ┌──────────────┼──────────────┐          │
│                    │              │              │           │
...
│              ┌─────────┐  ┌───────────┐  ┌────────────┐    │
│              │ Serve    │  │ Serve +   │  │ Pause +    │    │
│              │ directly │  │ async     │  │ escalate   │    │
│              │          │  │ human     │  │ to human   │    │
│              │ Cost: $0 │  │ review    │  │ immediately│    │
│              │          │  │           │  │            │    │
│              │ 85% of   │  │ 12% of   │  │ 3% of     │    │
│              │ queries  │  │ queries   │  │ queries    │    │
│              └─────────┘  └───────────┘  └────────────┘    │
│                                                             │
│  Result: 97% of queries handled by AI.                      │
│  Only 3% require immediate human intervention.              │
│  12% get async quality review (no user delay).              │
└─────────────────────────────────────────────────────────────┘
```

> **Architect's Tip**: "An Architect builds 'Self-Aware' systems. If the model's output tokens have high Perplexity (low confidence), or if your 'Teacher' and 'Student' models disagree during an async check, the system should automatically 'Pause' the thread and flag it for human review. This is 'Architecture by Exception' — you only use expensive human labor when the AI admits it's confused."
...
│        ▼                                                    │
│   ┌─────────┐    Thumbs Down    ┌──────────────────┐       │
│   │ User    │ ──────────────── → │ Negative Signal  │       │
│   │ Session │    Guardrail Trip  │ Collector        │       │
│   └─────────┘    Low Eval Score  └────────┬─────────┘       │
│                                           │                 │
│                                           ▼                 │
│                                  ┌──────────────────┐       │
...
│   │              GOLDEN DATASET                       │     │
│   │                                                   │     │
│   │  Happy Path (40%)  │  Negative Samples (35%)      │     │
│   │  ─────────────────  │  ──────────────────────      │     │
│   │  Standard queries   │  Thumbs-down interactions    │     │
│   │  Known good answers │  Guardrail trigger cases     │     │
│   │                     │  Production hallucinations   │     │
│   │                     │                              │     │
│   │  Edge Cases (15%)  │  Adversarial (10%)           │     │
│   │  ─────────────────  │  ──────────────────────      │     │
│   │  Empty inputs       │  Prompt injection attempts   │     │
│   │  Multi-language      │  Boundary-pushing queries   │     │
│   │  Ambiguous queries  │  Known attack patterns       │     │
│   └──────────────────────────────────────────────────┘     │
│                          │                                  │
│                          ▼                                  │
│                 [Regression Suite]                           │
 ...
 * a regression test case — ensuring your "Teacher" model specifically
...
async function ingestNegativeSamples(): Promise<{
  ...
  console.log(`📊 Negative Sampling Results:`)
...
async function dailyNegativeSamplingJob() {
  console.log('🔄 Running daily negative sampling pipeline...')
  ...
  if (results.autoAddedToDataset > 10) {
    await alertSlack({
      channel: '#ai-alerts',
      text: `🚨 ${results.autoAddedToDataset} critical failures auto-added to golden dataset in the last 24h. Review immediately.`
    })
  }
}
```
...
┌─────────────────────────────────────────────────────────┐
│         DIFFERENTIAL TEST REPORT                        │
│         v2.3.1 → v2.4.0 (Polite Prompt Update)         │
├─────────────────────────────────────────────────────────┤
│                                                         │
│  Metric          v2.3.1    v2.4.0    Change             │
│  ──────────────  ────────  ────────  ──────             │
│  Relevance       0.85      0.92      +0.07  ✅          │
│  Faithfulness    0.95      0.80      -0.15  🚨          │
│  Safety          1.00      1.00       0.00  ✅          │
│  Compliance      1.00      0.98      -0.02  ⚠️          │
│                                                         │
│  RECOMMENDATION: ???                                    │
└─────────────────────────────────────────────────────────┘
```
...
**B)** No. A drop in Faithfulness means the AI is now "hallucinating" — it's giving polite and relevant-sounding answers that aren't actually supported by the retrieved data. This is a Regression Failure. You must revert and tune the prompt to maintain the 0.95 Faithfulness floor.
...
- **Relevance** measures "did the answer address the question?" — important, but not dangerous when slightly off
- **Faithfulness** measures "is the answer actually supported by the retrieved data?" — when this drops, the AI is generating **confident, polite hallucinations**
...
function shouldDeploy(oldManifest: ArtifactManifest, newManifest: ArtifactManifest): boolean {
  ...
  if (newEval.faithfulnessAvg &lt; oldEval.faithfulnessAvg - 0.02) {
    console.log('🚨 BLOCKED: Faithfulness regression detected')
    console.log(`   ${oldEval.faithfulnessAvg.toFixed(2)} → ${newEval.faithfulnessAvg.toFixed(2)}`)
    return false
  }

  if (newEval.safetyPassRate &lt; oldEval.safetyPassRate) {
    console.log('🚨 BLOCKED: Safety regression detected')
    return false
  }

  if (newEval.compliancePassRate &lt; oldEval.compliancePassRate - 0.01) {
    console.log('🚨 BLOCKED: Compliance regression detected')
    return false
  }

  // SOFT METRICS: Relevance and latency can trade off
  console.log('✅ Hard floors maintained, checking soft metrics...')
  return true
}
```
...
1. **Cross-Model Evaluation Hierarchy**: The judge must be at least one capability tier above the student — frontier models require reasoning-model judges
...
5. **Faithfulness Floor**: Never sacrifice grounding/faithfulness for style or relevance — polite hallucinations are more dangerous than blunt correct answers
6. **Deployment Gate**: Require 95%+ pass rate with hard floors on safety, compliance, and faithfulness
7. **Uncertainty-Triggered HITL**: Use logprob perplexity, hedging detection, and cross-model disagreement to auto-escalate low-confidence outputs to human review
8. **Continuous Eval**: Sample production traffic hourly to detect drift before it becomes a crisis
