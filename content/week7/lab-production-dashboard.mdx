...
├─ extract_data (800ms, 1500 tokens)
└─ check_inventory (450ms, in_stock: true)
```

**✅ Checkpoint**: Can you identify which step is slowest?
...
export async function runEvalPipeline() {
  ...
  console.log(`\n📊 Results: ${passed}/${GOLDEN_DATASET.length} passed (${passRate.toFixed(1)}%)`)

  if (passRate &lt; 90) {
    throw new Error('❌ Eval pipeline failed: Pass rate below 90%')
  }

  console.log('✅ Eval pipeline passed!')
}
```

**✅ Checkpoint**: Does your system achieve &gt;90% pass rate?
...
export class CostTracker {
  ...
  trackQuery(sessionId: string, tokens: { input: number; output: number }) {
    ...
    if (newCost > this.MAX_SESSION_COST) {
      throw new Error(`🚨 Circuit breaker triggered: Session cost ($${newCost.toFixed(2)}) exceeded limit ($${this.MAX_SESSION_COST})`)
    }
...
export async function safeProcessTicket(sessionId: string, email: string) {
  try {
    ...
    console.log(`💰 Query cost: $${costInfo.queryCost.toFixed(4)}`)
...
**✅ Checkpoint**: Test with a loop that triggers the circuit breaker.
...
export async function guardrailedProcessTicket(email: string) {
  ...
  if (piiCheck.found) {
    console.error('⚠️ Output guardrail: PII detected in response')
    return {
...
export async function runCapstoneTests() {
  console.log('🏆 Running Capstone Stress-Test Suite\n')
...
|--------|----------|---------------|
| 1 | Complex Retrieval | ✅ Cited correct page and date |
| 2 | Intent Classification | ✅ JSON log captured both 'billing' and 'scheduling' |
| 3 | Poison Pill | ✅ System stopped LLM from giving medical advice |
| 4 | Hallucination | ✅ Compliance Agent flagged missing info |
...
✅ **Production-grade observability** - Trace every query to identify bottlenecks
✅ **Automated quality assurance** - LLM-as-a-Judge evaluates every response
✅ **Cost controls** - Circuit breakers prevent runaway spending
✅ **Safety guarantees** - Guardrails block unsafe inputs and outputs
✅ **Capstone readiness** - Pass all 4 stress tests
