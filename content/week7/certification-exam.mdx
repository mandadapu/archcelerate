...
**Grading Standard**: This exam is graded at the **Director/Staff Architect** level. You are expected to treat deployment as a **gated process** — no change reaches production without a passed regression suite, cross-model validation, and cost impact analysis. Solutions that rely on "trusting the model" or "manually reviewing the logs" will not pass.

**Core Principle**: In production AI, **"Up" does not mean "Correct."** Traditional SRE guarantees (uptime, latency) are necessary but insufficient. You must monitor and enforce **Semantic Health** — faithfulness, safety, and economic sustainability — with the same discipline as infrastructure health.
...
**Current Situation**:
A new version of the medical knowledge base has just been ingested (v4.2 → v4.3, adding 45,000 updated drug interaction tables). Simultaneously, the product team wants to deploy a new "more empathetic" system prompt designed to improve clinician satisfaction scores.

**Non-Functional Requirements**:
- Latency SLA: &lt;2s end-to-end (P99)
- Faithfulness: &gt;0.95 (regulatory requirement — FDA 21 CFR Part 11 compliance)
...
```
Shadow Evaluation Proxy — 4-Hour Canary Window (v4.3 + new prompt)

Metric                    Baseline (v4.2)    Canary (v4.3)    Delta
──────────────────────────────────────────────────────────────────
Faithfulness              0.98               0.88             -0.10  ⚠️ CRITICAL
Answer Relevance          0.94               0.96             +0.02  ✅
Toxicity                  0.01               0.01              0.00  ✅
P99 Latency               1.7s              1.8s             +0.1s  ✅
Uptime                    100%              100%               0.0%  ✅
Cache Hit Rate            42%               38%              -4.0%  ⚠️
Top-1 Similarity Delta    0.18              0.09             -0.09  ⚠️
```
...
**Pillar 1 — Traces (Semantic Telemetry)**:
...
async function diagnoseCanaryRegression(canaryWindow: TimeRange) {
  ...
  if (canaryAvgSimilarity < baselineAvgSimilarity * 0.90) {
    return {
      ...
      action: 'Audit v4.3 knowledge base chunks — likely poor chunking or embedding drift'
    }
  }
  ...
  return {
    ...
    evidence: 'Retrieval quality is stable but faithfulness dropped — ' +
      'the model is receiving good context but generating unfaithful responses',
    action: 'Audit the new empathetic system prompt — likely over-weighting tone over accuracy'
  }
}
```

**Pillar 2 — Evaluations (Shadow Evaluation Proxy)**:
...
If **context citation accuracy is stable** (94%) but **claims supported by context dropped** (82%), the retrieval layer is healthy — the model is receiving good context but choosing to generate unsupported "empathetic" additions like "Don't worry, this is very treatable" without evidence.

**Pillar 3 — Unit Economics**:

- Cache hit rate dropped 42% → 38% — the new knowledge base may have shifted embedding distributions
- Top-1 similarity delta dropped 0.18 → 0.09 — retrieval is becoming ambiguous, suggesting embedding drift from v4.3 documents

**Verdict**: Cross-reference all three pillars. If Traces show stable retrieval + Evaluations show unsupported claims + Economics show minor cache disruption → **Root cause is the empathetic prompt**. If Traces show degraded retrieval + Evaluations show poor context → **Root cause is the data ingestion**.
...
Faithfulness: 0.82
Unsupported Claims:
  - "most patients tolerate metformin very well" ← NOT IN CONTEXT
  - "side effects typically resolve within a few weeks" ← NOT IN CONTEXT
...
async function safeRollback(
  ...
  switch (issue) {
    case 'prompt':
      // Rollback ONLY the prompt — keep the new knowledge base
      return {
    ...
    case 'knowledge_base':
      // Rollback ONLY the knowledge base — keep the current prompt
      return {
...
**Key Insight**: The rollback is **granular** — you can independently revert the prompt, the knowledge base, or the model. Reverting everything at once is the naive approach that throws away the valid v4.3 knowledge base update.
...
|---|---|---|
| Dropped significantly (0.18 → 0.06) | Dropped | **Data-layer failure**: Embeddings shifted, retrieval quality degraded. LLM is faithful to bad context. |
| Stable (0.18 → 0.16) | Dropped | **Prompt-layer failure**: Good context is being retrieved but the model is generating unfaithful responses. |
...
**In the MediSecure scenario**: Similarity delta dropped 0.18 → 0.09 (moderate) while faithfulness dropped 0.98 → 0.88. This is a **mixed signal** — the v4.3 ingestion slightly disrupted retrieval (explaining the delta drop), but the primary cause is the prompt (explaining why faithfulness dropped more than retrieval quality).
...
const DIAGNOSTIC_THRESHOLDS = {
  ...
  dataLayerInvestigation: {
    ...
    trigger: 'Similarity delta collapsed — check embeddings, chunking, index'
  },
  ...
  promptLayerInvestigation: {
    ...
    trigger: 'Faithfulness dropped with stable retrieval — check system prompt'
  }
}
```
...
- ✅ Three Pillars diagnostic framework (Traces, Evaluations, Economics)
- ✅ Semantic Telemetry (similarity delta as regression signal)
- ✅ Trace-Linked Chunk ID auditing (chunk-level provenance)
- ✅ Granular rollback strategy (independent prompt/KB/model revert)
- ✅ Shadow Evaluation Proxy data interpretation
...
Verdict: ✅ PASS (false negative — attack bypasses all keyword filters)
```
...
1. **Encoding bypass**: Base64, URL encoding (`%49gnore%20previous`), Unicode homoglyphs (`Ιgnore` using Greek Ι instead of Latin I), ROT13, hex encoding
...
| Unicode homoglyphs | `Ιgnore prevіous` (Greek Ι, Cyrillic і) | Visually identical, different codepoints |
...
**Fundamental Limitation**: Keyword filters operate on the **syntactic surface** of text. Prompt injection is a **semantic attack** — the meaning is what matters, not the exact tokens. You need a semantic defense.
...
const MEDISECURE_GUARDRAILS: GuardrailPipeline = {
  layers: [
    ...
    // Layer 4: Structural Output Validation (deterministic, &lt;5ms)
    {
      ...
      type: 'deterministic',
      action: 'Enforce output schema — block any response containing PII patterns'
    }
  ],
...
**Layer 1 — Encoding Detector and Decoder**:
...
**Layer 3 — Dual-LLM Semantic Firewall** (see Question 2.3 for full detail).
...
interface SecurityIncident {
  ...
  // User context (for forensics — WHO attempted this?)
  user: {
    userId: string
    sessionId: string
    ipAddress: string
...
Your answer must demonstrate understanding of:
- ✅ Encoding-aware input preprocessing (Base64, URL, Unicode normalization)
- ✅ Defense-in-Depth layering (4+ layers, deterministic + probabilistic)
- ✅ Dual-LLM Semantic Firewall (cross-architecture security judge)
- ✅ Structural Output Validation (schema-enforced PII blocking)
- ✅ HIPAA-compliant incident logging and breach assessment
...
|---|---|---|---|
| Full regression (1,000 cases) | 20 min | $50 | Low — catches all regressions |
| Spot check (10 cases) | 30 sec | $0.50 | High — misses tail-case regressions |
| No evaluation | 0 sec | $0 | Critical — flying blind |
...
const EVALUATION_TIERS: EvaluationTier[] = [
  {
    // Tier 1: Fast smoke test — runs on every PR
    name: 'smoke_test',
    ...
    estimatedCost: '$2.50',
    hardStopThresholds: {
      faithfulness: 0.90,          // Lower bar — catches catastrophic failures
      answer_relevance: 0.85,
      toxicity: 0.10               // Must be below 10%
    }
  },
  {
    // Tier 2: Stratified sample — runs on staging deployment
  ...
  {
    // Tier 3: Full regression — runs before production "Green" deployment
...
```
PR Merge → Tier 1 (50 cases, 90s, $2.50) → PASS?
  │                                           │
  │  NO → Block merge, fix regression         │
  │                                           │
  ▼  YES                                      │
Staging Deploy → Tier 2 (200 cases, 5m, $15) → PASS?
  │                                             │
  │  NO → Block staging, investigate           │
  │                                             │
  ▼  YES                                        │
Production Gate → Tier 3 (1000 cases, 20m, $50) → PASS?
  │                                               │
  │  NO → Block production, full audit            │
  │                                               │
  ▼  YES
...
const PRODUCTION_HARD_STOPS: HardStopConfig[] = [
  {
    ...
    threshold: 0.95,
    operator: 'gte',
    overridable: false,          // NEVER override — regulatory requirement
    overrideApprovers: [],
    regulatoryBasis: 'FDA 21 CFR Part 11 — medical device accuracy requirement'
  },
  {
    metric: 'toxicity',
    threshold: 0.02,
    ...
    overridable: false,
    overrideApprovers: [],
    regulatoryBasis: 'HIPAA — patient safety and dignity'
  },
  {
    ...
    overridable: false,
    overrideApprovers: [],
    regulatoryBasis: 'HIPAA Privacy Rule — zero tolerance for PHI leakage'
  },
  {
    ...
    regulatoryBasis: 'Internal quality standard — not regulatory'
  }
]
```
...
**Key Insight**: Regulatory thresholds (faithfulness, toxicity, PII) are **never overridable** — no human has the authority to ship a medical AI that fails FDA compliance. Quality thresholds (relevance) can be overridden with CMO approval and a documented justification.
...
Monthly changes: 8 × 4 = 32 changes

Full Regression Every Time (naive):
  32 changes × $50/run × 20 min = $1,600/month, 640 minutes (10.7 hours)

Tiered Approach:
  Tier 1 (every change): 32 × $2.50 = $80
  Tier 2 (staging, ~50% pass Tier 1): 16 × $15 = $240
  Tier 3 (production, ~75% pass Tier 2): 12 × $50 = $600
  Total: $80 + $240 + $600 = $920/month

  Time: 32 × 1.5min + 16 × 5min + 12 × 20min = 48 + 80 + 240 = 368 min (6.1 hours)

Savings:
  Cost: $1,600 - $920 = $680/month (42.5% reduction)
  Time: 640 - 368 = 272 minutes saved (42.5% faster)
  Safety: Identical — all production deployments pass full regression
```

**Key Insight**: The tiered approach doesn't reduce safety — every production deployment still passes the full 1,000-case regression. It reduces cost and time by **filtering out bad changes early** before they reach the expensive Tier 3 gate. A prompt change that fails Tier 1 (90-second, $2.50 smoke test) never incurs the $50 full regression cost.
...
Your answer must demonstrate understanding of:
- ✅ Tiered evaluation pipeline (smoke test → staging → production gate)
- ✅ Hard-stop thresholds (regulatory vs overridable)
- ✅ Stratified sampling (maximizing regression detection per case)
- ✅ Override policy (non-overridable regulatory gates)
- ✅ Cost and time ROI calculation
...
| Tokens per query | 3,200 | 9,800 | +206% ⚠️ |
| Daily token cost | $86 | $283 | +229% ⚠️ |
| Cache hit rate | 44% | 2% | -95.5% ⚠️ |
| Avg query length | 45 tokens | 180 tokens | +300% ⚠️ |
| Unique query embeddings | 4,200 | 17,800 | +324% ⚠️ |

**Financial Impact**:
- St. Mary's is on the **Professional plan** ($2,500/month)
- Their projected monthly AI cost: $283/day × 30 = **$8,490/month**
...
**Likely Scenarios**:
1. **New specialty onboarded**: St. Mary's added a psychiatric department — mental health queries are completely different from their historical surgical/cardiology queries
...
async function detectOutOfDistribution(
  ...
  return {
    ...
    diagnosis: centroidDrift > 0.15
      ? 'SIGNIFICANT_DRIFT: Query distribution has shifted — likely new use case or integration'
      : oodRate > 0.50
...
Diagnosis: St. Mary's added psychiatric services — their query
...
class TokenBucketThrottler {
  ...
  async checkAndConsume(
    ...
    // Budget exceeded — apply throttle strategy
...
In a medical setting, you **cannot simply block queries** — a clinician may need an answer during patient care. The degradation strategy is tiered:
...
Your answer must demonstrate understanding of:
- ✅ Embedding Centroid Monitoring (OOD detection for query drift)
- ✅ Token-Bucket Quotas (plan-aware throttling with degradation tiers)
- ✅ Proactive cache warming (pre-computing responses for new distributions)
- ✅ Medical-safe degradation (never hard-block clinical queries)
- ✅ Break-even analysis (cost modeling with throttling)
...
**Demonstrates**:
- ✅ Treats **"Security" and "Quality" as code-level constraints**, not afterthoughts
- ✅ Refuses to deploy without a passed regression suite (hard-stop thresholds)
- ✅ Uses **cross-model validation** to avoid shared blind spots (Dual-LLM Firewall)
- ✅ Manages cost through **tenant attribution** and **token-bucket quotas**
- ✅ Detects silent drift through **Semantic Telemetry** and **Shadow Evaluation**
- ✅ Implements **Defense-in-Depth** (encoding detection + semantic firewall + output validation)
- ✅ Calculates ROI for every architectural decision
...
**Gaps**:
- ⚠️ Relies on "trusting the model" or simple regex filters for security
- ⚠️ Views observability as "nice-to-have" rather than a deployment blocker
- ⚠️ No concept of tiered evaluation — either runs full regression or skips it
- ⚠️ Treats cost spikes as billing issues rather than architectural signals
- ⚠️ Missing encoding-aware guardrails (would miss Base64 injection)
...
- "That hospital is just using the system more — charge them extra"
...
- ❌ Suggests "manually reviewing the logs" as a primary security strategy
- ❌ Ignores the economic impact of token spikes ("that's a billing problem, not engineering")
- ❌ No understanding of encoding-based bypass techniques
- ❌ Believes "100% uptime means the system is working"
- ❌ No concept of deployment gates or regression testing for AI
...
- "We don't need to evaluate the prompt — just ship it and monitor"
...
1. ✅ **Challenge 1**: Score 85%+ on Silent Drift Detection
   - Must use Three Pillars diagnostic and Trace-Linked Chunk IDs

2. ✅ **Challenge 2**: Score 85%+ on Base64 Shadow Injection
   - Must show Defense-in-Depth with Dual-LLM Semantic Firewall

3. ✅ **Challenge 3**: Score 85%+ on Teacher-Student Deployment Gate
   - Must design tiered evaluation with hard-stop thresholds

4. ✅ **Challenge 4**: Score 85%+ on Toxic Tenant Triage
...
**Skills Validated**:
- ✅ Observability (Three Pillars, Semantic Telemetry, Shadow Evaluation)
- ✅ Security (Defense-in-Depth, Dual-LLM Firewall, encoding-aware guardrails)
- ✅ Automated Evaluation (tiered pipelines, stratified sampling, hard-stop gates)
- ✅ Unit Economics (tenant attribution, OOD detection, token-bucket quotas)
- ✅ Incident Response (HIPAA-compliant logging, forensic analysis)
- ✅ Deployment Governance (canary testing, granular rollback, override policies)
...
1. **Observability Pillars** — Three Pillars, Semantic Telemetry, Shadow Evaluation Proxy, Tenant-Aware Header Propagation
2. **Production Guardrails** — Defense-in-Depth, Dual-LLM Semantic Firewalls, Zero-Knowledge PII, Optimistic Delivery
3. **Automated Evaluation (LLM-as-a-Judge)** — Cross-Model Evaluation Hierarchy, Golden Datasets, Regression Pipelines
4. **Lab: LLM Judge Pipeline** — Hands-on implementation of Teacher-Student evaluation
5. **Lab: Production Dashboard** — OpenTelemetry setup, cost attribution, alerting
...
You are now equipped to **govern live AI systems** with the same rigor as traditional Tier-1 banking and medical applications — monitoring semantic health, enforcing security at the architecture level, gating deployments with automated evaluation, and managing unit economics at the tenant level.
