 ...
 *
 * Request Flow:
 * User Query → API Gateway → RAG Service → Vector DB → LLM → Response
 *
 * Trace shows:
 * - Total time: 2,340ms
 * - Embedding: 120ms
 * - Vector search: 450ms
 * - Re-ranking: 850ms ← BOTTLENECK
 * - LLM call: 920ms
 */
 ...
 * - Low Recall@K + high latency → Improve search algorithm
 * - High Recall@K + low latency + wrong answer → LLM problem
 * - Low top_similarity → Embedding model outdated
 */
...
```
Trace abc-123:
  ├─ retrieval: 450ms ✅ (looks fine)
  └─ llm_call: 920ms ✅ (looks fine)
...
```
Trace abc-123:
  ├─ retrieval.vector_search: 450ms
  │  ├─ search.recall_at_k: 0.42 ❌ (target >0.80)
  │  ├─ search.top_similarity: 0.58 ❌ (below threshold 0.65)
  │  ├─ search.avg_similarity: 0.51 ❌ (weak matches)
  │  ├─ embedding.model: "voyage-medical-v1.0" ⚠️ (OUTDATED)
  │  └─ search.top_chunk_id: "chunk_456"
  │
  └─ llm_call: 920ms
     └─ llm.model: "claude-3-opus-20240229"

Diagnosis: FOUND IT!
- Recall@K is 0.42 (should be >0.80) → Retrieval problem
- Top similarity 0.58 (below 0.65 threshold) → Weak embedding match
- Embedding model "v1.0" is outdated → Upgrade to "v1.2"
- Chunk ID "chunk_456" is top match → Inspect this document

Action:
1. Upgrade embedding model: v1.0 → v1.2
...
- **MTTR** (Mean Time To Resolution): 2 hours → 5 minutes (-96%)
...
- If Recall@K is low (e.g., 0.42) → Retrieval problem (bad search)
- If top similarity is low (e.g., 0.58) → Embedding mismatch (outdated model)
- If both are high but answer is wrong → LLM problem (hallucination)

This diagnostic metadata reduced our MTTR from 2 hours to 5 minutes—a 96% improvement. The key insight: **Timing alone is not enough. You need quality metrics in every span.**"
...
datadogTracer.trace('llm-call', (span) => {
  span.setTag('dd.model', 'claude-3-opus')  // ← Datadog-specific
  span.setTag('dd.tokens', 1024)            // ← Datadog-specific
  // ... rest of code
})
...
newrelic.startSegment('llm-call', true, () => {
  newrelic.addCustomAttribute('model', 'claude-3-opus')  // ← New Relic-specific
  // ... rest of code
})
```
...
const exporter = new OTLPTraceExporter({
  url: 'https://otlp.nr-data.net/v1/traces',  // ← New Relic endpoint
  headers: {
...
span.setAttributes({
  'gen_ai.request.model': 'claude-3-opus',  // ← Standard attribute
  'gen_ai.usage.total_tokens': 1024         // ← Standard attribute
})
```
...
This means we can send traces to Datadog, New Relic, Jaeger, or any OTLP-compatible backend by changing one config file—no code changes required.
...
**Without OpenTelemetry**:
- Migration effort: 2 weeks × 2 engineers × $150/hour × 40 hours = $24K
- Opportunity cost: 2 weeks delayed
- Risk: Migration bugs, incomplete coverage

**With OpenTelemetry**:
- Migration effort: 1 hour × 1 engineer × $150 = $150
...
**Observation**: In RAG systems, most "hallucinations" are not LLM failures—they're **data quality issues** (wrong chunks retrieved).

**Example**:
- User: "What is the patient's HbA1c?"
- LLM: "The patient's HbA1c is 9.2%" ❌ WRONG
...
async function ragWithContextLinking(
  ...
  try {
    ...
    // Step 3: Store trace → chunk mapping for future debugging
...
/**
 * Store trace → chunk mapping in database
 */
...
async function reportHallucination(traceId: string, userFeedback: string) {
  console.log(`🚨 Hallucination reported for trace ${traceId}`)
...
${chunks.rows.map(c => `- ${c.id}: ${c.content.slice(0, 100)}...`).join('\n')}
  ...
  console.log(`\n✅ Data quality ticket created`)
  console.log(`   Action: Re-index patient_records_august_2024.pdf`)
}
```

**Production Impact**:
- **Before**: Hallucination reported → "Not sure why" → 2 days debugging
- **After**: Hallucination reported → Query trace → Identify chunk → Fix data → 30 minutes

**MTTR**: 2 days → 30 minutes (-99%)

### Interview Defense Template

**Q**: "How do you close the loop between observability and data quality improvement?"

**A**: "We implement **context-linked tracing**—every trace includes the Chunk IDs of retrieved documents. When a user reports a hallucination, we:
...
This reduced our MTTR for hallucinations from 2 days to 30 minutes—a 99% improvement. The key insight: **Observability without context linking is just timing data. Context linking turns traces into a data quality debugging tool.**"
...
- Engineering cost: 2 days × $150/hour × 8 hours × 50 = $120K/month

**After Context Linking**:
- MTTR: 30 minutes per incident (instant trace → chunk inspection)
- Engineering cost: 0.5 hours × $150/hour × 50 = $3.75K/month
...
❌ **Wrong answer** - Logs can't show latency breakdown across services.
...
❌ **Wrong answer** - Metrics can't diagnose why a SPECIFIC request was slow.

---

### Option C: Traces ✅ CORRECT
...
Trace abc-123 (12,340ms total):
  ...
  │  └─ rerank: 130ms ✅
  │
  ├─ query_expansion: 9,200ms ❌ BOTTLENECK!
  │  ├─ expansion.generate: 8,850ms ❌
  │  │  └─ external_api.search: 8,700ms ❌ (waiting on slow API)
  │  └─ expansion.validate: 350ms ✅
  │
  └─ llm_call: 920ms ✅
...
✅ **Correct answer** - Traces are the ONLY way to see internal timing of individual steps inside a single request.
...
**Result**: User tries again → still slow (12 seconds).

**Diagnosis**: Feedback confirms the problem exists, but doesn't identify the root cause.

❌ **Wrong answer** - Feedback identifies THAT there's a problem, not WHERE the problem is.
...
After identifying the bottleneck (query expansion → external API), the team:
...
- [ ] Store trace_id → chunk_ids mapping in database
...
- [ ] Cost per 1K requests >$10 → Alert
- [ ] Recall@K <0.80 → Alert (data quality degradation)
- [ ] Error rate >1% → Alert
...
- [ ] Trace → chunk inspection tool (for debugging)
