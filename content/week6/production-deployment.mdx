...
The fundamentals above prepare you for launch. Now we elevate to **Mission-Critical Infrastructure**—patterns that ensure your AI system survives provider outages, economic attacks, and the pressures of high-scale operations.
...
async function deployCanary(
  ...
  await notifySlack({
    ...
    text: `🟢 Canary deployed: ${newModelId} at ${initialWeight * 100}% traffic`
  })
}
...
async function promoteCanary(): Promise<{
  ...
  await notifySlack({
    channel: '#ai-deployments',
    text: `✅ Canary promoted: ${newBlue.model_id} now serving 100% traffic`
  })
...
async function rollbackCanary(reason: string): Promise<void> {
  ...
  await notifySlack({
    channel: '#ai-deployments',
    text: `🔴 Canary rolled back: ${config.green.model_id}\nReason: ${reason}`
  })
...
// Hour 1:   Monitor - Faithfulness 91% (Blue: 92%) ✅ within threshold
// Hour 2:   Increase Green to 10%
// Hour 4:   Monitor - Faithfulness stable, latency 5% higher ⚠️ acceptable
// Hour 6:   Increase Green to 25%
// Hour 12:  Monitor - All metrics within threshold ✅
...
// User sends: 10 requests × 100K tokens each = 1M tokens
// Cost: $15 in 60 seconds (from a single "power user")
// Daily burn rate if unchecked: $21,600 😱
```
...
// Attack scenario: 100K tokens/request × 10 requests/min = 1M tokens/min
// Cost: $15/min × 60 × 24 = $21,600/day potential exposure

// With token-based limits (10K tokens/min for free):
// Attack capped at: 10K tokens/min × 60 × 24 = 14.4M tokens/day
 ...
 * business logic doesn't know which LLM it's talking to—the orchestration
...
>
> **You:** "We don't run them in parallel—we fail over sequentially. The secondary provider only activates when the primary circuit opens. In practice, failover events are rare (0.04% of requests), so the cost is negligible. But the 4-nines availability improvement is invaluable for user trust. We also use the local Llama as a zero-cost emergency fallback for truly catastrophic scenarios."

---

## 🎯 The "Go-Live Crisis" Diagnostic Quiz
...
**Root Cause Analysis:**
1. ✅ "Rate Limit Exceeded" from provider → API-level issue
2. ⚠️ Only 15% of tier usage → NOT a legitimate scaling problem
3. 🚨 500% token spike from single IP → **Malicious or runaway client**
4. 🚨 95% error rate → Attacker consuming YOUR rate limit allocation

**The Problem:** A single IP is performing an Economic Denial of Service attack. They're sending massive token requests that exhaust your provider's rate limit allocation, causing legitimate requests to fail. This is NOT about your code—it's about resource exhaustion.
...
// → Identify if it's a compromised API key or external attack
...
**Reasoning:** Request count doesn't show intensity. Latency is a symptom, not a cause. Token consumption per IP immediately reveals if one source is disproportionately consuming resources—the hallmark of EDoS.
...
**Reasoning:** Token-bucket rate limiting looks at token consumption, not request count. 5 requests × 2,000 tokens = 10,000 tokens, which exhausts the minute's allocation. The remaining 45 requests will be throttled until the bucket refills.
