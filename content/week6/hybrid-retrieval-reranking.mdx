...
## 🏥 Real-World Challenge: The Clinical RAG System
...
- **Recall**: Must also understand semantic queries ("blood sugar control" → HbA1c results)
...
// ❌ Naive Vector Search (returns semantically similar, not exact)
...
// Returns:
// 1. "HbA1c: 6.8% (March 15, 2022)" ✅ EXACT match
// 2. "Follow-up HbA1c scheduled for March 2022" (related context)
// 3. "Previous HbA1c (Jan 2022): 7.1% → trending down" (trend context)
```
...
User Query: "HbA1c March 2022"
      ↓
┌─────────────────────┐
│   Query Embedding   │ (OpenAI ada-002: &lt;50ms)
└──────────┬──────────┘
           ↓
     ┌─────┴─────┐
     ↓           ↓
...
// Doctor prescribed based on wrong month → Incorrect medication adjustment
// Hybrid RAG caught: Keyword search filtered out "April", returned only "March"
// → Correct treatment decision
```

**[👉 Lab: Build the Clinical RAG System](/curriculum/week-6/labs/advanced-rag-system)**
...
4. A/B testing: Measure precision improvement (65% → 94%)
5. Cost optimization: Intelligent model routing (Haiku for simple, Opus for complex)

**Key Architectural Insight**: Medical/legal/financial RAG systems **must use hybrid search**—vector-only is insufficient for domains requiring exact match precision.
...
// ❌ Vector-Only RAG (65% precision - dangerous!)
const vectorResults = await vectorSearch("metformin renal impairment starting dose")
// Returns:
// 1. "Metformin should be used with caution in renal impairment" ← Generic warning, no dose
// 2. "Initial dose: 500mg twice daily" ← For normal renal function (WRONG!)
// 3. "Adjust dose based on eGFR &lt;30 mL/min" ← Vague, no specific dose

// ✅ Hybrid Search + Reranking (94% precision)
...
// Returns:
// 1. "For eGFR 30-45 mL/min: Starting dose 500mg once daily, max 1000mg/day" ← EXACT match ✅
// 2. "Contraindicated if eGFR &lt;30 mL/min" ← Critical safety info ✅
// 3. "Monitor renal function every 3 months" ← Clinical guidance ✅
```
...
async function hybridSearch(
  ...
  console.log(`🔍 Stage 1: Hybrid Search (${options.limit} candidates)`)
...
async function rerankWithCrossEncoder(
  ...
  console.log(`\n🎯 Stage 2: Cross-Encoder Reranking (top ${topN})`)
  ...
  console.log(`   Reranked ${candidates.length} → ${reranked.results.length} results`)
...
export async function queryFDADrugLabels(
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`🏥 FDA DRUG LABEL QUERY SYSTEM`)
...
- Total: $10,200/month
- Budget: $15,000/month
- **Status**: ✅ Within budget, 32% headroom

**ROI Analysis**:
- Development cost: $45K (2 engineers × 3 weeks)
...
- "metformin 850mg once daily" ← Wrong dose!
- "metformin XR 1000mg daily" ← Wrong formulation!
...
Query 2: "Initial dose: 1000mg daily total"  ← Same info, different phrasing (confusing!)

// Temperature 0.0 (deterministic):
Query 1: "Starting dose: 500mg twice daily with meals"
Query 2: "Starting dose: 500mg twice daily with meals"  ← Identical phrasing (reliable!)
...
**✅ Hybrid + Reranking** (Production Choice):
...
```typescript
// ❌ BAD: Assumes scores are normalized and comparable
...
```typescript
// ❌ ANTI-PATTERN: Weighted Average (Mathematically Unsound)
...
**RRF Formula**:
```
RRF_score(doc_d) = Σ_{r ∈ R} 1 / (k + rank_r(doc_d))
```
...
RRF(doc_12) = 1/(60+1) + 1/(60+1)
            = 0.01639 + 0.01639
            = 0.03278  ← HIGHEST (appears in both rankings!)
...
**Final Ranking**:
1. doc_12 (RRF: 0.03278) ← Winner (appeared in both rankings)
...
|--------|----------------------|------------------|----------------------|
| Weighted Average | ✅ Yes (must tune weights) | ✅ Yes (weights change per dataset) | ❌ No (unit mismatch) |
| Min-Max Normalization | ⚠️ Somewhat (must set ranges) | ✅ Yes (ranges change) | ⚠️ Fragile (outliers break) |
| Z-Score Normalization | ⚠️ Somewhat (assumes Gaussian) | ✅ Yes (mean/std change) | ⚠️ Assumes normal distribution |
| **RRF** | ✅ **No tuning needed** | ✅ **No (rank-based)** | ✅ **Yes (distribution-agnostic)** |

**Interview Defense**:
> "I use Reciprocal Rank Fusion (RRF) instead of weighted averages because vector scores (0-1) and BM25 scores (0-15) are different units—you can't mathematically add them. RRF only cares about rank position, making it normalization-independent and eliminating manual weight tuning. In production, this prevented the keyword scores from dominating vector scores unfairly."

**Production Incident Prevented**:
- **Before** (weighted average): Keyword scores dominated → 73% precision
- **After** (RRF): Consensus-based ranking → 91% precision
...
RRF scores (k=60):
- doc_12: 1/(60+1) + 1/(60+1) = **0.0328** ← Appears in both (BOOSTED!)
...
- Doc B: "HbA1c declined from 8.1 → 7.2 over 6 months" (trend, no "glucose")
...
function selectSearchStrategy(analysis: QueryAnalysis): 'pure_vector' | 'hybrid' | 'pure_keyword' {
  // Rule 1: Identifiers → Hybrid (favor keyword)
  if (analysis.hasIdentifiers || analysis.hasExactTerms) {
    return 'hybrid'
  }

  // Rule 2: Date ranges → Hybrid
  if (analysis.hasDateRange) {
    return 'hybrid'
  }

  // Rule 3: High semantic complexity → Pure vector
...
|--------|------------|------------|-------------|---------|
| **Recall@10** | 0.72 | **0.88** | **+22%** | &lt;0.001 ✅ |
| **MRR** | 0.65 | **0.81** | **+25%** | &lt;0.001 ✅ |
| **Precision@5** | 0.68 | **0.87** | **+28%** | &lt;0.001 ✅ |
| **Latency P95** | 120ms | 230ms | +92% ⚠️ | &lt;0.001 |
| **Cost/Query** | $0.0001 | $0.0022 | +2100% ⚠️ | - |
...
// RAG System:
Retrieved Docs: ✅ Correct (Recall@5 = 1.0)
  - Doc 1: "eGFR 30-44: max 1000mg" ✅
  - Doc 2: "eGFR 45-59: max 2000mg" ✅

LLM Answer: ❌ HALLUCINATION
  "For eGFR 45, start at 850mg twice daily"  ← NOT in retrieved docs!
...
// Verification:
Claim 1: ❌ NO (context says "30-44", query is 45)
Claim 2: ❌ NO (max 1000mg is for eGFR 30-44, not 45)
Claim 3: ⚠️ PARTIAL (context says "3-6 months", answer says "3 months")

// Faithfulness Score: 0/3 = 0.0  ← HALLUCINATION DETECTED!
```
...
if (faithfulness.score < 0.9) {
  await alertSlack({
    channel: '#ai-safety',
    text: `⚠️ LOW FAITHFULNESS (${(faithfulness.score * 100).toFixed(1)}%)\n` +
...
const PRODUCTION_THRESHOLDS = {
  ...
  faithfulness: 0.90,         // ← CRITICAL: 90% of claims must be supported
...
async function monitorProductionMetrics(metrics: ProductionRAGMetrics) {
  ...
  if (metrics.faithfulness < PRODUCTION_THRESHOLDS.faithfulness) {
    violations.push(
      `⚠️ CRITICAL: Faithfulness ${(metrics.faithfulness * 100).toFixed(1)}% ` +
  ...
  if (metrics.recall_at_10 < PRODUCTION_THRESHOLDS.recall_at_10) {
    violations.push(
      `⚠️ Recall@10 ${(metrics.recall_at_10 * 100).toFixed(1)}% ` +
...
**Interview Defense**:
> "I use RAGAS metrics (Faithfulness, Answer Relevancy) in addition to traditional retrieval metrics (Recall@K, MRR). Recall@K tells me if I found the right document, but Faithfulness tells me if the LLM hallucinated. In production, I maintain Faithfulness >90%—if it drops below that threshold, it triggers an alert because the system is generating unsupported claims. This prevented a medical dosing hallucination incident where traditional metrics showed 100% recall but the answer was factually wrong."

**Production Impact**:
- **Before** (Recall@10 only): 100% retrieval score, but 23% of answers had hallucinations
- **After** (Recall@10 + Faithfulness): 91% Faithfulness → 9% hallucination rate
...
async function continuousEvaluation() {
  ...
  setInterval(async () => {
    ...
    if (avgRecall &lt; 0.75) {
      await alertSlack({
        channel: '#ai-alerts',
        text: `⚠️ Recall dropped to ${(avgRecall * 100).toFixed(1)}% (target: &gt;75%)`
      })
    }
...
|--------|----------------------|-------------------|--------|--------|
| Recall@10 | 0.72 | **0.88** | &gt;0.80 | ✅ |
| MRR | 0.65 | **0.82** | &gt;0.75 | ✅ |
| Precision@5 | 0.68 | **0.91** | &gt;0.80 | ✅ |
| NDCG@10 | 0.70 | **0.89** | &gt;0.80 | ✅ |
| Latency P95 | 120ms | 230ms | &lt;300ms | ✅ |
| Cost/1K Queries | $0.10 | $2.20 | &lt;$5.00 | ✅ |
...
8. "Elderly patients (≥65): Start 2.5-5mg daily due to decreased renal function" ← EXACT MATCH
...
**Why This Fails**:
- ❌ **Worsens "Lost in the Middle"**: More documents = more noise
- ❌ **Increases latency**: 20 documents → +150ms retrieval + re-ranking time
- ❌ **Higher cost**: 2x context tokens → $0.012/query (vs $0.006 target)
- ❌ **Doesn't solve root cause**: Vector search is too "fuzzy" to distinguish age-specific dosing from general dosing
...
### Option B: Implement a Cross-Encoder Re-ranker ✅ CORRECT
...
// Result:
// 1. "Elderly patients (≥65): Start 2.5-5mg daily" ← NOW RANK #1!
// 2. "Renal impairment: reduce dose"
// 3. "Monitor: BP, creatinine, potassium"
// 4. "Standard adult dose: 10-40mg" ← De-ranked (not age-specific)
// 5. "Titrate every 1-2 weeks"
```

**Why This Works**:
- ✅ **Query-Document Interaction**: Cross-encoder sees '65yo' in query + 'Elderly ≥65' in doc → high relevance score
- ✅ **Age-Specific Ranking**: Distinguishes "elderly dosing" from "standard adult dosing"
- ✅ **Precision**: Document #8 moves from rank 8 → rank 1
- ✅ **LLM Focus**: Top 5 documents are all age-relevant → correct answer
...
**Interview Defense**:
> "A cross-encoder re-ranker is essential for clinical precision because bi-encoder vector search is too fuzzy. It can't distinguish between 'general adult dosing' and 'elderly-specific dosing' because both documents are semantically similar. The cross-encoder encodes [query + document] together, allowing it to see the '65yo' in the query and the '≥65' age threshold in the document, moving age-specific guidance to rank #1."
...
**Why This Fails**:
- ❌ **User Experience**: Doctors won't use a system that requires perfect queries
- ❌ **Not Scalable**: Can't train every physician on prompt engineering
- ❌ **Still Fails**: Even with a longer query, bi-encoder vector search has limited precision
- ❌ **Architectural Cop-Out**: An Architect doesn't blame the user—they fix the system
...
**Why This Fails**:
- ❌ **"Lost in the Middle"**: LLMs statistically ignore mid-ranked documents (positions 5-8)
- ❌ **Doesn't Improve Ranking**: Document #8 is still in the middle → still ignored
- ❌ **Cost**: Sending 10 documents costs 2x vs 5 documents ($0.012 vs $0.006/query)
- ❌ **Latency**: More context → slower generation (+300ms)
...
|--------|------------|-------------|-------------------------|
| Precision@5 (age-specific) | 67% | 78% | **94%** ✅ |
| Recall@10 | 72% | 88% | **91%** ✅ |
| "Lost in Middle" errors | 31% | 18% | **8%** ✅ |
| Latency P95 | 180ms | 220ms | **300ms** ⚠️ |
| Cost/query | $0.006 | $0.008 | **$0.012** ⚠️ |
| **Physician Satisfaction** | 68% | 79% | **91%** ✅ |
...
**4. Larger Context ≠ Better Answers**
