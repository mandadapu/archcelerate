...
2. **Two-Stage Retrieval:** Fast hybrid search (top 100, 92% recall) → Slow cross-encoder reranking (top 5, 91% precision) = 230ms total latency
...
4. **Parent-Document Retrieval:** Search small chunks (precision) → Retrieve large parents (context) to solve fragmentation
...
In the lab, you'll build a Medical Records Navigator that uses all three techniques to find "HbA1c result from March 2022" in 5,000 pages—with 91% precision in &lt;230ms.
