...
**Enterprise Monitoring**: "Detect semantic drift 3 days before user satisfaction drops. P99 latency diverging from P50 by 8x signals vector DB index degradation. Support bot unit economics broken at $50 cost per $5 ticket—implement context pruning."
...
- **Day 5**: User satisfaction drops from 4.2 → 3.8 (NOW you notice)
...
async function calculateDailyCentroid(date: Date): Promise<EmbeddingCentroid> {
  ...
  console.log(`📊 Calculating centroid for ${date.toISOString().split('T')[0]}`)
...
async function detectSemanticDrift(): Promise<{
  ...
  if (cosineDist < 0.01) {
    ...
    interpretation = `⚠️  SIGNIFICANT DRIFT DETECTED: ${(cosineDist * 100).toFixed(1)}% shift in output semantics`
    driftDetected = true
  }
...
Possible causes:
1. Provider updated model (e.g., GPT-4-turbo → GPT-4-turbo-2024-01-15)
...
Thursday: User satisfaction drops from 4.2 → 3.9 (you notice but don't know why)
...
Tuesday 9am: Team reviews → finds OpenAI release notes (GPT-4-turbo-2024-01-15)
Tuesday 10am: Test new model → outputs 30% longer but same quality
...
**Production Impact**:
- **MTTR**: 5 days → 10 hours (-96%)
- **Engineering cost**: $4,800 → $200 (-96%)
...
If the distance exceeds 5%, we alert immediately—this is a leading indicator that the model is behaving differently (more verbose, different tone, etc.).
...
- User satisfaction: 4.2 → 3.8
- Churn rate: 2% → 5% (300 users lost)
- Revenue loss: 300 × $50 × 6 months (avg lifetime) = $90K
...
**Example**:
- **P50 (median)**: 1.8s ✅ (50% of requests faster than this)
- **P95**: 3.2s ✅ (95% of requests faster than this)
- **P99**: 42s ❌ (1% of requests slower than this)
...
async function calculatePercentileMetrics(
  ...
  console.log(`📊 Percentile Analysis (${windowMinutes}m window):`)
...
async function detectP99Divergence() {
  ...
  if (metrics.p99_p50_ratio < HEALTHY_RATIO) {
    ...
    status = 'warning'
    diagnosis = `⚠️  P99 divergence detected: ${metrics.p99_p50_ratio.toFixed(1)}x ratio
...
Action: Investigate slow queries (P99 percentile)`
    ...
    status = 'critical'
    diagnosis = `🚨 CRITICAL P99 DIVERGENCE: ${metrics.p99_p50_ratio.toFixed(1)}x ratio
...
4. Review recent code deployments`
  ...
  if (status === 'critical') {
    await slack.send({
      channel: '#ai-alerts',
      text: `🚨 CRITICAL P99 LATENCY DIVERGENCE

P99/P50 Ratio: ${metrics.p99_p50_ratio.toFixed(1)}x (threshold: 10x)

P50: ${metrics.p50_latency_ms.toFixed(0)}ms ✅
P99: ${metrics.p99_latency_ms.toFixed(0)}ms ❌
...
async function diagnoseP99Bottleneck() {
  console.log(`🔍 Diagnosing P99 bottleneck...\n`)
  ...
  console.log(`📊 P99 Query Characteristics:`)
...
```
Week 2 Day 3: P99/P50 ratio hits 6x → Alert triggered
Week 2 Day 3: Team investigates → finds pgvector VACUUM needed
Week 2 Day 3: Run VACUUM ANALYZE → P99 returns to 2.4s
...
We alert when the P99/P50 ratio exceeds 10x—this signals a tail latency problem like vector DB index degradation or token-length explosion in edge cases.
...
- LTV: $50/month × 12 months = $600
- Revenue loss: 150 × $600 = $90K
...
1. Use a smaller model (GPT-4 → GPT-3.5)
...
async function calculateFeatureEconomics(
  ...
  console.log(`\n💰 Feature Economics (${dateRange.start.toISOString().split('T')[0]} to ${dateRange.end.toISOString().split('T')[0]}):`)
  console.log(`\n${'Feature'.padEnd(20)} | ${'Requests'.padEnd(10)} | ${'Revenue/1K Tokens'.padEnd(18)} | ${'Margin'.padEnd(8)} | Status`)
  console.log(`${'─'.repeat(20)}─┼─${'─'.repeat(10)}─┼─${'─'.repeat(18)}─┼─${'─'.repeat(8)}─┼─${'─'.repeat(12)}`)

  economics.forEach(e => {
    const status = e.is_profitable ? '✅ Profitable' : '❌ UNPROFITABLE'
    console.log(
...
async function alertOnUnprofitableFeatures() {
  ...
  if (unprofitableFeatures.length > 0) {
    await slack.send({
      ...
      text: `⚠️  UNPROFITABLE FEATURES DETECTED
...
1. Use smaller model (GPT-4 → GPT-3.5)
2. Implement context pruning
...
- Cost: $1.50 per ticket (50K × $0.03/1K)
- Revenue: $5 per ticket (deflected from human agent)
- Profit margin: ($5 - $1.50) / $5 = 70% ✅ (looks profitable)
...
Real profit margin: ($0.50 - $1.50) / $0.50 = -200% ❌ LOSING MONEY
```
...
2. Implemented context pruning (50K → 10K tokens avg)
3. New cost: 10K × $0.003/1K = $0.03 per ticket

New margin: ($0.50 - $0.03) / $0.50 = 94% ✅ PROFITABLE
```

**Production Impact**:
- Monthly cost: $15K → $300 (-98%)
...
We fixed it by switching to GPT-3.5 and pruning context (50K → 10K tokens). Cost dropped 98%, margin went from -200% to +94%.
...
❌ **Wrong answer** - Price hasn't changed (confirmed by "cost per token stable").

---

### Option B: Tokens per Request (Context Density) ✅ CORRECT
...
------------+--------------------+----------------
2024-03     | 8,200              | 100,000  ← CURRENT (cost: $24.6K)
2024-02     | 4,100              | 91,000   ← LAST MONTH (cost: $11.2K)
2024-01     | 4,000              | 88,000
```

**Diagnosis**: **FOUND IT!**
- Traffic grew 10% (91K → 100K requests)
- **Tokens per request DOUBLED** (4.1K → 8.2K)
- Cost formula: `Requests × Tokens/Request × Cost/Token`
  - Before: 91K × 4.1K × $0.03/1K = $11.2K
  - After: 100K × 8.2K × $0.03/1K = $24.6K
...
← FOUND IT! Conversation history is 3.2K tokens (was 800 tokens last month)
```
...
**Result**:
- Tokens per request: 8.2K → 4.5K (-45%)
- Monthly cost: $24.6K → $13.5K (-45%)
- User experience: Unchanged (10 turns is plenty of context)

✅ **Correct answer** - Most cost spikes are caused by context leaks (over-retrieval, unbounded history, prompt bloat).
...
- **User satisfaction**: 4.2 → 3.1 (users complain about wrong answers)
- **Churn rate**: 2% → 8% (300% increase)

**Analysis**: Switching models without diagnosing the root cause is like treating symptoms, not the disease.

❌ **Wrong answer** - Should diagnose first, then optimize. Context leak is the root cause.
...
❌ **Wrong answer** - Traffic growth doesn't explain a 2x cost increase.
...
**The Cost Formula**:
```
Total Cost = Requests × (Tokens/Request) × (Cost/Token)
```
...
1. **RAG Over-Retrieval**:
   ...
   - After: 10 chunks × 500 tokens = 5,000 tokens
   - **Cause**: Someone changed `limit: 5` → `limit: 10`

2. **Conversation Memory Unbounded**:
   - Before: 10 turns × 80 tokens/message = 800 tokens
   - After: 40 turns × 80 tokens/message = 3,200 tokens
...
3. **Tokens/Request** (context density) ← This is the leak detector
4. **Cost/Token** (provider pricing)

**Result**:
- **MTTR for cost spikes**: 2 days → 30 minutes (with Tokens/Request dashboard)
