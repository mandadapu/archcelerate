...
const subQuestions = [
  ...
  "How do they compare?"  ← ❌ PROBLEM: Cannot answer without Step 1 & 2!
]
...
// Result:
// - Q1: "$12.5M" ✅
// - Q2: "$15M industry average" ✅
// - Q3: "Unable to determine comparison without specific revenue figures" ❌
//   → Step 3 didn't have access to answers from Steps 1 & 2!
```
...
async function executeDAG(subQuestions: SubQuestion[]): Promise<Map<string, string>> {
  ...
  console.log('📊 DAG Execution Order:\n')
  ...
  for (const questionId of executionOrder) {
    ...
    console.log(`\n🔍 Executing: ${node.subQuestion.question}`)
    ...
    const answer = await retrieveAndAnswer(
      ...
      dependencyContext  // ← KEY: Dependencies in context
    )
...
const subQuestions = [
  ...
  },
  {
    ...
    dependencies: ['q1', 'q2']  // ← DEPENDS on both Q1 and Q2
  }
]
...
Step 3: Execute q3 (depends on q1, q2)
  ...
  ✅ Correct! Step 3 had access to answers from Steps 1 & 2
```
...
- Result: "Cannot compare without specific revenue figures" ❌
...
|--------|---------------|---------------|
| Comparison queries answered | 42% | **94%** ✅ |
| Timeline queries answered | 31% | **89%** ✅ |
| Avg sub-questions executed | 3.2 | 3.2 |
| Execution time | 2.1s (parallel) | 4.8s (sequential) ⚠️ |
| **Accuracy** | 38% | **91%** ✅ |
...
// → Dangerous hallucinated retrieval
```
...
async function hydeWithGuardrails(
  ...
  console.log('🔬 HyDE Step 1: Generate hypothetical document...')
...
Write a paragraph (100-200 words) with specific terminology and structure.
  ...
  console.log('\n🛡️  HyDE Step 2: Ground Truth Verification...')
  ...
  if (maxSimilarity < config.similarityThreshold) {
    console.warn(`   ⚠️  HyDE GUARDRAIL TRIGGERED: Similarity too low!`)
    console.warn(`   Hypothetical document may be hallucinated/irrelevant`)
    
    if (config.fallbackToOriginal) {
      console.log(`   ↩️  Falling back to original user query\n`)
  ...
  console.log(`   ✅ HyDE verified: Proceeding with hypothetical embedding\n`)
...
const fakeAnswer = "Lisinopril and enalapril (ACE inhibitors)..." // ❌ WRONG
...
// Guardrail triggers: 0.58 < 0.65 ❌

console.log('⚠️  HyDE GUARDRAIL: Similarity too low (58% < 65%)')
...
// ✅ No ACE inhibitors (contraindicated)
```
...
```typescript
// ❌ ANTI-PATTERN: Naive Concatenation
const allResults = [
...
async function multiQueryWithRRF(
  ...
  console.log('📝 Step 1: Generate query variations...')
  ...
  console.log(`\n🔍 Step 2: Parallel retrieval (${config.docsPerQuery} docs per query)...`)
...
Doc A: Appears at rank 1 in ALL 3 queries
  RRF = 1/(60+1) + 1/(60+1) + 1/(60+1)
      ...
      = 0.04918  ← HIGHEST (consensus winner!)
...
// Final Ranking:
1. Doc A (RRF: 0.04918) ← Meningitis (appeared in ALL 3 queries)
2. Doc B (RRF: 0.03200) ← Migraine
3. Doc D (RRF: 0.03200) ← Subarachnoid hemorrhage
4. Doc E (RRF: 0.01613) ← Encephalitis
5. Doc C (RRF: 0.01587) ← Influenza
```

**Why RRF Wins**:
- **Doc A** (Meningitis) appeared at **rank #1 in all 3 queries** → RRF score 3x higher
...
|--------|-------------------|------------|
| Total docs sent to LLM | 15 (3 queries × 5 docs) | 5 (top after fusion) |
...
**Production Impact**:
- **Before** (concatenation): 15 docs → 78% precision, $0.018/query
- **After** (RRF fusion): 5 docs → 91% precision, $0.009/query
- **Improvement**: +17% precision, -50% cost

**Interview Defense**:
> "I use Reciprocal Rank Fusion (RRF) to aggregate multi-query expansion results. Instead of concatenating all 15 documents (3 queries × 5 docs), I use RRF to 'vote' on the most relevant docs. A document that appears at rank #2 in all 3 queries gets a higher RRF score than one that's #1 in only one query. This eliminates context dilution, reduces cost by 50%, and improves precision by 17%."
...
// Result: No relevant statutes found ❌
```
...
**Why This Fails**:
- ❌ **Still plain language**: All variations are in layperson terms, not legalese
- ❌ **Terminology gap persists**: "kick me out" vs "unlawful detainer" still no match
- ❌ **5x retrieval cost**: $0.005 → $0.025/query
- ❌ **Doesn't bridge semantic gap**: More queries ≠ better terminology

**Production Result**: Precision@5 improves to 47% (+9%) but still poor

---

### Option B: HyDE (Hypothetical Document Embeddings) ✅ **CORRECT ANSWER**
...
**Why This Works**:
- ✅ **Terminology translation**: Plain language → Legal jargon
- ✅ **Semantic bridge**: LLM knows both languages (layperson + legalese)
- ✅ **Embedding match**: "unlawful detainer" in fake statute matches real statutes
- ✅ **Cost efficient**: Single LLM call ($0.003) + 1 embedding ($0.0001)
...
**Why This Fails**:
- ❌ **Terminology gap too wide**: No embedding model bridges "kick out" → "unlawful detainer" well
- ❌ **Higher cost**: text-embedding-3-large costs 5x more ($0.00013 vs $0.00002)
- ❌ **Marginal improvement**: Precision@5 improves to 44% (only +6%)
- ❌ **Doesn't solve root cause**: Still searching with layperson language
...
**Why This Fails**:
- ❌ **Terminology gap persists**: Smaller plain language pieces still don't match legalese
- ❌ **More retrieval calls**: 3 sub-questions = 3x cost
- ❌ **Doesn't bridge domain gap**: Breaking it up doesn't add legal terminology
...
| Gap | Solution |
|-----|----------|
| Layperson → Legal jargon | HyDE generates legal language |
| "kick me out" → "unlawful detainer" | LLM knows both vocabularies |
| Plain language → Technical terminology | Hypothetical doc uses expert terms |

**Production Metrics**:

| Metric | Plain Language | Multi-Query | Larger Embedding | HyDE |
|--------|---------------|------------|-----------------|------|
| Precision@5 | 38% | 47% (+9%) | 44% (+6%) | **89%** (+51%) ✅ |
| Recall@10 | 52% | 61% (+9%) | 56% (+4%) | **91%** (+39%) ✅ |
| Cost/query | $0.005 | $0.025 (5x) | $0.008 (1.6x) | **$0.008** ✅ |
| User satisfaction | 51% | 58% | 54% | **92%** ✅ |

**ROI Analysis**:
- Manual Westlaw searches: 62% of queries → 38% after HyDE (-39%)
- Lawyer time saved: 15 min/query × 1,000 queries/month × $300/hour × 39% reduction
- **Annual savings**: $351K/year
- HyDE cost: $0.003 extra per query × 12K queries/month = $36/month
...
**3. Multi-Query Expansion ≠ Terminology Translation**
- More plain language queries ≠ Legal terminology
- Volume doesn't solve vocabulary mismatch

**4. Domain-Specific RAG Requires Specialized Patterns**
- Medical: HyDE (layperson → medical terminology)
- Legal: HyDE (plain language → legalese)
- Technical: HyDE (business speak → engineering jargon)

**5. Production Decision Framework**:
- **Terminology gap** (layperson → expert): Use **HyDE**
