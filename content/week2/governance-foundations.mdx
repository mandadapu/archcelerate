...
**Without governance**:
- 💸 Unexpected $10K API bills
- 🚨 Content policy violations in production
- 🤷 No idea why the AI misbehaved
- ⚖️ Compliance violations, legal risks
- 📉 Poor user experience from abuse

**With governance**:
- ✅ Predictable, controlled costs
- ✅ Safe, compliant outputs
...
### Try It Yourself: Input Validation

<CodePlayground
  title="Interactive Input Validation Demo"
  description="Click 'Run Code' to see Zod schema validation in action with various test inputs"
  exerciseType="input-validation"
  code={`// This example demonstrates Zod validation
// Try modifying the test cases to see different validation errors
...
// Test different inputs to see validation results`}
/>
...
<CodePlayground
  ...
  description="Click 'Run Code' to see Claude Haiku classify content for policy violations"
  exerciseType="content-moderation"
  code={`// This example demonstrates content moderation
// Claude analyzes messages for policy violations
...
// Run to see classification results for each message`}
/>
