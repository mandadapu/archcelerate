...
## 🏥 Scenario: MediShield AI Platform
...
## 🛡️ Challenge 1: The Zero-Trust Perimeter
...
**Technical Constraint**: The LLM must maintain **"Contextual Integrity"**—it needs to reason about the patient's history across multiple messages without ever seeing their real name, SSN, or other PHI.

**Example Failure Case**:
```
Message 1: "Patient John Doe has diabetes and high blood pressure"
→ Redacted: "Patient [REDACTED] has diabetes and high blood pressure"

Message 2: "What's John Doe's current medication coverage?"
→ Redacted: "What's [REDACTED]'s current medication coverage?"

LLM Response: "I don't have information about this patient. Could you provide their name?"
```

The LLM doesn't know that `[REDACTED]` in message 2 is the same patient as message 1—**context is lost**.
...
## 🛡️ Challenge 2: The "Constitutional" Conflict
...
**CEO's Requirement**: "Don't just block users—it creates terrible UX. Can we salvage the interaction by correcting the request?"
...
3. **Dual-Pass Flow**: Describe the two-pass architecture:
   - **Pass 1**: Safety Critic detects violation → generates a "safe rewrite" of the user's query
   - **Pass 2**: Main LLM responds to the rewritten query (not the original)
   - Example: "What dosage of sumatriptan..." → "What billing codes are typically associated with migraine treatment?"
...
## 📊 Challenge 3: The Forensic Audit
...
**Stakes**:
- Potential fine: $50,000 per violation × 2,000 affected cases = **$100M**
...
4. **Bias Testing Logs**: What pre-deployment bias tests are logged?
   ...
   - Disparate Impact Analysis: Did you measure the 80% rule (selection rate for protected group ≥ 80% of highest group)?
...
## 📉 Challenge 4: The SLA "Death Spiral"
...
- **Queueing Delay**: 22.7s (P99) ← **The problem**

**Root Cause**: During peak load, requests queue behind slow cases. The 22.7s queueing delay is caused by **head-of-line blocking**—a few complex cases (5% of traffic) take 10-15 seconds, blocking all subsequent requests.
...
2. **Parallel Safety Scans**: Explain how you parallelize safety checks:
   - Current flow: `Redact → Safety Proxy → LLM → Response` (sequential)
   - Proposed flow: `Redact → [Safety Proxy + LLM] in parallel → Merge results`
   - How do you handle the case where the LLM finishes before Safety Proxy? (Do you wait or stream?)
   - What latency savings do you achieve? (600ms sequential → 0ms if parallelized correctly)

3. **Latency Budget Per Layer**: Allocate a latency budget for each layer:
   - Redaction: Budget 100ms (down from 400ms) → How? (Model optimization, caching, batching)
   - Safety Proxy: Budget 200ms (down from 600ms) → How? (Lighter model, rule-based fast path)
   - LLM Inference: Budget 800ms (fixed) → How do you optimize? (Prompt caching, batching)
...
## 🏁 Grading Rubric: The Director's Verdict
...
### ⚠️ Developer Tier (Partial Credit)
...
**Why This Fails**:
- **Misconception #1**: "Fair model" doesn't prove **your system** is fair (model ≠ system)
- **Misconception #2**: Deleting logs **destroys evidence** needed for defense (violates HIPAA § 164.316)
...
> **To Hiring Managers**: A candidate who passes this exam doesn't just "build with AI"—they can **govern and scale AI in the most restricted environments on Earth**.
>
> They understand:
