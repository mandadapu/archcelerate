...
### The "Early Exit" Pattern — Confidence-Based Re-ranker Skip

The re-ranker is your most expensive step (~160ms, 53% of pipeline latency). But you don't always need it. When Stage 1 retrieval is already highly confident, the re-ranker is a tax you pay for no benefit.

**Architect's Tip**: *"Monitor the Similarity Delta of your Stage 1 results. If your #1 result has a similarity score of 0.95 and the #2 result is 0.40, your Stage 1 search is already highly precise. Skip the re-ranker and go straight to the LLM. This 'Early Exit' strategy can cut your P50 latency by 50% without sacrificing accuracy."*

```typescript
interface EarlyExitConfig {
  similarityDelta: number     // e.g., 0.4 — gap between #1 and #2
  absoluteConfidence: number  // e.g., 0.85 — #1 score must be above this
}

function shouldSkipReranker(
  stage1Results: RankedDocument[],
  config: EarlyExitConfig = { similarityDelta: 0.4, absoluteConfidence: 0.85 }
): { skip: boolean; reason: string; savedLatencyMs: number } {
  if (stage1Results.length < 2) {
    return { skip: true, reason: 'Only 1 candidate', savedLatencyMs: 160 }
  }

  const top1 = stage1Results[0].score
  const top2 = stage1Results[1].score
  const delta = top1 - top2

  // Early exit condition: high confidence + large gap
  if (top1 >= config.absoluteConfidence && delta >= config.similarityDelta) {
    return {
      skip: true,
      reason: `High confidence: #1 (${top1.toFixed(3)}) leads #2 (${top2.toFixed(3)}) ` +
        `by ${delta.toFixed(3)} — re-ranker would not change order`,
      savedLatencyMs: 160
    }
  }
  ...
  return {
    skip: false,
    reason: `Ambiguous: #1 (${top1.toFixed(3)}) leads #2 (${top2.toFixed(3)}) ` +
      `by only ${delta.toFixed(3)} — re-ranking required`,
    savedLatencyMs: 0
  }
}

// Integrate into precision pipeline
async function precisionRetrievalWithEarlyExit(
  query: string,
  candidates: RankedDocument[]
): Promise<{ results: RankedDocument[]; reranked: boolean; latencyMs: number }> {
  const start = Date.now()

  // Check early exit condition BEFORE paying the re-ranker tax
  const exit = shouldSkipReranker(candidates)
  console.log(`Early exit check: ${exit.reason}`)

  if (exit.skip) {
    // Skip re-ranker — apply threshold directly to Stage 1 results
    const { documents } = applySemanticThreshold(candidates)
    return {
      results: documents,
      reranked: false,
      latencyMs: Date.now() - start  // ~5ms (threshold only)
    }
  }

  // Ambiguous results — pay the re-ranker tax
  const reranked = await rerank(query, candidates.slice(0, 15))
  const { documents } = applySemanticThreshold(reranked)
  return {
    results: documents,
    reranked: true,
    latencyMs: Date.now() - start  // ~160-180ms (rerank + threshold)
  }
}
```

**Production Impact of Early Exit**:

| Query Type | #1 Score | #2 Score | Delta | Re-rank? | Pipeline Latency |
|---|---|---|---|---|---|
| "RFC 6585" (exact ID) | 0.96 | 0.31 | 0.65 | ❌ Skip | 140ms (saved 160ms) |
| "rate limiting best practices" | 0.82 | 0.79 | 0.03 | ✅ Rerank | 300ms |
| "HIPAA compliance checklist" | 0.91 | 0.42 | 0.49 | ❌ Skip | 140ms (saved 160ms) |
| "how to optimize performance" | 0.74 | 0.71 | 0.03 | ✅ Rerank | 300ms |

In production, **40-60% of queries** have a clear top-1 winner and trigger early exit — cutting P50 latency from 300ms to 140ms with zero accuracy loss.
...
- **Early Exit**: Skip the re-ranker when Stage 1 confidence is high (#1 score &gt;0.85 AND delta &gt;0.4 to #2) — saves 160ms on 40-60% of queries
