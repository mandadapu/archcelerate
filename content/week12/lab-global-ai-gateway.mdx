...
External Apps
  ...
  └─ CLI Tool
   ...
   [Your Gateway]
         ...
         ├─ Multi-Region Routing (us-east, us-west, eu-west)
         ├─ Provider Abstraction (Anthropic, OpenAI, local)
         ├─ Cost Attribution (by user, dept, project)
...
**Problems**:
1. 8 different teams calling AI APIs directly → No cost visibility
2. Users hitting provider rate limits → App crashes
3. Single region deployment → Downtime when AWS us-east-1 has outages
4. No way to switch providers → Locked into one vendor
...
│   │   ├── rate-limiter.ts      # Hierarchical rate limiting
│   │   └── cost-tracker.ts      # FinOps tracking
│   ├── router/
│   │   ├── gateway.ts           # Main routing logic
│   │   └── providers/
│   │       ├── anthropic.ts
│   │       ├── openai.ts
│   │       └── base.ts
│   ├── health/
│   │   └── checker.ts           # Health checks
│   ├── metrics/
...
interface RateLimitConfig {
  ...
  tenant_per_hour: Record<string, number>  // tier → limit
...
for (let i = 0; i &lt; 15; i++) {
  ...
  console.log(`Request ${i + 1}:`, result.allowed ? '✅ PASS' : '❌ BLOCKED')
}
...
app.listen(PORT, () => {
  console.log(`\n🚀 Global AI Gateway running on port ${PORT}`)
...
You're now ready to architect and deploy AI systems at scale. 🎉
