...
description: "The ultimate cumulative test — synthesize agentic logic, RAG, security, compliance, and enterprise-scale governance into a sovereign AI platform"
estimatedMinutes: 60
week: 12
concept: 5
difficulty: expert
...
This exam is the **capstone of the entire 12-week program**. It requires you to synthesize every pillar — from agentic orchestration and RAG retrieval to multi-tenant governance and sovereign compliance — into a single, coherent architecture.
...
- **Regulatory Environment**: HIPAA (US), GDPR (EU), PIPEDA (Canada) — the strictest healthcare data regulations in the world
...
- **99.9% availability SLA** — doctors in emergency rooms cannot tolerate downtime
...
CharitéConnect, a major hospital group in Berlin operating 47 hospitals across Germany, wants to sign a **$2M annual contract**. Their legal team has two non-negotiable requirements:

1. **Data Residency**: All patient data — queries, RAG chunks, model responses, and audit logs — must **never leave German soil**. Not even for processing.
...
Your current architecture runs on a single AWS us-east-1 cluster with a shared PostgreSQL database and shared pgvector store. CharitéConnect's legal team has reviewed your architecture and rejected it.

**Question:** Design a **Cell-Based Infrastructure** that satisfies CharitéConnect's requirements without rebuilding your entire platform.
 ...
 * of the SovereignHealth stack — its own database, vector store,
 ...
 * that forwards requests based on tenant ID → cell mapping.
 *
 * Architecture:
 *
 *   [Doctor's Query]
 *        ↓
 *   [Global Tenant Router] (Cloudflare Workers — edge, stateless)
 *        ↓ (tenant-id → cell lookup)
 *        ├── [Cell: EU-DE] (Frankfurt) — CharitéConnect
 *        │     ├── PostgreSQL (eu-central-1)
 *        │     ├── pgvector (eu-central-1)
 *        │     ├── Redis (eu-central-1)
 *        │     ├── Claude API (EU endpoint)
 *        │     └── Audit Log (eu-central-1)
 *        │
 *        ├── [Cell: US-EAST] (Virginia) — US hospitals
 *        │     ├── PostgreSQL (us-east-1)
 *        │     ├── pgvector (us-east-1)
 *        │     ├── Redis (us-east-1)
 *        │     ├── Claude API (US endpoint)
 *        │     └── Audit Log (us-east-1)
 *        │
 *        └── [Cell: CA-CENTRAL] (Montreal) — Canadian hospitals
 *              ├── PostgreSQL (ca-central-1)
 *              ├── pgvector (ca-central-1)
 *              ├── Redis (ca-central-1)
 *              ├── Claude API (US endpoint)
 *              └── Audit Log (ca-central-1)
 */
...
class GlobalTenantRouter {
  // The router is STATELESS — it holds no patient data.
  ...
  async routeRequest(
    ...
    // Step 1: Look up tenant → cell mapping
    ...
    console.log(
      `Routing ${tenantId} → Cell ${cell.cellId} (${cell.region})`
    )
  ...
  assignTenant(mapping: TenantCellMapping): void {
    ...
    if (cell && mapping.dataResidency !== cell.country) {
      throw new Error(
        ...
        `(${mapping.dataResidency} ≠ ${cell.country})`
      )
    }
...
// Cell deployment for CharitéConnect:
//
...
// |---------------------|-------------|----------------|--------------|
// | Shared cluster      | $12K        | ❌ Violated     | ❌ Logical only |
// | Sovereign cell (DE) | $18K        | ✅ Compliant    | ✅ Physical    |
// | Dedicated hardware  | $45K        | ✅ Compliant    | ✅ Physical    |
//
// The sovereign cell costs $6K/month more than shared,
// but unlocks a $2M/year contract. ROI: 27x.
//
// CharitéConnect's $2M contract easily justifies the $72K/year
// premium for dedicated German infrastructure.
```

**The Architect's Principle:** "Data residency is not a software problem — it's an **infrastructure topology** problem. No amount of encryption or access controls satisfies 'data never leaves German soil' if your compute runs in Virginia. The only architectural answer is **physical isolation**: the database, the vector store, the model endpoint, and the audit log all live in `eu-central-1`. The Global Tenant Router is a stateless edge proxy that touches zero patient data — it only knows which tenant maps to which cell."
...
An Architect intercepts PII **before it leaves the trust boundary**. The PII never reaches the LLM — only redacted placeholders do. The LLM processes anonymized text, and the proxy restores the original values on the way back.
 ...
 *   [Doctor's Query]
 *        ↓
 *   [PII Governance Proxy] ← Runs INSIDE your cell (not at the LLM)
 *        ├── Step 1: NER Scan (detect PII entities)
 *        ├── Step 2: Redact (replace PII with tokens)
 *        ├── Step 3: Store mapping (token → original, encrypted)
 *        ↓
 *   [Redacted Query] → [LLM API] (sees "[SSN_1]" not "123-45-6789")
 *        ↓
 *   [LLM Response] (contains "[SSN_1]" references)
 *        ↓
 *   [PII Governance Proxy]
 *        ├── Step 4: Restore (replace tokens with original PII)
 *        ↓
...
interface RedactionResult {
  ...
  mappingId: string              // ID to retrieve the token → PII mapping
  piiDetected: boolean           // Were any PII entities found?
  highRiskEntities: PIIEntity[]  // SSN, MRN — always block, never send
}
...
class PIIGovernanceProxy {
  ...
  async redact(inputText: string): Promise<RedactionResult> {
    ...
    if (highRiskEntities.length > 0) {
      console.log(
        `🚨 HIGH-RISK PII DETECTED: ${highRiskEntities.length} entities ` +
  ...
  private async llmNERScan(text: string): Promise<PIIEntity[]> {
    ...
    // This runs INSIDE the sovereign cell — never leaves the region
...
//   //  Presenting with chest pain and shortness of breath."
//
//   // Step 2: Send REDACTED query to LLM (safe — no PII leaves the cell)
...
1. **Cross-Tenant Isolation**: Prove that proprietary medical research from CharitéConnect's RAG system was **never leaked** to query results for any other hospital over the last six months.
 ...
 * hash, which breaks the chain — making tampering immediately
 ...
 * Architecture:
 *
 *   Entry 1 → hash(Entry 1 + "genesis")     = H1
 *   Entry 2 → hash(Entry 2 + H1)            = H2
 *   Entry 3 → hash(Entry 3 + H2)            = H3
 *   ...
 *   Entry N → hash(Entry N + H(N-1))        = HN
 *
 *   If an admin modifies Entry 2:
 *     H2' ≠ H2 → H3 references H2, not H2' → CHAIN BROKEN
...
class SovereignAuditLog {
  ...
  async appendEntry(
    ...
    if (crossTenantAccess.length > 0) {
      ...
      // This entry is STILL logged — the violation is recorded immutably
    }
  ...
  private async createMerkleCheckpoint(
    ...
    console.log(
      `📌 Merkle checkpoint: entries ${startFrom}-${upToSequence} → ` +
...
//
// // Regulator requests: "Prove CharitéConnect's data was isolated for 6 months"
...
//   Chain integrity: ✅ VERIFIED (all 847,293 hashes valid)
...
It's January — peak flu season. The US-East sovereign cell experiences a **10x traffic spike** as emergency rooms across the Eastern seaboard flood SovereignHealth with clinical queries.
...
The result: **"Metro Trauma Center"** — a Level 1 trauma center with 200 ER doctors — is getting `429 Rate Limit Exceeded` errors. Doctors in the emergency room cannot get clinical decision support for patients in critical condition.
...
An Architect doesn't just rate-limit — they implement **tiered resource governance** where clinical urgency determines access priority. Batch jobs never compete with emergency room doctors for the same resource pool.
 ...
 *   [Incoming Query]
 *        ↓
 *   [Priority Classifier] → Assigns priority tier
 *        ↓
 *   [Tenant Rate Limiter] → Enforces per-tenant limits
 *        ↓
 *   [Priority Queue] → Orders by urgency, not arrival time
 *        ↓
 *   [Global Rate Limiter] → Enforces API rate limit
 *        ↓
 *   [LLM API]
 *
 * Priority Tiers:
 *   P0 (CRITICAL): Emergency room, ICU, surgical — NEVER throttled
 *   P1 (HIGH):     Active patient consultations — throttled last
 *   P2 (NORMAL):   Routine clinical queries — standard limits
 *   P3 (LOW):      Batch processing, reports, analytics — throttled first
 */
...
interface QueuedRequest {
  ...
  query: string
  enqueuedAt: Date
  deadline: Date                   // SLA deadline — drop if exceeded
}
...
class PriorityRateLimiter {
  ...
  async processRequest(
    ...
    if (tenantBucket.tokens < 1) {
      // Tenant over their limit — queue or reject based on priority
      if (priority <= 1) {
    ...
    if (this.globalTokenBucket.tokens < 1) {
      // Global limit hit — only P0 requests bypass
      if (priority === 0) {
        // P0 CRITICAL: Use guaranteed capacity (always reserved)
        console.log(
          `🚨 P0 BYPASS: ${config.name} using guaranteed capacity`
        )
        return { allowed: true }
      }
  ...
  activateSurgeMode(): void {
    console.log('⚡ SURGE MODE ACTIVATED — throttling batch tenants')
    ...
    for (const [tenantId, config] of this.tenantConfigs) {
      if (config.tier === 'batch') {
        ...
        console.log(
          `  ${config.name}: ${config.maxRequestsPerMinute} → ` +
...
//
//   SmallTown Clinic: 200 → 20 req/min (90% reduction)
//   Metro Trauma:     3000 → 4500 req/min (50% increase)
//
//   Result:
...
//   ✅ Physical isolation via regionalized compute cells
//   ✅ Stateless Global Tenant Router (stores no patient data)
//   ✅ Network policies blocking cross-region egress
//   ✅ Cost-benefit analysis ($6K/month premium → $2M/year contract)
//
// Challenge 2 (Compliance Firewall):
//   ✅ Pre-inference proxy intercepts PII BEFORE it leaves trust boundary
//   ✅ Two-layer NER: regex (fast, structured) + LLM (unstructured)
//   ✅ Token-based redaction with encrypted mapping inside sovereign cell
//   ✅ PII restoration on response (doctor sees full data, LLM never did)
//
// Challenge 3 (Immutable Proof):
//   ✅ Hash chaining: each entry includes hash of previous entry
//   ✅ Merkle Tree checkpoints anchored to external timestamping authority
//   ✅ Regulator can independently verify chain integrity
//   ✅ Cross-tenant isolation provable via RAG source tenant IDs
//
// Challenge 4 (Surge & Starvation):
//   ✅ Token-bucket rate limiting per tenant (not just global)
//   ✅ Priority queuing: P0 (ER) > P1 (active) > P2 (routine) > P3 (batch)
//   ✅ Surge mode: dynamically reallocates from batch to critical
//   ✅ Guaranteed capacity reserves for critical tenants
//
...
- **Challenge 1**: "Encrypt the database and add row-level security" — encryption doesn't satisfy "data never leaves German soil" when compute runs in Virginia
- **Challenge 2**: "Add a disclaimer asking doctors not to include PII" — policy-based solutions fail in practice; 100% of hospitals have accidental PII incidents
- **Challenge 3**: "Query the audit log database and export it to CSV" — an admin with database access can modify records before export; no cryptographic proof
- **Challenge 4**: "Increase the API rate limit" or "Add more servers" — doesn't address the **fairness** problem; SmallTown Clinic would still monopolize capacity

### Junior Tier (Fail)

The student fails to account for the fundamental constraints of regulated enterprise systems:

- **Challenge 1**: "Use a VPN" or "Just put it in the cloud" — no understanding of data residency as a physical infrastructure requirement
- **Challenge 2**: "Manually check logs for PII after the fact" — post-hoc detection doesn't prevent the HIPAA violation; the PII already reached the LLM provider
- **Challenge 3**: "Trust the admin not to modify logs" — trust is not a compliance strategy; regulators require mathematical proof
- **Challenge 4**: "Tell the clinic to stop running batch jobs" — operational intervention is not an architectural solution; the system must handle this autonomously
...
The difference between a developer and an architect is this: a developer builds features. An architect builds **systems that remain trustworthy under adversarial conditions, regulatory scrutiny, and 10x traffic spikes** — simultaneously.
