...
export function LearningObjectives({ objectives, accentColor = 'text-purple-600' }: LearningObjectivesProps) {
  return (
          ...
          <ul className="space-y-3 mt-2">
            {objectives.map((objective, i) => (
              <li key={i} className="flex items-start gap-3">
                <CheckCircle2 className={`h-5 w-5 ${accentColor} mt-0.5 flex-shrink-0`} />
