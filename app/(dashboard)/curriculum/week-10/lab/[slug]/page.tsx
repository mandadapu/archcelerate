...
export default async function LabPage({ params }: Props) {
  ...
  async function submitExercise(formData: FormData) {
    ...
    if (allSubmissions.length === totalExercises) {
      await prisma.userWeekProgress.upsert({
        ...
        create: {
          userId: user.id,
          weekId: weekId,
          conceptsTotal: 3, // Week 10 has 4 concepts
          labCompleted: true
        },
        update: {
          labCompleted: true
    ...
    redirect(`/curriculum/week-10/lab/${params.slug}`)
  }
      ...
      {completedCount === exercises.length && (
        <Card className="mt-8 bg-gradient-to-br from-green-50 to-blue-50 dark:from-green-950 dark:to-blue-950 border-green-200">
          <CardHeader>
            ...
            <CardDescription>
              Great work! You&apos;ve completed all exercises. Head back to Week 10 to continue with the project.
            </CardDescription>
          </CardHeader>
          <CardContent>
