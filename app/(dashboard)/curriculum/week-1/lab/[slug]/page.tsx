...
export default async function LabPage({ params }: Props) {
  ...
  async function submitExercise(formData: FormData) {
    ...
    redirect(`/curriculum/week-1/lab/${params.slug}`)
  }
              ...
              <CardContent>
                ...
                <div className="mb-4 text-sm text-muted-foreground">
                  {exercise.type === 'visual' && (
                    <p>Build this component using a visual builder (Flowise or similar). Document your approach and save the flow configuration.</p>
                  )}
                  {exercise.type === 'analysis' && (
                    <p>Analyze the visual flow you created. Explain how each component works and how they connect together.</p>
                  )}
                  {exercise.type === 'export' && (
                    <p>Export your visual flow to code. Document the generated code and any modifications you made.</p>
                  )}
                  {exercise.type === 'coding' && (
                    <p>Rebuild the functionality from scratch using code (TypeScript or Python). Compare your implementation with the exported code.</p>
                  )}
                  {exercise.type === 'reflection' && (
                    <p>Write a reflection comparing the visual and code approaches. What were the tradeoffs? When would you use each?</p>
                  )}
                </div>
