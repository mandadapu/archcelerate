...
export default async function ConceptPage({ params }: Props) {
  ...
  async function markComplete() {
    ...
    if (nextSlug) {
      redirect(`/curriculum/week-3/concepts/${nextSlug}`)
    } else {
      redirect('/curriculum/week-3')
    }
  }
  ...
  return (
    <div className="container max-w-4xl py-8">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        <Link href="/curriculum/week-3" className="hover:text-foreground">
          Week 3
        </Link>
        <span>/</span>
        <span>Concepts</span>
      ...
      <div className="mt-12 flex items-center justify-between border-t pt-8">
        ...
        <div className="flex items-center gap-4">
          {previousConcept && (
            <Link href={`/curriculum/week-3/concepts/${previousConcept.slug}`}>
          ...
          {nextConcept && (
            <Link href={`/curriculum/week-3/concepts/${nextConcept.slug}`}>
              <Button variant="outline">
                Next
