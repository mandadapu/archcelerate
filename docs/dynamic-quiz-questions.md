...
GET /api/diagnosis/questions
    ↓
Check Redis Cache
    ├─ Cache Hit → Return cached questions (⚡ instant)
    └─ Cache Miss → Generate with Claude (🤖 ~10s)
         ↓
...
- 🤖 **AI Generated** - Freshly generated by Claude
- ⚡ **Cached** - Served from Redis cache
- 📋 **Fallback** - Static questions (if generation failed)
...
✅ **Dynamic Content**: Questions can evolve without code changes
✅ **Performance**: Redis caching = instant loading
✅ **Reliability**: Automatic fallback to static questions
✅ **Fresh Content**: Weekly refresh keeps questions updated
✅ **Cost Effective**: Claude called only once per 7 days
